(function () {
    ...
    function EditScreenController(dataService, $state, $stateParams, common, $filter, $scope, commonService, auth, notificationService, busyIndicatorService, $modal, $rootScope) {
        ...
        function activate() {
            ...
            sidePanelManager.setTitle(commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_1'));
        ...
        function initDictionary() {
            //初始化 物料分类
            self.typeDepartment = {
                value: { ItemName: commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_2'), ItemValue: "" },
                options: [{ ItemName: commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_2'), ItemValue: "" }]
            };
            self.typeData = {
                value: { ItemName: commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_2'), ItemValue: "" },
                options: [{ ItemName: commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_2'), ItemValue: "" }]
            ...
            self.typeEnabled = {
                value: { ItemValue: true, ItemName: commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_3') },
                options: [
                    { ItemValue: true, ItemName: commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_3') },
                    { ItemValue: false, ItemName: commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_4') }
                ]
            };
        ...
        function save() {
            busyIndicatorService.show({ message: commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_5') });
        ...
        function onSaveSuccess(data) {
            ...
            if (data.data.success) {
                ...
                //关闭侧边栏
                sidePanelManager.close();
                commonService.showInfo(commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_6'));
                //刷新局部
                $rootScope.$emit('to-parentDetail', 'parent');
                $state.go('^', {}, { reload: false });
            } else {
                busyIndicatorService.hide();
                backendService.genericError(data.data.returnMsg, commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_7'));
            }
        }
        ...
        function onSaveError(error) {
            busyIndicatorService.hide();
            backendService.genericError('[' + error.status + '] - ' + error.statusText, commonService.$t('Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_7'));
        }
        ...
        var state = {
            ...
            views: {
                'property-area-container@': {
                    templateUrl: moduleFolder + '/TestMaintenance-editDetail.html',
                    controller: EditScreenController,
                    controllerAs: 'vm'
                }
            },
            data: {
                title: 'Siemens.SimaticIT.QualityApp.IPQCManage.TestMaintenanceeditDetailctrl.Tips_8'
            },
            params: {
                selectedItem: null,
            }
