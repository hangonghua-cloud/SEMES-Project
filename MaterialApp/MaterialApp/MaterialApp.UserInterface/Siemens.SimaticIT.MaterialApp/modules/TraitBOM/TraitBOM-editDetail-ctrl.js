(function () {
    ...
    function EditScreenController(dataService, $state, $stateParams, common, $filter, $scope,
        ...
        function activate() {
            ...
            sidePanelManager.setTitle(commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_1'));
        ...
        function initDictionary() {
            //初始化 物料分类
            self.MaterialClass = {
                value: { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), ItemValue: "" },
                options: [{ ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), ItemValue: "" }]
            };
            ...
            self.Warehouse = {
                value: { ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), ResourceCode: "" },
                options: [{ ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), ResourceCode: "" }]
            };
            ...
            self.TypeBom = {
                value: { BOMCode: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), Id: "" },
                options: [{ BOMCode: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), Id: "" }]
            };
            ...
            commonService.getWarehouseByFactory({ factoryCode: self.currentItem.FactoryCode }).then(function (res) {
                if (res && res.data.success) {
                    ...
                    self.Warehouse.options.splice(0, 0, {
                        ResourceCode: "",
                        ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2')
                    });
        ...
        function materialChange(oldvalu, newvalue) {
            ...
            commonService.callWebApiGet(url1, null).then(function (res) {
                if (res && res.data.success && res.data.resultData.length > 0) {
                    ...
                    self.MaterialClass.value = { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), ItemValue: "" };
                    self.SmallClass.value = { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), ItemValue: "" };
                    self.typeUnit.value = { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), ItemValue: "" };
                }
            });
            ...
            commonService.callWebApiGet(url2, null).then(function (res) {
                if (res && res.data.success) {
                    ...
                    self.TypeBom.options.splice(0, 0, {
                        Id: "",
                        BOMCode: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2')
                    });
                    if (!!self.currentItem.BOMCode) {
                        self.TypeBom.value = self.TypeBom.options.find(t => t.BOMCode == self.currentItem.BOMCode);
                    }
                } else {
                    self.TypeBom.value = { BOMCode: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_2'), Id: "" };
                }
            });
        ...
        function save() {
            ...
            if (!self.currentItem.MaterialName) {
                backendService.genericError(commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_3'));
                return;
            }
            busyIndicatorService.show({ message: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_4') });
        ...
        function onSaveSuccess(data) {
            ...
            if (data.data.success) {
                ...
                //关闭侧边栏
                sidePanelManager.close();
                commonService.showInfo(commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_5'));
                //刷新局部
                $rootScope.$emit('to-parentDetail', 'parent');
                $state.go('^', {}, { reload: false });
            } else {
                busyIndicatorService.hide();
                backendService.genericError(data.data.returnMsg, commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_6'));
            }
        }
        ...
        function onSaveError(error) {
            busyIndicatorService.hide();
            backendService.genericError('[' + error.status + '] - ' + error.statusText, commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_6'));
        }
        ...
        var state = {
            ...
            views: {
                'property-area-container@': {
                    templateUrl: moduleFolder + '/TraitBOM-editDetail.html',
                    controller: EditScreenController,
                    controllerAs: 'vm'
                }
            },
            data: {
                title: 'Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMeditDetailctrl.Tips_1'
            },
            params: {
                selectedItem: null,
            }
