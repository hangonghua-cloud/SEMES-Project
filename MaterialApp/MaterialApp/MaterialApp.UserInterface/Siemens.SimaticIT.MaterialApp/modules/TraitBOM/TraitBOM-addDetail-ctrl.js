(function () {
    ...
    function AddScreenController(dataService, $state, $stateParams, common, $filter, $scope,
        ...
        function activate() {
            ...
            sidePanelManager.setTitle(commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_1'));
        ...
        function initDictionary() {
            //初始化 物料分类
            self.MaterialClass = {
                value: { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), ItemValue: "" },
                options: [{ ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), ItemValue: "" }]
            };
            ...
            self.Warehouse = {
                value: { ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), ResourceCode: "" },
                options: [{ ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), ResourceCode: "" }]
            };
            ...
            self.TypeBom = {
                value: { BOMCode: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), Id: "" },
                options: [{ BOMCode: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), Id: "" }]
            };
            ...
            commonService.getWarehouseByFactory({ factoryCode: self.currentItem.FactoryCode }).then(function (res) {
                if (res && res.data.success) {
                    ...
                    self.Warehouse.options.splice(0, 0, {
                        ResourceCode: "",
                        ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2')
                    });
                }
            });
            // 获取工序
        ...
        function materialChange(oldvalu, newvalue) {
            ...
            commonService.callWebApiGet(url1, null).then(function (res) {
                if (res && res.data.success && res.data.resultData.length > 0) {
                    ...
                    self.MaterialClass.value = { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), ItemValue: "" };
                    self.SmallClass.value = { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), ItemValue: "" };
                    self.typeUnit.value = { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), ItemValue: "" };
                }
            });
            //获取物料BOM信息
            ...
            commonService.callWebApiGet(url2, null).then(function (res) {
                if (res && res.data.success) {
                    ...
                    self.TypeBom.options.splice(0, 0, {
                        Id: "",
                        BOMCode: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2')
                    });

                } else {
                    self.TypeBom.value = { BOMCode: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_2'), Id: "" };
                }
            });
        ...
        function save() {
            ...
            if (!self.currentItem.MaterialName) {
                backendService.genericError(commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_3'));
                return;
            }
            busyIndicatorService.show({ message: commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_4') });
            //字典类型 取值参考
        ...
        function onSaveSuccess(data) {
            if (data.data.success) {
                ...
                //关闭侧边栏
                sidePanelManager.close();
                commonService.showInfo(commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_5'));
                //刷新局部
                $rootScope.$emit('to-parentdetail', 'parent');
                $state.go('^', {}, { reload: false });
            } else {
                busyIndicatorService.hide();
                backendService.genericError(data.data.returnMsg, commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_6'));
            }
        }
        function onSaveError(error) {
            busyIndicatorService.hide();
            backendService.genericError('[' + error.status + '] - ' + error.statusText, commonService.$t('Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_6'));
        }
        ...
        var state = {
            ...
            views: {
                'property-area-container@': {
                    templateUrl: moduleFolder + '/TraitBOM-addDetail.html',
                    controller: AddScreenController,
                    controllerAs: 'vm'
                }
            },
            data: {
                title: 'Siemens.SimaticIT.MaterialApp.TraitBOM.TraitBOMaddDetailctrl.Tips_1'
            },
            params: {
                selectedItem: null,
            }
