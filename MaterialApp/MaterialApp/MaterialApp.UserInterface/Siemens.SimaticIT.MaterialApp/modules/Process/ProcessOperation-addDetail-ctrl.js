(function () {
    ...
    function AddScreenController(dataService, $state, $stateParams, common, $filter, $scope, commonService, auth, notificationService, busyIndicatorService, $modal, $rootScope) {
        ...
        function activate() {
            ...
            sidePanelManager.setTitle(commonService.$t('Siemens.SimaticIT.MaterialApp.Process.ProcessOperationaddDetailctrl.Tips_1'));
        ...
        function initDictionary() {
            //初始化
            self.typeOperation = {
                value: { ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.Process.ProcessOperationaddDetailctrl.Tips_2'), ResourceCode: "" },
                options: [{ ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.Process.ProcessOperationaddDetailctrl.Tips_2'), ResourceCode: "" }]
            }
            ...
            commonService.getProcessByFactory({ LevelCode: self.currentItem.FactoryCode }).then(function (res) {
                if (res && res.data.success) {
                    ...
                    self.typeOperation.options.splice(0, 0, {
                        ResourceCode: "",
                        ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.Process.ProcessOperationaddDetailctrl.Tips_2')
                    });
                }
            });
        ...
        function save() {
            ...
            busyIndicatorService.show({ message: commonService.$t('Siemens.SimaticIT.MaterialApp.Process.ProcessOperationaddDetailctrl.Tips_3') });
        ...
        function onSaveSuccess(data) {
            ...
            if (data.data.success) {
                ...
                //关闭侧边栏
                // sidePanelManager.close();
                commonService.showInfo(commonService.$t('Siemens.SimaticIT.MaterialApp.Process.ProcessOperationaddDetailctrl.Tips_4'));
                //刷新局部
                ...
                busyIndicatorService.hide();
                backendService.genericError(data.data.returnMsg, commonService.$t('Siemens.SimaticIT.MaterialApp.Process.ProcessOperationaddDetailctrl.Tips_5'));
            }
        }
        //保存失败事件
        function onSaveError(error) {
            busyIndicatorService.hide();
            backendService.genericError('[' + error.status + '] - ' + error.statusText, commonService.$t('Siemens.SimaticIT.MaterialApp.Process.ProcessOperationaddDetailctrl.Tips_5'));
        }
        ...
        var state = {
            ...
            views: {
                'property-area-container@': {
                    templateUrl: moduleFolder + '/ProcessOperation-addDetail.html',
                    controller: AddScreenController,
                    controllerAs: 'vm'
                }
            },
            data: {
                title: 'Siemens.SimaticIT.MaterialApp.Process.ProcessOperationaddDetailctrl.Tips_1'
            },
            params: {
                selectedItem: null,
            }
