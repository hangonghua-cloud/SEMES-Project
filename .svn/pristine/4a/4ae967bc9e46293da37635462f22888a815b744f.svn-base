...
{ 
    ...
    /// 1.创建日期: 2021-08-26
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PM_AbrasiveBGService 业务服务类
    /// 4.任务编号: 磨粉料工单管理
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_AbrasiveBG_Service : RepositoryFactory<PM_AbrasiveBGEntity>, PM_AbrasiveBGIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<PM_AbrasiveBGEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PM_AbrasiveBG] where 1=1  ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                // 是否为空进行查询
                if (!queryParam["MaterailCode"].IsEmpty())
                {
                    //sql.Append($" AND MaterailCode = N'{queryParam["MaterailCode"]}'");
                    sql.Append($" AND MaterailCode like N'%{queryParam["MaterailCode"]}%'");
                }
                // 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT a.Id,
                               ...
                               a.BatchNo,
                               a.ProcessCode,
                               a.Machine,a.PackingType,
                               b.ResourceName MachineName,
                               a.Shift,
                               v1.ItemName ShiftName,
                               a.BGQty,
                               a.UserGroup,
							   a.UserNames,
                               a.Creator,
                               a.CreateTime,
                               a.BGDate
                        FROM dbo.PM_AbrasiveBG a
                            LEFT JOIN dbo.BS_ModelWithResource b
                                ON b.ModelLeve = 'Machine'
                                   AND b.ParentResource = a.ProcessCode
                                   AND a.Machine = b.ResourceCode
                            LEFT JOIN dbo.V_DataDictionary v1
                                ON v1.EnCode = 'Shift'
                                   AND a.Shift = v1.ItemValue
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //磨粉料工单Id 是否为空进行查询
                if (!queryParam["AbrasiveId"].IsEmpty())
                {
                    //sql.Append($" AND AbrasiveId = N'{queryParam["AbrasiveId"]}'");
                    sql.Append($" AND a.AbrasiveId like N'%{queryParam["AbrasiveId"]}%'");
                }
                //生成批次 是否为空进行查询
                ...
                //生产机台 是否为空进行查询
                if (!queryParam["Machine"].IsEmpty())
                {
                    //sql.Append($" AND Machine = N'{queryParam["Machine"]}'");
                    sql.Append($" AND Machine like N'%{queryParam["Machine"]}%'");
                }
                //班次 是否为空进行查询
                if (!queryParam["Shift"].IsEmpty())
                {
                    //sql.Append($" AND Shift = N'{queryParam["Shift"]}'");
                    sql.Append($" AND Shift like N'%{queryParam["Shift"]}%'");
                }
                //报工数量 是否为空进行查询
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_AbrasiveBGEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, PM_AbrasiveBGEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PM_AbrasiveBGEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_AbrasiveBGEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_AbrasiveBG] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PM_AbrasiveBG] (
                                            [Id]
                                            ,[AbrasiveId]
                                            ,[BatchNo]
                                            ,[ProcessCode]
        ...
        {
            int result = 0;
            PM_AbrasiveBGEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_AbrasiveBG] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_AbrasiveBGEntity</returns>
        public PM_AbrasiveBGEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_AbrasiveBGEntity</returns>
        public PM_AbrasiveBGEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_AbrasiveBGEntity 对象</returns>
        public PM_AbrasiveBGEntity Get_ExpressionEntity(Expression<Func<PM_AbrasiveBGEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_AbrasiveBGEntity 列表</returns>
        public IEnumerable<PM_AbrasiveBGEntity> Get_ExpressionList(Expression<Func<PM_AbrasiveBGEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_AbrasiveBGEntity> bomService = new RepositoryFactory<PM_AbrasiveBGEntity>();
        
        //    PM_AbrasiveBGEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_AbrasiveBGDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_AbrasiveBG_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_AbrasiveBGEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_AbrasiveBGEntity> PM_AbrasiveBGEntity_list =  db2.FindList<PM_AbrasiveBGEntity>(sql.ToString());
                return PM_AbrasiveBGEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_AbrasiveBGEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_AbrasiveBGEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public int RemoveForm(Expression<Func<PM_AbrasiveBGEntity, bool>> condition)
        {
