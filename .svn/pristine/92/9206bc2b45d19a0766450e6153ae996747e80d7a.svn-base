...
namespace ALP.Application.Service.ProduceManage
{
    ...
    /// 1.创建日期: 2021-08-26
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PM_OwnProductTransferService 业务服务类
    /// 4.任务编号: 自制半成品工单
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_OwnProductTransfer_Service : RepositoryFactory<PM_OwnProductTransferEntity>, PM_OwnProductTransferIService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<PM_OwnProductTransferEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PM_OwnProductTransfer] where 1=1  ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT T.[Id],
                               ...
                               PB.BGProcess,
                               PB.BGQty,
                               M.ResourceName BGProcessName,
                               P.Name CreatorName
                        FROM [dbo].[PM_OwnProductTransfer] T
                            LEFT JOIN dbo.PM_OwnProductBG PB
                                ON PB.TransferCode = T.TransferCode
                            LEFT JOIN dbo.BS_ModelWithResource M
                                ON M.ResourceCode = PB.BGProcess
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                if (!queryParam["WorkOrder"].IsEmpty())
                {
                    sql.Append($" AND T.WorkOrder = N'{queryParam["WorkOrder"]}'");
                }
                //流转卡号 是否为空进行查询
                if (!queryParam["TransferCode"].IsEmpty())
                {
                    //sql.Append($" AND TransferCode = N'{queryParam["TransferCode"]}'");
                    sql.Append($" AND T.TransferCode like N'%{queryParam["TransferCode"]}%'");
                }
                //流转卡名称 是否为空进行查询
                if (!queryParam["TransferName"].IsEmpty())
                {
                    //sql.Append($" AND TransferName = N'{queryParam["TransferName"]}'");
                    sql.Append($" AND T.TransferName like N'%{queryParam["TransferName"]}%'");
                }
                //流转卡状态 是否为空进行查询
                if (!queryParam["TransferStatus"].IsEmpty())
                {
                    sql.Append($" AND T.TransferStatus = N'{queryParam["TransferStatus"]}'");
                }
        ...
        /// <param name="List<PM_OwnProductTransferEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_OwnProductTransfer] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            PM_OwnProductTransferEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_OwnProductTransfer] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_OwnProductTransferEntity</returns>
        ...
        /// <returns>返回PM_OwnProductTransferEntity 对象</returns>
        ...
        public List<dynamic> CodeBarScan(string WorkOrder, string batchNo)
        {
            string sql = $@"SELECT A.WorkOrder,
                                   ...
                                   c.MaterialName,
                                   A.[TransferStatus],
                                   CASE A.TransferStatus
                                       WHEN '1' THEN
                                           '未报工'
                                       ELSE
                                           '已报工'
                                   END TransferStatusName,
                                   A.CardStatus,
                                   v1.ItemName CardStatusName,
                                   b.BGQty
                            FROM dbo.PM_OwnProductTransfer A
                                LEFT JOIN
                                (
                                    SELECT b1.TransferCode,
                                           SUM(b1.BGQty) BGQty
                                    FROM dbo.PM_OwnProductBG b1
                                    GROUP BY b1.TransferCode
                                ) b
                                    ON A.TransferCode = b.TransferCode
                                LEFT JOIN dbo.PM_OwnProductOrder c
                                    ON A.WorkOrder = c.WorkOrder
                                LEFT JOIN dbo.V_DataDictionary v1
                                    ON v1.EnCode = 'CirculationCardStatus'
                                       AND A.CardStatus = v1.ItemValue
                            WHERE A.WorkOrder = '{WorkOrder}'
                                  AND A.BatchNumber = '{batchNo}'
                            ORDER BY A.TransferName ";
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_OwnProductTransferEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_OwnProductTransferEntity> bomService = new RepositoryFactory<PM_OwnProductTransferEntity>();

        //    PM_OwnProductTransferEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_OwnProductTransferDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_OwnProductTransfer_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<PM_OwnProductTransferEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_OwnProductTransferEntity> PM_OwnProductTransferEntity_list = db2.FindList<PM_OwnProductTransferEntity>(sql.ToString());
                return PM_OwnProductTransferEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_OwnProductTransferEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_OwnProductTransferEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [WorkOrder] as '自制半成品Id'
                      ,[TransferCode] as '流转卡号'
                      ,[TransferName] as '流转卡名称'
                      ,[TransferStatus] as '流转卡状态'
                      ,[Creator] as '创建人'
                      ,[CreateTime] as '创建时间'
                      ,[ModifyBy] as '最后修改人'
