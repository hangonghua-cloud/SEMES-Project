...
{ 
    ...
    /// 1.创建日期: 2021-08-19
    /// 2.创建作者: liyongguo
    /// 3.功能描述: QC_TestMaintenanceService 业务服务类
    /// 4.任务编号: QC检测维护
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class QC_TestMaintenance_Service : RepositoryFactory<QC_TestMaintenanceEntity>, QC_TestMaintenanceIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<QC_TestMaintenanceEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[IsEnabled],SmallClass
                      ,[Creator]
                      ,[CreateTime]
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[QC_TestMaintenance] where IsEnabled= 1 ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT Q.[Id],
                        ...
                        FROM [dbo].[QC_TestMaintenance] Q
                            LEFT JOIN dbo.V_DataDictionary V
                                ON V.EnCode = 'PQCInspection'
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<QC_TestMaintenanceEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, QC_TestMaintenanceEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<QC_TestMaintenanceEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<QC_TestMaintenanceEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[QC_TestMaintenance] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            QC_TestMaintenanceEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[QC_TestMaintenance] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回QC_TestMaintenanceEntity</returns>
        public QC_TestMaintenanceEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回QC_TestMaintenanceEntity</returns>
        public QC_TestMaintenanceEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回QC_TestMaintenanceEntity 对象</returns>
        public QC_TestMaintenanceEntity Get_ExpressionEntity(Expression<Func<QC_TestMaintenanceEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回QC_TestMaintenanceEntity 列表</returns>
        public IEnumerable<QC_TestMaintenanceEntity> Get_ExpressionList(Expression<Func<QC_TestMaintenanceEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<QC_TestMaintenanceEntity> bomService = new RepositoryFactory<QC_TestMaintenanceEntity>();
        
        //    QC_TestMaintenanceEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    QC_TestMaintenanceDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.QC_TestMaintenance_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<QC_TestMaintenanceEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<QC_TestMaintenanceEntity> QC_TestMaintenanceEntity_list =  db2.FindList<QC_TestMaintenanceEntity>(sql.ToString());
                return QC_TestMaintenanceEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable QC_TestMaintenanceEntity_DataTable = db2.FindTable(sql.ToString());
                return QC_TestMaintenanceEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public int RemoveForm(Expression<Func<QC_TestMaintenanceEntity, bool>> condition)
        {
