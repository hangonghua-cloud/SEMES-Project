...
{ 
    ...
    /// 1.创建日期: 2021-07-27
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PL_LoadingScheduleService 业务服务类
    /// 4.任务编号: 装柜计划表
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PL_LoadingSchedule_Service : RepositoryFactory<PL_LoadingScheduleEntity>, PL_LoadingScheduleIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        /// 创建日期: 2021-07-27 16:27:50
        /// 任务编号: 装柜计划表
        /// </summary>
        ...
        public IEnumerable<PL_LoadingScheduleEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PL_LoadingSchedule] where 1=1  ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@" SELECT 
					        PW.ProductOrder,Pw.WorkOrder,PW.ContainerNO,PW.CustomerPO,PW.OrderPieces,
					        PL.Id,PL.LoadingDate,PL.InvoiceNO,PL.LoadingBill,PL.BoxType,PL.GrossWeight,PL.Volume
				          FROM dbo.PL_WorkOrder PW
				          LEFT JOIN [dbo].[PL_LoadingSchedule] PL ON PW.WorkOrder=PL.WorkOrder
				          WHERE  1=1  ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                if (!queryParam["workOrderString"].IsEmpty())
                {
                    sql.Append(" and PW.WorkOrder in (" + queryParam["workOrderString"].ToString() + ")");
        ...
        /// <param name="List<PL_LoadingScheduleEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PL_LoadingScheduleEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PL_LoadingSchedule] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PL_LoadingSchedule] (
                                            [Id]
                                            ,[FactoryCode]
                                            ,[ProductOrder]
                                            ,[WorkOrder]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,'{(Save_obj.LoadingDate == null? DateTime.Now:Save_obj.LoadingDate)}'
        ...
        {
            int result = 0;
            PL_LoadingScheduleEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PL_LoadingSchedule] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PL_LoadingScheduleEntity</returns>
        ...
        /// <returns>返回PL_LoadingScheduleEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PL_LoadingScheduleEntity> bomService = new RepositoryFactory<PL_LoadingScheduleEntity>();
        
        //    PL_LoadingScheduleEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PL_LoadingScheduleDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PL_LoadingSchedule_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<PL_LoadingScheduleEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PL_LoadingScheduleEntity> PL_LoadingScheduleEntity_list =  db2.FindList<PL_LoadingScheduleEntity>(sql.ToString());
                return PL_LoadingScheduleEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PL_LoadingScheduleEntity_DataTable = db2.FindTable(sql.ToString());
                return PL_LoadingScheduleEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "装柜计划表_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("装柜计划表", dt, true);
                //保存
