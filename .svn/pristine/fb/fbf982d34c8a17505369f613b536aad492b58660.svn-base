...
{ 
    ...
    /// 1.创建日期: 2021-07-21
    /// 2.创建作者: liyongguo
    /// 3.功能描述: Base_MaterialBindTempService 业务服务类
    /// 4.任务编号: 任务名称或编号
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class Base_MaterialBindTemp_Service : RepositoryFactory<Base_MaterialBindTempEntity>, Base_MaterialBindTempIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<Base_MaterialBindTempEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[Base_MaterialBindTemp] where IsEnabled=1  ");
        ...
        /// <param name="List<Base_MaterialBindTempEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<Base_MaterialBindTempEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[Base_MaterialBindTemp] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[Base_MaterialBindTemp] (
                                            [Id]
                                            ,[TempCode]
                                            ,[TempName]
                                            ,[Sort]
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[Base_MaterialBindTemp] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回Base_MaterialBindTempEntity</returns>
        ...
        /// <returns>返回Base_MaterialBindTempEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回Base_MaterialBindTempEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<Base_MaterialBindTempEntity> bomService = new RepositoryFactory<Base_MaterialBindTempEntity>();
        
        //    Base_MaterialBindTempEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    Base_MaterialBindTempDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.Base_MaterialBindTemp_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<Base_MaterialBindTempEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<Base_MaterialBindTempEntity> Base_MaterialBindTempEntity_list =  db2.FindList<Base_MaterialBindTempEntity>(sql.ToString());
                return Base_MaterialBindTempEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable Base_MaterialBindTempEntity_DataTable = db2.FindTable(sql.ToString());
                return Base_MaterialBindTempEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
