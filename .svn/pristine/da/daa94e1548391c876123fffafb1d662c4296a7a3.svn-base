(function () {
    ...
    function AddScreenController(dataService, $state, $stateParams, common, $filter, $scope, commonService, auth, notificationService, busyIndicatorService, $modal, $rootScope) {
        ...
        function activate() {
            ...
            sidePanelManager.setTitle(commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_1'));
        ...
        function initDictionary() {
            self.PTeam = {
                value: { PTeamName: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_2'), PTeamCode: "" },
                options: [{ PTeamName: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_2'), PTeamCode: "" }]
            };
            self.User = {
                value: { UserName: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_2'), UserCode: "" },
                options: [{ UserName: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_2'), UserCode: "" }]
            };
            self.Post = {
                value: { Col2: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_2'), Col1: "" },
                options: [{ Col2: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_2'), Col1: "" }]
            };
            //生产小组
            ...
            commonService.callWebApiGet(url).then(function (res) {
                if ((res) && (res.data.success)) {
                    ...
                    self.PTeam.options.splice(0, 0, {
                        PTeamCode: "",
                        PTeamName: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_2')
                    });
                }
            });
            ...
            commonService.getKeyParameterItem({ ItemCode: self.currentItem.ProcessCode, EnCode: "ProcessPost" }).then(function (res) {
                if (res && res.data.success) {
                    ...
                    self.Post.options.splice(0, 0, {
                        Col1: "",
                        Col2: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_2')
                    });
                }
            });
            ...
            commonService.callWebApiGet(url).then(function (res) {
                if ((res) && (res.data.success)) {
                    ...
                    self.User.options.splice(0, 0, {
                        UserCode: "",
                        UserName: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_2')
                    });
                }
            });
        }
        ...
        //保存
        function save() {
            busyIndicatorService.show({ message: commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_3') });
        ...
        function onSaveSuccess(data) {
            ...
            if (data.data.success) {
                ...
                //关闭侧边栏
                sidePanelManager.close();
                commonService.showInfo(commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_4'));
                //刷新局部
                $rootScope.$emit('to-parentPerson', 'parent');
                $state.go('^', {}, { reload: false });
            } else {
                busyIndicatorService.hide();
                backendService.genericError(data.data.returnMsg, commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_5'));
            }
        }
        ...
        function onSaveError(error) {
            busyIndicatorService.hide();
            backendService.genericError('[' + error.status + '] - ' + error.statusText, commonService.$t('Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_5'));
        }
        ...
        var state = {
            ...
            views: {
                'property-area-container@': {
                    templateUrl: moduleFolder + '/TransferCard-addUser.html',
                    controller: AddScreenController,
                    controllerAs: 'vm'
                }
            },
            data: {
                title: 'Siemens.SimaticIT.ProductionApp.TransferList.addUserJS.Tips_6'
            },
            params: {
                selectedItem: null
            }
