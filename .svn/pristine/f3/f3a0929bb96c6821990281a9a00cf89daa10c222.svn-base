...
{ 
    ...
    /// 1.创建日期: 2021-08-18
    /// 2.创建作者: admin
    /// 3.功能描述: PM_MaterialBatchConsumeRecordService 业务服务类
    /// 4.任务编号: PM_物料批次绑定记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_MaterialBatchConsumeRecord_Service : RepositoryFactory<PM_MaterialBatchConsumeRecordEntity>, PM_MaterialBatchConsumeRecord_IService
    {
        ...
        public DataTable GetPageList1(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT
					PB.CardCode,PT.CardName,PB.ProcessCode,PMB.MaterialCode,PMB.MaterialName,PMB.Spec,PMB.BatchNo,PMB.RecoilQty,
					PMB.CreateTime,M.ResourceName ProcessName,PMB.Unit,bm.SupplierName
				  FROM dbo.PM_TransferCard PT
				  INNER JOIN dbo.PM_TranferCardBGRecord PB ON PB.CardCode = PT.CardCode
				  INNER JOIN [dbo].PM_MaterialBatchConsumeRecord PMB ON PB.Id=PMB.BGID
				  LEFT JOIN dbo.BS_ModelWithResource M ON M.ResourceCode=PB.ProcessCode
				  LEFT JOIN  [dbo].MM_RawMaterialStock RO ON RO.BatchNo = PMB.BatchNo
			LEFT JOIN Base_SupplierManage BM ON RO.SupplierCode =BM.SupplierCode 
             WHERE 1=1");
        ...
        public IEnumerable<PM_MaterialBatchConsumeRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,[IsEnabled]
                  FROM [dbo].[PM_MaterialBatchConsumeRecord] where 1=1  ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                JObject queryParam = queryJson.ToJObject();
                ...
                if (!queryParam["GroupCode"].IsEmpty())
                {
                    ...
                    sql.Append($" AND GroupCode like N'%{queryParam["GroupCode"]}%'");
                }
                //物料批次 是否为空进行查询
                ...
                //倒冲数量 是否为空进行查询
                if (!queryParam["RecoilQty"].IsEmpty())
                {
                    //sql.Append($" AND RecoilQty = N'{queryParam["RecoilQty"]}'");
                    sql.Append($" AND RecoilQty like N'%{queryParam["RecoilQty"]}%'");
                }
                //创建人编码 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,[IsEnabled]
                  FROM [dbo].[PM_MaterialBatchConsumeRecord] where IsEnabled=1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                JObject queryParam = queryJson.ToJObject();
        ...
        /// <param name="List<PM_MaterialBatchConsumeRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_MaterialBatchConsumeRecord] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            PM_MaterialBatchConsumeRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_MaterialBatchConsumeRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_MaterialBatchConsumeRecordEntity</returns>
        ...
        /// <returns>返回PM_MaterialBatchConsumeRecordEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_MaterialBatchConsumeRecordEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_MaterialBatchConsumeRecordEntity> bomService = new RepositoryFactory<PM_MaterialBatchConsumeRecordEntity>();
        
        //    PM_MaterialBatchConsumeRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_MaterialBatchConsumeRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_MaterialBatchConsumeRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        public IEnumerable<PM_MaterialBatchConsumeRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_MaterialBatchConsumeRecordEntity> PM_MaterialBatchConsumeRecordEntity_list =  db2.FindList<PM_MaterialBatchConsumeRecordEntity>(sql.ToString());
                return PM_MaterialBatchConsumeRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_MaterialBatchConsumeRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_MaterialBatchConsumeRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "PM_物料批次绑定记录_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("PM_物料批次绑定记录", dt, true);
                //保存
