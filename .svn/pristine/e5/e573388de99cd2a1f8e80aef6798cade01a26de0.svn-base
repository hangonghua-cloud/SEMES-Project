...
{ 
    ...
    /// 1.创建日期: 2021-08-19
    /// 2.创建作者: liyongguo
    /// 3.功能描述: QC_TestProcessMaintenanceService 业务服务类
    /// 4.任务编号: QC检测维护
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class QC_TestProcessMaintenance_Service : RepositoryFactory<QC_TestProcessMaintenanceEntity>, QC_TestProcessMaintenanceIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        /// 创建日期: 2021-08-19 09:53:08
        /// 任务编号: QC检测维护
        /// </summary>
        /// <param name="pagination">分页</param>
        /// <param name="queryJson">查询参数</param>
        /// <returns>返回分页列表</returns>
        public IEnumerable<QC_TestProcessMaintenanceEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[QC_TestProcessMaintenance] where 1=1 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                if (!queryParam["TestMaintenanceId"].IsEmpty())
                {
                    ...
                    sql.Append($" AND TestMaintenanceId like N'%{queryParam["TestMaintenanceId"]}%'");
                }
                //工序 是否为空进行查询
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<QC_TestProcessMaintenanceEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, QC_TestProcessMaintenanceEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<QC_TestProcessMaintenanceEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<QC_TestProcessMaintenanceEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[QC_TestProcessMaintenance] set ");
                            string keyValue = "";
                            //循环实体
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[QC_TestProcessMaintenance] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回QC_TestProcessMaintenanceEntity</returns>
        public QC_TestProcessMaintenanceEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回QC_TestProcessMaintenanceEntity</returns>
        public QC_TestProcessMaintenanceEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回QC_TestProcessMaintenanceEntity 对象</returns>
        public QC_TestProcessMaintenanceEntity Get_ExpressionEntity(Expression<Func<QC_TestProcessMaintenanceEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回QC_TestProcessMaintenanceEntity 列表</returns>
        public IEnumerable<QC_TestProcessMaintenanceEntity> Get_ExpressionList(Expression<Func<QC_TestProcessMaintenanceEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<QC_TestProcessMaintenanceEntity> bomService = new RepositoryFactory<QC_TestProcessMaintenanceEntity>();
        
        //    QC_TestProcessMaintenanceEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    QC_TestProcessMaintenanceDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.QC_TestProcessMaintenance_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<QC_TestProcessMaintenanceEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<QC_TestProcessMaintenanceEntity> QC_TestProcessMaintenanceEntity_list =  db2.FindList<QC_TestProcessMaintenanceEntity>(sql.ToString());
                return QC_TestProcessMaintenanceEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable QC_TestProcessMaintenanceEntity_DataTable = db2.FindTable(sql.ToString());
                return QC_TestProcessMaintenanceEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public int RemoveForm(Expression<Func<QC_TestProcessMaintenanceEntity, bool>> condition)
        {
