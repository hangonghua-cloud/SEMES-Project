...
namespace ALP.Application.Service.ProduceManage
{
    ...
    /// 1.创建日期: 2021-08-18
    /// 2.创建作者: admin
    /// 3.功能描述: PM_BGBadRecordService 业务服务类
    /// 4.任务编号: 报工信息
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_BGBadRecord_Service : RepositoryFactory<PM_BGBadRecordEntity>, PM_BGBadRecord_IService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<PM_BGBadRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,[IsEnabled]
                  FROM [dbo].[PM_BGBadRecord] where IsDeleted = 0 ");
        ...
        public DataTable GetPageDataTableList1(Pagination pagination, string queryJson)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
					PB.CardCode,PT.CardName,PB.ProcessCode,PMB.BadItemCode,PMB.BadItemName,PMB.BadQty,PMB.CreateTime,
					M.ResourceName ProcessName
				  FROM dbo.PM_TransferCard PT
				  INNER JOIN dbo.PM_TranferCardBGRecord PB ON PB.CardCode = PT.CardCode
				  INNER JOIN [dbo].[PM_BGBadRecord] PMB ON PB.Id=PMB.BGID
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,[IsEnabled]
                  FROM [dbo].[PM_BGBadRecord] where IsEnabled=1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                JObject queryParam = queryJson.ToJObject();
                ...
                {
                    //sql.Append($" AND BadItemCode = N'{queryParam["BadItemCode"]}'");
                    sql.Append($" AND BadItemCode like N'%{queryParam["BadItemCode"]}%'");
                }
                //不良项目名称 是否为空进行查询
                if (!queryParam["BadItemName"].IsEmpty())
                {
                    //sql.Append($" AND BadItemName = N'{queryParam["BadItemName"]}'");
                    sql.Append($" AND BadItemName like N'%{queryParam["BadItemName"]}%'");
                }
                //不良数量 是否为空进行查询
                if (!queryParam["BadQty"].IsEmpty())
                {
        ...
        /// <param name="List<PM_BGBadRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_BGBadRecord] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            PM_BGBadRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_BGBadRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_BGBadRecordEntity</returns>
        ...
        /// <returns>返回PM_BGBadRecordEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_BGBadRecordEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_BGBadRecordEntity> bomService = new RepositoryFactory<PM_BGBadRecordEntity>();

        //    PM_BGBadRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_BGBadRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_BGBadRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        public IEnumerable<PM_BGBadRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_BGBadRecordEntity> PM_BGBadRecordEntity_list = db2.FindList<PM_BGBadRecordEntity>(sql.ToString());
                return PM_BGBadRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_BGBadRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_BGBadRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                      [BGID] as '报工ID'
                      ,[BadItemCode] as '不良项目编码'
                      ,[BadItemName] as '不良项目名称'
                      ,[BadQty] as '不良数量'
                      ,[Creator] as '创建人编码'
                      ,[CreateTime] as '创建时间'
                      ,[ModifyBy] as '修改人编码'
