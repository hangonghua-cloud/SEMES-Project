...
ngJSTree.directive('jsTree', ['$http', function ($http) {
  ...
  var treeDir = {
      ...
      if (a.treePlugins) {
        ...
        if (config.plugins.indexOf('search') >= 0) {
          ...
          if (e.prev().attr('class') !== 'ng-tree-search') {
            e.before('<input type="text" placeholder="搜索..." class="ng-tree-search"/>')
              .prev()
    ...
    init: function (s, e, a, config) {
      ...
      $.extend(config, a.treeConfig ? s[a.treeConfig] : config);
      this.tree = $(e).jstree(config);
      treeDir.manageEvents(s, e, a);
      if (a.treeGetAllChecked) {
        s[a.treeGetAllChecked] = function () {
          var checked = $(e).jstree("get_checked");
          var halfChecked = $(e).jstree("get_undetermined");
          var all = _.union(checked, halfChecked);
          return {
            checked: checked,
      ...
      if(a.unSelectNodes){
        s[a.unSelectNodes] = function (arr) {
          _.each(arr,function(ele,index){
            $(e).jstree('uncheck_node', ele, true );
          });
        }
      }

      if (a.checkAllNodes) {
        s[a.checkAllNodes] = function () {
          $(e).jstree("checked_all");
        }
      }
    }
  };
