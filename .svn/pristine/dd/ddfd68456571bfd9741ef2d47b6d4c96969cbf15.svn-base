...
{ 
    ...
    /// 1.创建日期: 2021-07-27
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PL_AvoidProduceService 业务服务类
    /// 4.任务编号: 免产单关系表
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PL_AvoidProduce_Service : RepositoryFactory<PL_AvoidProduceEntity>, PL_AvoidProduceIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<PL_AvoidProduceEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PL_AvoidProduce] where 1=1  ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //发货总盒数 是否为空进行查询
                if (!queryParam["LoadingDate"].IsEmpty())
                {
                    //sql.Append($" AND LoadingDate = N'{queryParam["LoadingDate"]}'");
                    sql.Append($" AND LoadingDate like N'%{queryParam["LoadingDate"]}%'");
                }
                //发货总托数 是否为空进行查询
                if (!queryParam["InvoiceNO"].IsEmpty())
                {
                    //sql.Append($" AND InvoiceNO = N'{queryParam["InvoiceNO"]}'");
                    sql.Append($" AND InvoiceNO like N'%{queryParam["InvoiceNO"]}%'");
                }
                //原订单号 是否为空进行查询
                if (!queryParam["LoadingBill"].IsEmpty())
                {
                    //sql.Append($" AND LoadingBill = N'{queryParam["LoadingBill"]}'");
                    sql.Append($" AND LoadingBill like N'%{queryParam["LoadingBill"]}%'");
                }
                //原订单PO号 是否为空进行查询
                if (!queryParam["BoxType"].IsEmpty())
                {
                    //sql.Append($" AND BoxType = N'{queryParam["BoxType"]}'");
                    sql.Append($" AND BoxType like N'%{queryParam["BoxType"]}%'");
                }
                //原柜号 是否为空进行查询
                if (!queryParam["GrossWeight"].IsEmpty())
                {
                    //sql.Append($" AND GrossWeight = N'{queryParam["GrossWeight"]}'");
                    sql.Append($" AND GrossWeight like N'%{queryParam["GrossWeight"]}%'");
                }
                //本次消化托数 是否为空进行查询
                if (!queryParam["Volume"].IsEmpty())
                {
                    //sql.Append($" AND Volume = N'{queryParam["Volume"]}'");
                    sql.Append($" AND Volume like N'%{queryParam["Volume"]}%'");
                }
                //创建人 是否为空进行查询
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PL_AvoidProduceEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, PL_AvoidProduceEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PL_AvoidProduceEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PL_AvoidProduceEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PL_AvoidProduce] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PL_AvoidProduce] (
                                            [Id]
                                            ,[ProductOrder]
                                            ,[WorkOrder]
                                            ,[CustomerPO]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,'{(Save_obj.LoadingDate == null? DateTimeOffset.Now:Save_obj.LoadingDate)}'
                                ,N'{Save_obj.InvoiceNO}'
                                ,N'{Save_obj.LoadingBill}'
                                ,N'{Save_obj.BoxType}'
                                ,{Save_obj.GrossWeight}
                                ,{Save_obj.Volume}
                                ,N'{Save_obj.Creator}'
        ...
        {
            int result = 0;
            PL_AvoidProduceEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PL_AvoidProduce] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PL_AvoidProduceEntity</returns>
        public PL_AvoidProduceEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PL_AvoidProduceEntity</returns>
        public PL_AvoidProduceEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PL_AvoidProduceEntity 列表</returns>
        public IEnumerable<PL_AvoidProduceEntity> Get_ExpressionList(Expression<Func<PL_AvoidProduceEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PL_AvoidProduceEntity> bomService = new RepositoryFactory<PL_AvoidProduceEntity>();
        
        //    PL_AvoidProduceEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PL_AvoidProduceDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PL_AvoidProduce_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PL_AvoidProduceEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PL_AvoidProduceEntity> PL_AvoidProduceEntity_list =  db2.FindList<PL_AvoidProduceEntity>(sql.ToString());
                return PL_AvoidProduceEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PL_AvoidProduceEntity_DataTable = db2.FindTable(sql.ToString());
                return PL_AvoidProduceEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "免产单关系表_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("免产单关系表", dt, true);
                //保存
        ...
        public PL_AvoidProduceEntity Get_ExpressionEntity(Expression<Func<PL_AvoidProduceEntity, bool>> condition)
        {
            return this.ReportBaseRepository().FindEntity(condition);
        }
    }
}
