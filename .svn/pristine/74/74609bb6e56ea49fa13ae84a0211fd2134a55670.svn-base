(function () {
    'use strict';
    angular.module('Siemens.SimaticIT.ProductionApp.AbrasiveOrder').controller('Siemens.SimaticIT.ProductionApp.AbrasiveOrder.EditBomItem',
        ['common.base', '$filter', '$scope', '$uibModalInstance', 'data', 'commonService',
            function (common, $filter, $scope, $modalInstance, data, commonService) {
                ...
                function initDictionary() {
                    //初始化 物料分类
                    vm.MaterialClass = {
                        value: { ItemName: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), ItemValue: "" },
                        options: [{ ItemName: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), ItemValue: "" }]
                    };
                    ...
                    vm.Warehouse = {
                        value: { ResourceName: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), ResourceCode: "" },
                        options: [{ ResourceName: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), ResourceCode: "" }]
                    };
                    ...
                    vm.TypeBom = {
                        value: { BOMCode: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), Id: "" },
                        options: [{ BOMCode: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), Id: "" }]
                    };
                    ...
                    commonService.getResourceExtendInfo({ LevelCode: "Warehouse" }).then(function (res) {
                        if (res && res.data.success) {
                            ...
                            vm.Warehouse.options.splice(0, 0, {
                                ResourceCode: "",
                                ResourceName: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1')
                            });
                        }
                        vm.Process.value = vm.Process.options.find(t => t.ResourceCode == vm.currentItem.Warehouse);
                    });
                ...
                function materialChange(oldvalu, newvalue) {
                    ...
                    commonService.callWebApiGet(url1, null).then(function (res) {
                        if (res && res.data.success && res.data.resultData.length > 0) {
                            ...
                            vm.currentItem.Spec = "";
                            vm.MaterialClass.value = { ItemName: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), ItemValue: "" };
                            vm.SmallClass.value = { ItemName: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), ItemValue: "" };
                            vm.typeUnit.value = { ItemName: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), ItemValue: "" };
                        }
                    });
                    ...
                    commonService.callWebApiGet(url2, null).then(function (res) {
                        if (res && res.data.success) {
                            ...
                            vm.TypeBom.options.splice(0, 0, {
                                Id: "",
                                BOMCode: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1')
                            });
                            if (!!vm.currentItem.BOMCode) {
                                vm.TypeBom.value = vm.TypeBom.options.find(t => t.BOMCode == vm.currentItem.BOMCode);
                            }
                        } else {
                            vm.TypeBom.value = { BOMCode: commonService.$t('Siemens.SimaticIT.ProductionApp.OwnProduct.editJS.Tips_1'), Id: "" };
                        }
                    });
