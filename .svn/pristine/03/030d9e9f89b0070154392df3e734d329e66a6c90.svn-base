...
namespace ALP.Application.Service.AuthorizeManage
{
    ...
    public class AuthorizeService : RepositoryFactory, IAuthorizeService
    {
        ...
        /// <returns></returns>
        public IEnumerable<ModuleEntity> GetModuleList(string userId, string fullName = "")
        {
            ...
            strSql.Append(@"SELECT  *
                            ...
                            AND EnabledMark = 1  AND DeleteMark = 0 AND CHARINDEX(FullName,@FullName)" + v + "0  Order By SortCode");

            DbParameter[] parameter =
            {
                DbParameters.CreateDbParameter("@UserId",userId),
                DbParameters.CreateDbParameter("@FullName",fullName)
            };
        ...
        /// <returns></returns>
        public IEnumerable<ModuleEntity> GetIsMenuModuleList(string userId, string fullName)
        {
            StringBuilder strSql = new StringBuilder();
            string v = ">";
            if (string.IsNullOrEmpty(fullName))
            ...
            strSql.Append(@"SELECT  *
                            FROM    Base_Module
                            WHERE  [IsMenu]=1 And ModuleId IN (
                                    SELECT  ItemId
                                    FROM    Base_Authorize
                                    WHERE   ItemType = 1
                                            AND ( ObjectId IN (
        ...
        public IEnumerable<ModuleEntity> GetModuleList(string userId)
        {
            ...
            strSql.Append(@"SELECT  ModuleId,ParentId,EnCode,FullName,UrlAddress,IsMenu,EnabledMark
                            FROM    Base_Module
                            WHERE   ModuleId IN (
                                    SELECT  ItemId
                                    FROM    Base_Authorize
        ...
        public DataTable GetModuleNames()
        {
            ...
            try
            {
                string sql = "select FullName,UrlAddress from Base_Module where FullName is not null and UrlAddress is not null";
