(function () {
    ...
    function AddScreenController(dataService, $state, $stateParams, common, $filter, $scope, commonService, auth, notificationService, busyIndicatorService, $modal, $rootScope) {
        ...
        function activate() {
            ...
            sidePanelManager.setTitle(commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_1'));
        ...
        function initdictionary() {
            ...
            self.typeWarehouse = {
                value: { ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_2'), ResourceCode: "" },
                options: [{ ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_2'), ResourceCode: "" }]
            };
            self.Location = {
                value: { ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_2'), ResourceCode: "" },
                options: [{ ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_2'), ResourceCode: "" }]
            ...
            //是否合批
            self.IsMergeBatch = {
                value: { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_2'), ItemValue: "" },
                options: [{ ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_2'), ItemValue: "" },
                { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_3'), ItemValue: "1" },
                { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_4'), ItemValue: "0" }]
            };
            //启用批次管理，默认为否
            if (self.currentItem.IsUsed) {
                self.IsMergeBatch.value = { ItemName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_4'), ItemValue: "0" };
            }
            ...
            if (self.currentItem.SmallClass == "BC") {
                ...
                self.IsMergeBatch.value.ItemName = commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_3');
            ...
            commonService.getResourceExtendInfo({ LevelCode: "Warehouse" }).then(function (res) {
                if (res && res.data.success) {
                    ...
                    self.typeWarehouse.options.splice(0, 0, {
                        ResourceCode: "",
                        ResourceName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_2')
                    });
        ...
        function selectClick1() {
                ...
                resolve: {
                    data: function () {
                        return {
                            ...
                            columnDefs: [
                                {
                                    name: 'rowNum', displayName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_5'), minWidth: 80, width: 90, enableSorting: false, cellTemplate:
                                        '<div class="ui-grid-cell-contents">{{grid.renderContainers.body.visibleRowCache.indexOf(row) + 1}}</div>'
                                },
                                // {
                                //     field: 'ResourceCode',
                                //     displayName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_6'),
                                //     width: 200
                                // },
                                {
                                    field: 'ResourceName',
                                    displayName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_7'),
                                    width: 350
                                }
                            ],
                        };
            ...
            modalInstance.result.then(function (data) {
                if ((!data || data.length <= 0)) {
                    showWarning(commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_8'));
                } else {
        ...
        function selectClick2() {
            if (!self.currentItem.WhsCode) {
                backendService.genericError(commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_9'), commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_10'));
                return;
            }
            var modalInstance = commonService.openModel({
                templateUrl: 'CCS.CommonApp/modules/CommonUI/SelectMaterialModal.html',
                    ...
                    data: function () {
                        return {
                            ...
                            columnDefs: [
                                ...
                                {
                                    field: 'ResourceName',
                                    displayName: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_11'),
                                    width: 350
                                }
                            ],
                        };
        ...
        function save() {
            ...
            // self.currentItem.LocationCode = self.Location.value.ResourceCode;
            self.currentItem.BusinessId = self.currentItem.Id;
            if (!!self.BatchDate) {
            ...
            var postData = {
                ...
                PurchaseId: self.currentItem.PurchaseId,//采购订单Id
            ...
            busyIndicatorService.show({ message: commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_12') });
        ...
        function onSaveSuccess(data) {
            ...
            if (data.data.success) {
                ...
                //关闭侧边栏
                sidePanelManager.close();
                commonService.showInfo(commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_13'));
                //刷新局部
                $rootScope.$emit('to-parentDetail1', self.currentItem);
                $state.go('^', {}, { reload: false });
            } else {
                busyIndicatorService.hide();
                backendService.genericError(data.data.returnMsg, commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_10'));
            }
        }
        ...
        function onSaveError(error) {
            busyIndicatorService.hide();
            backendService.genericError('[' + error.status + '] - ' + error.statusText, commonService.$t('Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_10'));
        }
        ...
        var state = {
            ...
            views: {
                'property-area-container@': {
                    templateUrl: moduleFolder + '/ReceiptNotice-addDetail.html',
                    controller: AddScreenController,
                    controllerAs: 'vm'
                }
            },
            data: {
                title: 'Siemens.SimaticIT.MaterialApp.MMReceiptNotice.ReceiptNoticeaddDetailctrl.Tips_1'
            },
            params: {
                selectedItem: null,
            }
