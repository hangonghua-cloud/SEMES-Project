...
{
    public partial class Frm_PTeam : Form
    {
        ...
        private void Frm_PTeam_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    this.label3.Left = this.cmbFactory.Left - this.label3.Width - 2;//工厂
                //    this.label1.Left = this.txtPTeamCode.Left - this.label1.Width - 2;//小组编码
                //    this.label2.Left = this.txtPTeamName.Left - this.label2.Width - 2;//小组名称
                //},
                //() =>
                //{
                //    this.label3.Left = this.cmbFactory.Left - this.label3.Width - 2;//工厂
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //选择行数
                List<string> lstPTeamCode = new List<string>();
                for (int i = 0; i < dgvm.Rows.Count; i++)
                {
                    if ((bool)dgvm.Rows[i].Cells[0].EditedFormattedValue == true)
                    {
                        lstPTeamCode.Add(dgvm.Rows[i].Cells["PTeamCode"].Value.ToString());
                    }
                }
                if (lstPTeamCode.Count == 0)
                {
                    MessageBox.Show(Lib.Common.Language.GetText("Frm_PTeam.MessageTips_1", "请选择要打印的数据行！"));
                    return;
                }
                #endregion

                //var pTeamList = _baseDataBll.GetPTeamEntityList(lstPTeamCode).OrderBy(t => t.PTeamCode).ToList();
                //foreach (var item in pTeamList)
                //{
                //    Print(item); 
                //};

                string codes = string.Join(",", lstPTeamCode);
                Dictionary<string, object> dic = new Dictionary<string, object>();
                dic.Add("Code", codes);
                string filename = Application.StartupPath + @"/Template/生产小组.frx";
        ...
        private void Bind()
        {
            ...
            {
                MessageBox.Show(Lib.Common.Language.GetText("Frm_PTeam.MessageTips_2", "请选择工厂！"));
                return;
            }

            #region 参数赋值
            PTeamDto query = new PTeamDto();
            query.FactoryCode = cmbFactory.SelectedValue.ToString();
            query.PTeamCode = txtPTeamCode.Text.Trim();
            query.PTeamName = txtPTeamName.Text.Trim();
            #endregion

            query.CurrentPage = pagerControl1.CurrentPage;
            query.PageSize = pagerControl1.PageSize;
            query.Sidx = "PTeamCode";
            int record = 0;
            //var data = _baseDataBll.GetPTeamListWithPage(query, out record);
            var data = _baseDataBll.GetPTeamDataTableWithPage(query, out record);
        ...
        /// <returns></returns>
        private void Print(TeamPersonEntity item)
        {
            ...
            postData = postData.Substring(0, postData.Length - 1) + ",\"Image:Photo1\": {\"Type\": \"QRCode\",\"Value\": \"" + entity.PTeamCode + "\",\"Width\": \"250\",\"Height\": \"250\"}}";
            PubFunction.Post(postData, Template.PTeam.ToString());
