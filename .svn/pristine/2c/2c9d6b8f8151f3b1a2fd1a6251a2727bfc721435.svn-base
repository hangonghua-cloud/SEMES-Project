...
{ 
    ...
    /// 1.创建日期: 2021-10-09
    /// 2.创建作者: liyongguo
    /// 3.功能描述: QC_OQCQualityCheckItemService 业务服务类
    /// 4.任务编号: OQC检验记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class QC_OQCQualityCheckItem_Service : RepositoryFactory<QC_OQCQualityCheckItemEntity>, QC_OQCQualityCheckItemItemIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<QC_OQCQualityCheckItemEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[QC_OQCQualityCheckItem] where 1=1  ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT Q.[Id],
                               ...
                               Q.[Creator],
                               Q.[CreateTime],
                               V.ItemName TestDepartmentName
                        FROM [dbo].[QC_OQCQualityCheckItem] Q
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                if (!queryParam["OQCQualityCheckId"].IsEmpty())
                {
                    sql.Append($" AND Q.OQCQualityCheckId = N'{queryParam["OQCQualityCheckId"]}'");
                }
                //检测项目编码 是否为空进行查询
                ...
                //判别 是否为空进行查询
                if (!queryParam["Distinguish"].IsEmpty())
                {
                    //sql.Append($" AND Distinguish = N'{queryParam["Distinguish"]}'");
                    sql.Append($" AND Distinguish like N'%{queryParam["Distinguish"]}%'");
                }
        ...
        /// <param name="List<QC_OQCQualityCheckItemEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[QC_OQCQualityCheckItem] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            QC_OQCQualityCheckItemEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[QC_OQCQualityCheckItem] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回QC_OQCQualityCheckItemEntity</returns>
        ...
        /// <returns>返回QC_OQCQualityCheckItemEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回QC_OQCQualityCheckItemEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<QC_OQCQualityCheckItemEntity> bomService = new RepositoryFactory<QC_OQCQualityCheckItemEntity>();
        
        //    QC_OQCQualityCheckItemEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    QC_OQCQualityCheckItemDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.QC_OQCQualityCheckItem_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<QC_OQCQualityCheckItemEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<QC_OQCQualityCheckItemEntity> QC_OQCQualityCheckItemEntity_list =  db2.FindList<QC_OQCQualityCheckItemEntity>(sql.ToString());
                return QC_OQCQualityCheckItemEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable QC_OQCQualityCheckItemEntity_DataTable = db2.FindTable(sql.ToString());
                return QC_OQCQualityCheckItemEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [OQCQualityCheckId] as '检验记录Id'
                      ,[TestItemCoading] as '检测项目编码'
                      ,[TestItemName] as '检测项目名称'
                      ,[TestItemStandard] as '合格指标'
                      ,[UpperLimit] as '合格上限'
                      ,[LowerLimit] as '合格下限'
                      ,[DataType] as '数据类型'
