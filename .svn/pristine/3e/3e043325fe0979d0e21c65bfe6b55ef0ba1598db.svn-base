(function () {
    ...
    function AddScreenController(dataService, $state, $stateParams, common, $filter, $scope, i18nService, FileUploader,
        ...
        function activate() {
            init();
            registerEvents();
            sidePanelManager.setTitle(commonService.$t('Siemens.SimaticIT.PlanApp.Plan.importJS.Tips_6'));
        ...
        function save() {
            ...
            console.log("jpf1234" + JSON.stringify(post));
            busyIndicatorService.show({ message: commonService.$t('Siemens.SimaticIT.PlanApp.Plan.importJS.Tips_5') });
            var url = commonService.getMesApiAddress('plan') + "/PL_WorkOrder/SavePL_VCWorkOrder";
    ...
    function AddScreenStateConfig($stateProvider) {
        ...
        var state = {
            name: screenStateName + '.importVCWrd',
            url: '/importVCWrd',
            views: {
                'property-area-container@': {
                    templateUrl: moduleFolder + '/ProductionOrder-importVCWrd.html',
                    controller: AddScreenController,
                    controllerAs: 'vm'
                }
            },
            data: {
                title: 'Siemens.SimaticIT.PlanApp.Plan.importJS.Tips_6'
            }
        };
