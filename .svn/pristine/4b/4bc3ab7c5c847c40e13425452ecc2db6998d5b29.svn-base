...
namespace ALP.Application.Service.ProduceManage
{
    ...
    /// 1.创建日期: 2021-08-12
    /// 2.创建作者: admin
    /// 3.功能描述: PM_StartUpRecordService 业务服务类
    /// 4.任务编号: 开工记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_StartUpRecord_Service : RepositoryFactory<PM_StartUpRecordEntity>, PM_StartUpRecord_IService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<PM_StartUpRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,[IsEnabled]
                  FROM [dbo].[PM_StartUpRecord] where IsDeleted = 0 ");
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_StartUpRecordEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        /// <param name="List<PM_StartUpRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_StartUpRecordEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_StartUpRecord] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    //StringBuilder sql = new StringBuilder();
                    //sql.Append($@"INSERT INTO [dbo].[PM_StartUpRecord] (
                    //                        [Id]
                    //                        ,[ProductOrder]
                    //                        ,[WorkOrder]
                    //                        ,[ExeWorkOrder]
                    ...
                    //            ,'{Save_obj.ExeWorkOrder}'
                    //            ,N'{Save_obj.ProcessCode}'
                    //            ,N'{Save_obj.ProcessName}'
                    //            ,N'{Save_obj.CardCode}'
                    //            ,N'{Save_obj.Creator}'
        ...
        {
            int result = 0;
            PM_StartUpRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_StartUpRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_StartUpRecordEntity</returns>
        public PM_StartUpRecordEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_StartUpRecordEntity</returns>
        public PM_StartUpRecordEntity GetEntityByQuery(string QueryField)
        {
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_StartUpRecordEntity> bomService = new RepositoryFactory<PM_StartUpRecordEntity>();

        //    PM_StartUpRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_StartUpRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_StartUpRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_StartUpRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_StartUpRecordEntity> PM_StartUpRecordEntity_list = db2.FindList<PM_StartUpRecordEntity>(sql.ToString());
                return PM_StartUpRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_StartUpRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_StartUpRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ProcessCode] as '开工工序编码'
                      ,[ProcessName] as '开工工序名称'
                      ,[CardCode] as '流转卡编码'
                      ,[Creator] as '创建人编码'
                      ,[CreateTime] as '创建时间'
                      ,[ModifyBy] as '修改人编码'
            ...
            try
            {
                ...
                string saveFileName = "开工记录_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("开工记录", dt, true);
                //保存
