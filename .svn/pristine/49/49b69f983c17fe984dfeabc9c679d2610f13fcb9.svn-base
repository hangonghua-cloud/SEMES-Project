...
namespace Lib
{
    ...
    public class HttpHelper
    {
        ...
        #endregion
        public static T PostApi<T>(string url, object obj, Dictionary<string, string> header = null)
        {
            if (header == null) header = new Dictionary<string, string>();
            var content = JsonHelper.ToString(obj);
            byte[] bytesToPost = Encoding.UTF8.GetBytes(content); //转换为bytes数据
            ...
            {
                req.Headers.Add(item.Key, HttpUtility.UrlEncode(item.Value));
            }
            ...
            return JsonHelper.ToObject<T>(responseResult);
