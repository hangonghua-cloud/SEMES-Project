...
namespace ALP.Application.Service.MaterialManage
{
    ...
    /// 1.创建日期: 2021-10-19
    /// 2.创建作者: admin
    /// 3.功能描述: MM_ProductReworkService 业务服务类
    /// 4.任务编号: 成品返工单明细
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_ProductRework_Service : RepositoryFactory<MM_ProductReworkEntity>
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<MM_ProductReworkEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                      ,TotalPieceQty,WorkOrder,ReworkProcess
                  FROM [dbo].[MM_ProductRework] where 1=1 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //成品返工单状态 是否为空进行查询
                if (!queryParam["ReWorkStatus"].IsEmpty())
                {
                    //sql.Append($" AND ReWorkStatus = N'{queryParam["ReWorkStatus"]}'");
                    sql.Append($" AND ReWorkStatus like N'%{queryParam["ReWorkStatus"]}%'");
                }
                //创建人 是否为空进行查询
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_ProductReworkEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
                      ...
                      ,[CreateTime]
                      ,[ModifyBy]
                      ,[ModifyTime],TotalPieceQty,WorkOrder,ReworkProcess
                  FROM [dbo].[MM_ProductRework] where 1=1 ");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT a.ReWorkOrder,            --返工单号
                               a.ProductOrder,           --订单号
                               a.ContainerNO,            --柜号
                               a.CustomerPO,             --PO号
                               a.CustomerPO,             --PO号
                               v1.ItemName ReworkStatus, --返工状态
                               a.TotalPieceQty,          --总返工片数
                               SUM(b.PieceQty) PieceQty  --已返工片数
                        FROM [dbo].[MM_ProductRework] a
                            LEFT JOIN dbo.MM_ProductReworkBG b
                                ON a.ReWorkOrder = b.ReworkOrder
                            LEFT JOIN dbo.V_DataDictionary v1
                                ON v1.EnCode = 'ReworkStatus'
                                   AND a.ReWorkOrder = v1.ItemValue
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    sql.Append($" AND CONVERT(VARCHAR(10),a.CreateTime,120) >='{queryParam["StartTime"]}' ");
                }
                //结束日期 是否为空进行查询
                if (!queryParam["EndTime"].IsEmpty())
                {
                    sql.Append($" AND CONVERT(VARCHAR(10),a.CreateTime,120) <='{queryParam["EndTime"]}' ");
                }
                sql.Append(@" GROUP BY a.ReWorkOrder,
                                     a.ProductOrder,
                                     a.ContainerNO,
                                     a.CustomerPO,
                                     v1.ItemName,
        ...
        public int SaveEntity(string keyValue, MM_ProductReworkEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<MM_ProductReworkEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<MM_ProductReworkEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_ProductRework] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[MM_ProductRework] (
                                            [Id]
                                            ,[ReWorkOrder]
                                            ,[ProductOrder]
                                            ,[ContainerNO]
        ...
        {
            int result = 0;
            MM_ProductReworkEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_ProductRework] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_ProductReworkEntity</returns>
        public MM_ProductReworkEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回MM_ProductReworkEntity</returns>
        public MM_ProductReworkEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回MM_ProductReworkEntity 对象</returns>
        public MM_ProductReworkEntity Get_ExpressionEntity(Expression<Func<MM_ProductReworkEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_ProductReworkEntity 列表</returns>
        public IEnumerable<MM_ProductReworkEntity> Get_ExpressionList(Expression<Func<MM_ProductReworkEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_ProductReworkEntity> bomService = new RepositoryFactory<MM_ProductReworkEntity>();

        //    MM_ProductReworkEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_ProductReworkDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_ProductRework_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_ProductReworkEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_ProductReworkEntity> MM_ProductReworkEntity_list = db2.FindList<MM_ProductReworkEntity>(sql.ToString());
                return MM_ProductReworkEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_ProductReworkEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_ProductReworkEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [ReWorkOrder] as '成品返工单号'
                      ,[ProductOrder] as '订单号'
                      ,[ContainerNO] as '柜号'
                      ,[CustomerPO] as '客户PO号'
                      ,[MaterialCode] as '客户型号'
                      ,[DutyProcess] as '返工责任工序'
                      ,[ReWorkStatus] as '成品返工单状态'
                      ,[Creator] as '创建人'
                      ,[CreateTime] as '创建时间'
                      ,[ModifyBy] as '最后修改人'
            ...
            try
            {
                ...
                string saveFileName = "成品返工单明细_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("成品返工单明细", dt, true);
                //保存
