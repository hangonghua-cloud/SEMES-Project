...
{ 
    ...
    /// 1.创建日期: 2021-08-23
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PM_PrinterWorkOrderService 业务服务类
    /// 4.任务编号: 印刷计划工单
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_PrinterWorkOrder_Service : RepositoryFactory<PM_PrinterWorkOrderEntity>, PM_PrinterWorkOrderIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        /// 创建日期: 2021-08-23 16:05:49
        /// 任务编号: 印刷计划工单
        /// </summary>
        /// <param name="pagination">分页</param>
        /// <param name="queryJson">查询参数</param>
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PrinterWorkOrderEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PM_PrinterWorkOrder] where 1=1  ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //花色号 是否为空进行查询
                if (!queryParam["DesignColour"].IsEmpty())
                {
                    //sql.Append($" AND DesignColour = N'{queryParam["DesignColour"]}'");
                    sql.Append($" AND DesignColour like N'%{queryParam["DesignColour"]}%'");
                }
                //生产状态 是否为空进行查询
                ...
                //生产米数 是否为空进行查询
                if (!queryParam["MeterNum"].IsEmpty())
                {
                    //sql.Append($" AND MeterNum = N'{queryParam["MeterNum"]}'");
                    sql.Append($" AND MeterNum like N'%{queryParam["MeterNum"]}%'");
                }
                //生产卷数 是否为空进行查询
                if (!queryParam["ReelNum"].IsEmpty())
                {
                    //sql.Append($" AND ReelNum = N'{queryParam["ReelNum"]}'");
                    sql.Append($" AND ReelNum like N'%{queryParam["ReelNum"]}%'");
                }
                //生产托数 是否为空进行查询
                if (!queryParam["Pallet"].IsEmpty())
                {
                    //sql.Append($" AND Pallet = N'{queryParam["Pallet"]}'");
                    sql.Append($" AND Pallet like N'%{queryParam["Pallet"]}%'");
                }
                //工艺路线 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@" SELECT PW.[Id],
                                   ...
                                   PW.CreatorName,
                                   PW.[CreateTime],
                                   M.ProcessName ProcessRoutName,
                                   M2.YSReelNum,
                                   M2.YSMeterNum,
                                   M2.YSWeightNum,
                                   M2.DFReelNum,
                                   M2.DFMeterNum
                            FROM [FHMESDB].[dbo].[PM_PrinterWorkOrder] PW
                                LEFT JOIN dbo.BS_Process M
                                    ON PW.ProcessRoute = M.ProcessCode
                                LEFT JOIN
                                (
                                    SELECT WorkOrder,
                                           SUM(   CASE ProcessCode
                                                      WHEN 'FHYS' THEN
                                                          ReelNum
                                                      ELSE
                                                          0
                                    ...
                                    FROM dbo.PM_PrinterWorkOrderBG
                                    GROUP BY WorkOrder
                                ) M2
                                    ON M2.WorkOrder = PW.WorkOrder
                            WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PrinterWorkOrderEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, PM_PrinterWorkOrderEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PM_PrinterWorkOrderEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_PrinterWorkOrderEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_PrinterWorkOrder] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            PM_PrinterWorkOrderEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_PrinterWorkOrder] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_PrinterWorkOrderEntity</returns>
        public PM_PrinterWorkOrderEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_PrinterWorkOrderEntity</returns>
        public PM_PrinterWorkOrderEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_PrinterWorkOrderEntity 对象</returns>
        public PM_PrinterWorkOrderEntity Get_ExpressionEntity(Expression<Func<PM_PrinterWorkOrderEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_PrinterWorkOrderEntity 列表</returns>
        public IEnumerable<PM_PrinterWorkOrderEntity> Get_ExpressionList(Expression<Func<PM_PrinterWorkOrderEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_PrinterWorkOrderEntity> bomService = new RepositoryFactory<PM_PrinterWorkOrderEntity>();
        
        //    PM_PrinterWorkOrderEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_PrinterWorkOrderDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_PrinterWorkOrder_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PrinterWorkOrderEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_PrinterWorkOrderEntity> PM_PrinterWorkOrderEntity_list =  db2.FindList<PM_PrinterWorkOrderEntity>(sql.ToString());
                return PM_PrinterWorkOrderEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_PrinterWorkOrderEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_PrinterWorkOrderEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[DesignColour] as '花色号'
                      ,[WorkOrderType] as '生产状态'
                      ,[MeterNum] as '生产米数'
                      ,[ReelNum] as '生产卷数'
                      ,[Pallet] as '生产托数'
                      ,[ProcessRoute] as '工艺路线'
                      ,[BatchNo] as '生产批次'
                      ,[Remark] as '备注'
                      ,[Creator] as '制单人'
                      ,[CreateTime] as '制单时间'
        ...
        public int RemoveForm(Expression<Func<PM_PrinterWorkOrderEntity, bool>> condition)
        {
        ...
        #region table导入数据库
        public string WorkOrderImport(List<PM_PrinterWorkOrderEntity> listPrd)
        {
            var msg = "";
            ...
            try
            {
                ...
                db.ExecuteProcedure("P_PrintWorkOrderImport", parameters);
        ...
        /// <returns></returns>
        private DataTable WorkOrderToDataTable(List<PM_PrinterWorkOrderEntity> list)
        {
            ...
            dt.Columns.Add("PrinterOrder", System.Type.GetType("System.String"));
            dt.Columns.Add("PlanOrder", System.Type.GetType("System.String"));
            dt.Columns.Add("OrderDate", System.Type.GetType("System.String"));
            ...
            dt.Columns.Add("Cylinder", System.Type.GetType("System.String"));
            dt.Columns.Add("DesignColour", System.Type.GetType("System.String"));
            dt.Columns.Add("MeterNum", System.Type.GetType("System.Decimal"));
            dt.Columns.Add("ReelNum", System.Type.GetType("System.Decimal"));

            dt.Columns.Add("Reel", System.Type.GetType("System.String"));
            dt.Columns.Add("Pallet", System.Type.GetType("System.String"));
            dt.Columns.Add("ProcessRoute", System.Type.GetType("System.String"));
            dt.Columns.Add("BatchNo", System.Type.GetType("System.String"));
            dt.Columns.Add("Packaging", System.Type.GetType("System.String"));
            ...
            foreach (PM_PrinterWorkOrderEntity item in list)
            {
                dt.Rows.Add(item.FactoryCode, item.PrinterOrder, item.PlanOrder, item.OrderDate, item.DeliveryDate
                    , item.MaterialCode, item.Cylinder, item.DesignColour, item.MeterNum, item.ReelNum,
                    item.Reel, item.Pallet, item.ProcessRoute, item.BatchNo,item.Packaging,item.Remark,
