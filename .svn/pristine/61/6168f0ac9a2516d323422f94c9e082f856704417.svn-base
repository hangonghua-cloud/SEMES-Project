...
{ 
    ...
    /// 1.创建日期: 2021-08-05
    /// 2.创建作者: admin
    /// 3.功能描述: PM_TeamPerson_ItemsService 业务服务类
    /// 4.任务编号: 派工执行工单
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_TeamPerson_Items_Service : RepositoryFactory<PM_TeamPerson_ItemsEntity>, PM_TeamPerson_Items_IService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<PM_TeamPerson_ItemsEntity> GetPageList(Pagination pagination, string queryJson)
        {
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT a.Id,a.TeamId,
                               a.PTeamCode,
                               a.PostCode,
                               a.PostName,
                               a.UserCode,
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    sql.Append($" AND a.TeamId = N'{queryParam["TeamId"]}'");
                }
                //生产小组编码 是否为空进行查询
                if (!queryParam["PTeamCode"].IsEmpty())
                {
        ...
        /// 任务编号: 生产小组人员
        /// </summary>
        /// <param name="pTeamCode">查询条件</param>
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_TeamPerson_ItemsEntity> GetList(string pTeamCode, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append($@"SELECT a.Id,
                        ...
                        FROM dbo.PM_TeamPerson_Items a
                            LEFT JOIN dbo.BS_People c
                                ON a.UserCode = c.Code
                        WHERE 1 = 1 ");
            if (!string.IsNullOrEmpty(pTeamCode))
            {
                sql.Append($@" AND a.PTeamCode='{pTeamCode}' ");
            }
            msg = "";
            try
            {
        ...
        /// <param name="pTeamCode">查询条件</param>
        /// <returns>返回分页列表</returns>
        public DataTable GetDataTable(string pTeamCode, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT a.Id,
                               a.PTeamCode,
        ...
        public int SaveEntity(string keyValue, PM_TeamPerson_ItemsEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        public int InsertList(List<PM_TeamPerson_ItemsEntity> lstEntity, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                n = this.BaseRepository().Insert(lstEntity);
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        /// <param name="List<PM_TeamPerson_ItemsEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_TeamPerson_ItemsEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_TeamPerson_Items] set ");
                            string keyValue = "";
                            //循环实体
        ...
        public int DeleteByPID(string pid,out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                var strSql = $@"DELETE FROM dbo.PM_TeamPerson_Items WHERE PID='{pid}'";
        ...
        {
            int result = 0;
            PM_TeamPerson_ItemsEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int RemoveForm(Expression<Func<PM_TeamPerson_ItemsEntity, bool>> condition)
        {
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_TeamPerson_Items] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_TeamPerson_ItemsEntity</returns>
        public PM_TeamPerson_ItemsEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_TeamPerson_ItemsEntity</returns>
        public PM_TeamPerson_ItemsEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_TeamPerson_ItemsEntity 对象</returns>
        public PM_TeamPerson_ItemsEntity Get_ExpressionEntity(Expression<Func<PM_TeamPerson_ItemsEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_TeamPerson_ItemsEntity 列表</returns>
        public IEnumerable<PM_TeamPerson_ItemsEntity> Get_ExpressionList(Expression<Func<PM_TeamPerson_ItemsEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_TeamPerson_ItemsEntity> bomService = new RepositoryFactory<PM_TeamPerson_ItemsEntity>();
        
        //    PM_TeamPerson_ItemsEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_TeamPerson_ItemsDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_TeamPerson_Items_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_TeamPerson_ItemsEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_TeamPerson_ItemsEntity> PM_TeamPerson_ItemsEntity_list =  db2.FindList<PM_TeamPerson_ItemsEntity>(sql.ToString());
                return PM_TeamPerson_ItemsEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_TeamPerson_ItemsEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_TeamPerson_ItemsEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                  ...
                  FROM [dbo].[PM_TeamPerson_Items] where IsDeleted = 0 ");
            msg = "成功!";
