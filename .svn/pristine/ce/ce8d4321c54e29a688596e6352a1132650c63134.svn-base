...
{ 
    ...
    /// 1.创建日期: 2021-08-21
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PM_PackingRecordService 业务服务类
    /// 4.任务编号: 包装记录表
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_PackingRecord_Service : RepositoryFactory<PM_PackingRecordEntity>, PM_PackingRecordIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<PM_PackingRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PM_PackingRecord] where 1=1 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //包装状态 是否为空进行查询
                if (!queryParam["PackingStatus"].IsEmpty())
                {
                    //sql.Append($" AND PackingStatus = N'{queryParam["PackingStatus"]}'");
                    sql.Append($" AND PackingStatus like N'%{queryParam["PackingStatus"]}%'");
                }
                //创建人 是否为空进行查询
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT PMP.Id,PMP.FactoryCode,PMP.FactoryName,
                               PMP.ProductOrder,
                               PMP.WorkOrder,
                               PMP.ContainerNO,
                               PMP.CustomerPO,
	                           ...
	                           CEILING(PLW.DeliveryPieces/PLW.DeliveryPallet) PiecePerPallet,
                               ...
                               PMA.MMXH,
                               PMA.MMCJ,
                               [dbo].[Fun_GetTransferCardSumQty](PMP.WorkOrder) WorkQty,
                               dbo.Fun_GetPrintMarkNum(PMP.WorkOrder) PrintNum
                        FROM dbo.PM_PackingBGTransferCard PMP
                            INNER JOIN dbo.PL_WorkOrder PLW
                                ON PLW.WorkOrder = PMP.WorkOrder
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND ProductOrder = N'{queryParam["ProductOrder"]}'");
                    sql.Append($" AND PLW.FactoryCode = N'{queryParam["FactoryCode"]}'");
                }
                //订单号 是否为空进行查询
                if (!queryParam["ProductOrder"].IsEmpty())
                {
                    //sql.Append($" AND ProductOrder = N'{queryParam["ProductOrder"]}'");
                    sql.Append($" AND PMP.ProductOrder like N'%{queryParam["ProductOrder"]}%'");
                }
                //生产工单号 是否为空进行查询
                if (!queryParam["WorkOrder"].IsEmpty())
                {
                    //sql.Append($" AND WorkOrder = N'{queryParam["WorkOrder"]}'");
                    sql.Append($" AND PMP.WorkOrder like N'%{queryParam["WorkOrder"]}%'");
                }
                if (!queryParam["PaperBox"].IsEmpty())
                {
                    //sql.Append($" AND WorkOrder = N'{queryParam["WorkOrder"]}'");
                    sql.Append($" AND PMP.PaperBox like N'%{queryParam["PaperBox"]}%'");
                }
                //柜号 是否为空进行查询
                if (!queryParam["ContainerNO"].IsEmpty())
                {
                    //sql.Append($" AND ContainerNO = N'{queryParam["ContainerNO"]}'");
                    sql.Append($" AND PMP.ContainerNO = N'{queryParam["ContainerNO"]}'");
                }
                //PO号 是否为空进行查询
                if (!queryParam["CustomerPO"].IsEmpty())
                {
                    //sql.Append($" AND CustomerPO = N'{queryParam["CustomerPO"]}'");
                    sql.Append($" AND PMP.CustomerPO like N'%{queryParam["CustomerPO"]}%'");
                }
                //客户型号 是否为空进行查询
                if (!queryParam["MaterialCode"].IsEmpty())
                {
                    //sql.Append($" AND MaterialCode = N'{queryParam["MaterialCode"]}'");
                    sql.Append($" AND PMP.MaterialCode like N'%{queryParam["MaterialCode"]}%'");
                }
                ...
                {
                    //sql.Append($" AND PackingStatus = N'{queryParam["PackingStatus"]}'");
                    sql.Append($" AND PMP.PackingStatus = N'{queryParam["PackingStatus"]}'");
                }
                //创建人 是否为空进行查询
                if (!queryParam["Creator"].IsEmpty())
                {
                    //sql.Append($" AND Creator = N'{queryParam["Creator"]}'");
                    sql.Append($" AND PMP.Creator = N'{queryParam["Creator"]}'");
                }
                //创建时间 是否为空进行查询
                if (!queryParam["StartTime"].IsEmpty())
                {
                    //sql.Append($" AND CreateTime = N'{queryParam["CreateTime"]}'");
                    sql.Append($" AND PMP.CreateTime >= N'{queryParam["StartTime"]}'");
                }
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PackingRecordEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, PM_PackingRecordEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PM_PackingRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_PackingRecordEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_PackingRecord] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PM_PackingRecord] (
                                            [Id]
                                            ,[ProductOrder]
                                            ,[WorkOrder]
                                            ,[ContainerNO]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,N'{Save_obj.ContainerNO}'
                                ,N'{Save_obj.CustomerPO}'
                                ,N'{Save_obj.MaterialCode}'
                                ,N'{Save_obj.PackingStatus}'
                                ,N'{Save_obj.Creator}'
        ...
        {
            int result = 0;
            PM_PackingRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_PackingRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_PackingRecordEntity</returns>
        public PM_PackingRecordEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_PackingRecordEntity</returns>
        public PM_PackingRecordEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_PackingRecordEntity 对象</returns>
        public PM_PackingRecordEntity Get_ExpressionEntity(Expression<Func<PM_PackingRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_PackingRecordEntity 列表</returns>
        public IEnumerable<PM_PackingRecordEntity> Get_ExpressionList(Expression<Func<PM_PackingRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_PackingRecordEntity> bomService = new RepositoryFactory<PM_PackingRecordEntity>();
        
        //    PM_PackingRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_PackingRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_PackingRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PackingRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_PackingRecordEntity> PM_PackingRecordEntity_list =  db2.FindList<PM_PackingRecordEntity>(sql.ToString());
                return PM_PackingRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_PackingRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_PackingRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "包装记录表_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("包装记录表", dt, true);
                //保存
        ...
        public int RemoveForm(Expression<Func<PM_PackingRecordEntity, bool>> condition)
        {
