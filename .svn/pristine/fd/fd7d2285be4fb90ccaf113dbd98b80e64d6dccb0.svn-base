...
{ 
    ...
    /// 1.创建日期: 2021-10-09
    /// 2.创建作者: liyongguo
    /// 3.功能描述: QC_OQCCheckConfigItemService 业务服务类
    /// 4.任务编号: OQC检验配置
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class QC_OQCCheckConfigItem_Service : RepositoryFactory<QC_OQCCheckConfigItemEntity>, QC_OQCCheckConfigItemIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<QC_OQCCheckConfigItemEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[QC_OQCCheckConfigItem] where 1=1  ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT [Id],
                               ...
                               [ModifyBy],
                               [ModifyTime]
                        FROM [dbo].[QC_OQCCheckConfigItem]
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<QC_OQCCheckConfigItemEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, QC_OQCCheckConfigItemEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<QC_OQCCheckConfigItemEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<QC_OQCCheckConfigItemEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[QC_OQCCheckConfigItem] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            QC_OQCCheckConfigItemEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[QC_OQCCheckConfigItem] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回QC_OQCCheckConfigItemEntity</returns>
        public QC_OQCCheckConfigItemEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回QC_OQCCheckConfigItemEntity</returns>
        public QC_OQCCheckConfigItemEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回QC_OQCCheckConfigItemEntity 对象</returns>
        public QC_OQCCheckConfigItemEntity Get_ExpressionEntity(Expression<Func<QC_OQCCheckConfigItemEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回QC_OQCCheckConfigItemEntity 列表</returns>
        public IEnumerable<QC_OQCCheckConfigItemEntity> Get_ExpressionList(Expression<Func<QC_OQCCheckConfigItemEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<QC_OQCCheckConfigItemEntity> bomService = new RepositoryFactory<QC_OQCCheckConfigItemEntity>();
        
        //    QC_OQCCheckConfigItemEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    QC_OQCCheckConfigItemDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.QC_OQCCheckConfigItem_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<QC_OQCCheckConfigItemEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<QC_OQCCheckConfigItemEntity> QC_OQCCheckConfigItemEntity_list =  db2.FindList<QC_OQCCheckConfigItemEntity>(sql.ToString());
                return QC_OQCCheckConfigItemEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable QC_OQCCheckConfigItemEntity_DataTable = db2.FindTable(sql.ToString());
                return QC_OQCCheckConfigItemEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "OQC检验配置_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("OQC检验配置", dt, true);
                //保存
        ...
        public int RemoveForm(Expression<Func<QC_OQCCheckConfigItemEntity, bool>> condition)
        {
