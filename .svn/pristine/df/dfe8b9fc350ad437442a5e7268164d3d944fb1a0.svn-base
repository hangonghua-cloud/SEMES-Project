...
{ 
    ...
    /// 1.创建日期: 2021-10-09
    /// 2.创建作者: liyongguo
    /// 3.功能描述: QC_OQCQualityCheckService 业务服务类
    /// 4.任务编号: OQC检验记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class QC_OQCQualityCheck_Service : RepositoryFactory<QC_OQCQualityCheckEntity>, QC_OQCQualityCheckIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<QC_OQCQualityCheckEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[QC_OQCQualityCheck] where 1=1  ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //检测Id 是否为空进行查询
                if (!queryParam["OQCCheckConfigId"].IsEmpty())
                {
                    //sql.Append($" AND OQCCheckConfigId = N'{queryParam["OQCCheckConfigId"]}'");
                    sql.Append($" AND OQCCheckConfigId like N'%{queryParam["OQCCheckConfigId"]}%'");
                }
                //检验单号 是否为空进行查询
                ...
                //抽检托数 是否为空进行查询
                if (!queryParam["CheckTrayNum"].IsEmpty())
                {
                    //sql.Append($" AND CheckTrayNum = N'{queryParam["CheckTrayNum"]}'");
                    sql.Append($" AND CheckTrayNum like N'%{queryParam["CheckTrayNum"]}%'");
                }
                //抽样片数 是否为空进行查询
                if (!queryParam["CheckSliceNum"].IsEmpty())
                {
                    //sql.Append($" AND CheckSliceNum = N'{queryParam["CheckSliceNum"]}'");
                    sql.Append($" AND CheckSliceNum like N'%{queryParam["CheckSliceNum"]}%'");
                }
                //检验状态 是否为空进行查询
                if (!queryParam["CheckStatus"].IsEmpty())
                {
                    //sql.Append($" AND CheckStatus = N'{queryParam["CheckStatus"]}'");
                    sql.Append($" AND CheckStatus like N'%{queryParam["CheckStatus"]}%'");
                }
                //检验结论 是否为空进行查询
                ...
                //检验单完成日期 是否为空进行查询
                if (!queryParam["FinshTime"].IsEmpty())
                {
                    //sql.Append($" AND FinshTime = N'{queryParam["FinshTime"]}'");
                    sql.Append($" AND FinshTime like N'%{queryParam["FinshTime"]}%'");
                }
                //完成人 是否为空进行查询
                if (!queryParam["FinshBy"].IsEmpty())
                {
                    //sql.Append($" AND FinshBy = N'{queryParam["FinshBy"]}'");
                    sql.Append($" AND FinshBy like N'%{queryParam["FinshBy"]}%'");
                }
                //最后修改人 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT Q.[Id],
                               ...
                               P.ContainerNO,
                               P.Spec
                        FROM [dbo].[QC_OQCQualityCheck] Q
                            LEFT JOIN dbo.PM_PackingPrintMark P
                                ON P.PackTransferCode = Q.PackTransferCode
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    sql.Append($" AND P.MaterialCode like N'%{queryParam["MaterialCode"]}%'");
                }
                if (!queryParam["ContainerNO"].IsEmpty())
                {
                    sql.Append($" AND P.ContainerNO like N'%{queryParam["ContainerNO"]}%'");
                }
                if (!queryParam["Spec"].IsEmpty())
                { 
                    sql.Append($" AND P.Spec like N'%{queryParam["Spec"]}%'");
                }

                if (!queryParam["CheckStatus"].IsEmpty())
                {
                    sql.Append($" AND Q.CheckStatus = N'{queryParam["CheckStatus"]}'");
                }
                //检验结论 是否为空进行查询
                if (!queryParam["CheckResult"].IsEmpty())
                {
                    sql.Append($" AND Q.CheckResult = N'{queryParam["CheckResult"]}'");
                }
                //检验单创建日期 是否为空进行查询
                if (!queryParam["StartDate"].IsEmpty())
                {
                    sql.Append($" AND Q.CreateTime >= N'{queryParam["StartDate"]}'");  
                }
                if (!queryParam["EndDate"].IsEmpty())
                {
                    sql.Append($" AND Q.CreateTime <= N'{queryParam["EndDate"]} 23:59:59'");
                }
            }
            try
            {
                if (pagination == null)
                {
                    sql.Append(@" ORDER BY Q.CreateTime");
        ...
        /// <param name="List<QC_OQCQualityCheckEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[QC_OQCQualityCheck] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            QC_OQCQualityCheckEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[QC_OQCQualityCheck] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回QC_OQCQualityCheckEntity</returns>
        ...
        /// <returns>返回QC_OQCQualityCheckEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回QC_OQCQualityCheckEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<QC_OQCQualityCheckEntity> bomService = new RepositoryFactory<QC_OQCQualityCheckEntity>();
        
        //    QC_OQCQualityCheckEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    QC_OQCQualityCheckDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.QC_OQCQualityCheck_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<QC_OQCQualityCheckEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<QC_OQCQualityCheckEntity> QC_OQCQualityCheckEntity_list =  db2.FindList<QC_OQCQualityCheckEntity>(sql.ToString());
                return QC_OQCQualityCheckEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable QC_OQCQualityCheckEntity_DataTable = db2.FindTable(sql.ToString());
                return QC_OQCQualityCheckEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [OQCCheckConfigId] as '检测Id'
                      ,[InspectNo] as '检验单号'
                      ,[ProductOrder] as '订单号'
                      ,[CheckTrayNum] as '抽检托数'
                      ,[CheckSliceNum] as '抽样片数'
                      ,[CheckStatus] as '检验状态'
                      ,[CheckResult] as '检验结论'
                      ,[CreateTime] as '检验单创建日期'
                      ,[Creator] as '创建人'
                      ,[FinshTime] as '检验单完成日期'
                      ,[FinshBy] as '完成人'
                      ,[ModifyBy] as '最后修改人'
            ...
            try
            {
                ...
                string saveFileName = "OQC检验记录_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("OQC检验记录", dt, true);
                //保存
        ...
        {
            var sql = $@" UPDATE dbo.QC_OQCQualityCheck SET 
                                InspectNo='{inspectNo}', 
                                ModifyBy='{userCode}',
                                ModifyTime=GETDATE() 
                                WHERE Id IN ({idStr})";
