...
{ 
    ...
    /// 1.创建日期: 2021-07-24
    /// 2.创建作者: liyongguo
    /// 3.功能描述: Base_SupplierBindMaterialGroupService 业务服务类
    /// 4.任务编号: 供应商管理
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class Base_SupplierBindMaterialGroup_Service : RepositoryFactory<Base_SupplierBindMaterialGroupEntity>, Base_SupplierBindMaterialGroupIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<Base_SupplierBindMaterialGroupEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[Base_SupplierBindMaterialGroup] where 1=1  ");
        ...
        public DataTable GetSupplierBindMaterialSelect(string materialCode)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append($@" SELECT
                            SU.SupplierCode, SU.SupplierName, SU.Category, SU.Abbr, MG.MaterialCode
                          FROM dbo.Base_SupplierManage SU
                          INNER JOIN dbo.Base_SupplierBindMaterialGroup SB ON SU.SupplierCode= SB.SupplierCode
                          INNER JOIN dbo.Base_MaterialGroupBindMaterial MG ON SB.GroupCode= MG.GroupCode
				  WHERE MG.MaterialCode ='{materialCode}' ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                     S.Id,S.GroupCode,S.SupplierCode,S.Creator,S.CreateTime,M.GroupName
                  FROM [dbo].[Base_SupplierBindMaterialGroup] S
				  INNER JOIN dbo.Base_MaterialGroup M ON S.GroupCode=M.GroupCode
				  WHERE 1=1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND Id = N'{queryParam["Id"]}'");
                    sql.Append($" AND S.Id like N'%{queryParam["Id"]}%'");
                }
                //供应商编码 是否为空进行查询
                if (!queryParam["SupplierCode"].IsEmpty())
                {
                    //sql.Append($" AND SupplierCode = N'{queryParam["SupplierCode"]}'");
                    sql.Append($" AND S.SupplierCode like N'%{queryParam["SupplierCode"]}%'");
                }
                //物料组编码 是否为空进行查询
                if (!queryParam["GroupCode"].IsEmpty())
                {
                    //sql.Append($" AND GroupCode = N'{queryParam["GroupCode"]}'");
                    sql.Append($" AND S.GroupCode like N'%{queryParam["GroupCode"]}%'");
                }
                //创建人 是否为空进行查询
                if (!queryParam["Creator"].IsEmpty())
                {
                    //sql.Append($" AND Creator = N'{queryParam["Creator"]}'");
                    sql.Append($" AND S.Creator like N'%{queryParam["Creator"]}%'");
                }
                //创建时间 是否为空进行查询
                if (!queryParam["CreateTime"].IsEmpty())
                {
                    //sql.Append($" AND CreateTime = N'{queryParam["CreateTime"]}'");
                    sql.Append($" AND S.CreateTime like N'%{queryParam["CreateTime"]}%'");
                }
                //最后修改人 是否为空进行查询
                if (!queryParam["ModifyBy"].IsEmpty())
                {
                    //sql.Append($" AND ModifyBy = N'{queryParam["ModifyBy"]}'");
                    sql.Append($" AND S.ModifyBy like N'%{queryParam["ModifyBy"]}%'");
                }
                //最后修改时间 是否为空进行查询
                if (!queryParam["ModifyTime"].IsEmpty())
                {
                    //sql.Append($" AND ModifyTime = N'{queryParam["ModifyTime"]}'");
                    sql.Append($" AND S.ModifyTime like N'%{queryParam["ModifyTime"]}%'");
                }
                //queryName(选择弹窗关键名称) 是否为空进行查询
                if (!queryParam["queryName"].IsEmpty())
                {
        ...
        /// <param name="List<Base_SupplierBindMaterialGroupEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<Base_SupplierBindMaterialGroupEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[Base_SupplierBindMaterialGroup] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[Base_SupplierBindMaterialGroup] (
                                            [Id]
                                            ,[SupplierCode]
                                            ,[GroupCode]
                                            ,[Creator]
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[Base_SupplierBindMaterialGroup] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回Base_SupplierBindMaterialGroupEntity</returns>
        ...
        /// <returns>返回Base_SupplierBindMaterialGroupEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回Base_SupplierBindMaterialGroupEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<Base_SupplierBindMaterialGroupEntity> bomService = new RepositoryFactory<Base_SupplierBindMaterialGroupEntity>();
        
        //    Base_SupplierBindMaterialGroupEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    Base_SupplierBindMaterialGroupDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.Base_SupplierBindMaterialGroup_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<Base_SupplierBindMaterialGroupEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<Base_SupplierBindMaterialGroupEntity> Base_SupplierBindMaterialGroupEntity_list =  db2.FindList<Base_SupplierBindMaterialGroupEntity>(sql.ToString());
                return Base_SupplierBindMaterialGroupEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string materialCode, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
	                        SG.SupplierCode,S.SupplierName,S.Abbr,SG.GroupCode,MG.MaterialCode 
                        FROM dbo.Base_SupplierBindMaterialGroup SG 
                        INNER JOIN  dbo.Base_MaterialGroupBindMaterial MG ON MG.GroupCode = SG.GroupCode
                        LEFT JOIN dbo.Base_SupplierManage S ON S.SupplierCode=SG.SupplierCode
                        WHERE 1=1  ");
            if (!string.IsNullOrEmpty(materialCode))
            {
                sql.Append($@" AND MG.MaterialCode = '{materialCode}'");
            }
            msg = "";
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable Base_SupplierBindMaterialGroupEntity_DataTable = db2.FindTable(sql.ToString());
                return Base_SupplierBindMaterialGroupEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
