    ...
    public class Database : IDatabase
    {
        ...
        public int ExecuteBySql(string strSql)
        {
            ...
            else
            {
                ...
                var affectedRows = dbcontext.Database.ExecuteSqlCommand(strSql);
                return dbTransaction == null ? this.Commit() : affectedRows;
            }
        }
        public int ExecuteBySql(string strSql, params DbParameter[] dbParameter)
        {
            ...
            else
            {
                ...
                //return dbTransaction == null ? this.Commit() : 0;
                var affectedRows = dbcontext.Database.ExecuteSqlCommand(strSql, dbParameter);
                return dbTransaction == null ? this.Commit() : affectedRows;
            }
        }
        ...
        public int Update<T>(T entity) where T : class
        {
            ...
            foreach (string item in props.Keys)
            {
                ...
                var prop = typeof(T).GetProperty(item, System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
              
                bool isNotMapped = false;
                foreach (var notMapped in prop.CustomAttributes)
                {
        ...
        public dynamic QueryFirst(string strSql)
        {
            using (var dbConnection = dbcontext.Database.Connection)
            {
                ...
                //total = Convert.ToInt32(new DbHelper(dbConnection).ExecuteScalar(CommandType.Text, "Select Count(1) From (" + strSql + ") As t", dbParameter));
                var IDataReader = new DbHelper(dbConnection).ExecuteReader(CommandType.Text, builder.ToString());
                DataTable resultTable = ConvertExtension.IDataReaderToDataTable(IDataReader);
                var result = DataTableExtensions.ToDynamicList(resultTable);
                //var result = ConvertExtension.DataFillDynamicList(IDataReader);
