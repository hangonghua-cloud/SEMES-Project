...
{ 
    ...
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_ProcessBad_Service : RepositoryFactory<PM_ProcessBadEntity>, PM_ProcessBad_IService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<PM_ProcessBadEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,[IsEnabled]
                  FROM [dbo].[PM_ProcessBad] where IsEnabled=1 ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND IsEnabled = N'{queryParam["IsEnabled"]}'");
                    sql.Append($" AND a.IsEnabled like N'%{queryParam["IsEnabled"]}%'");
                }
        ...
        /// <param name="List<PM_ProcessBadEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_ProcessBadEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_ProcessBad] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PM_ProcessBad] (
                                            [Id]
                                            ,[FactoryCode]
                                            ,[ProcessCode]
                                            ,[Creator]
        ...
        public int DeleteList(List<PM_ProcessBadEntity> lstEntity,out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                //删除
                n = this.BaseRepository().Delete(lstEntity);
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        {
            int result = 0;
            PM_ProcessBadEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_ProcessBad] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_ProcessBadEntity</returns>
        ...
        /// <returns>返回PM_ProcessBadEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_ProcessBadEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_ProcessBadEntity> bomService = new RepositoryFactory<PM_ProcessBadEntity>();
        
        //    PM_ProcessBadEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_ProcessBadDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_ProcessBad_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        public IEnumerable<PM_ProcessBadEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_ProcessBadEntity> PM_ProcessBadEntity_list =  db2.FindList<PM_ProcessBadEntity>(sql.ToString());
                return PM_ProcessBadEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_ProcessBadEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_ProcessBadEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
