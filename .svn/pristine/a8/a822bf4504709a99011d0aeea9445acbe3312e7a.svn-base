...
{ 
    ...
    /// 1.创建日期: 2021-08-26
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PM_OwnProductBGService 业务服务类
    /// 4.任务编号: 自制半成品报工
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_OwnProductBG_Service : RepositoryFactory<PM_OwnProductBGEntity>, PM_OwnProductBGIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<PM_OwnProductBGEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PM_OwnProductBG] where 1=1  ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //报工ID 是否为空进行查询
                if (!queryParam["OwnProductId"].IsEmpty())
                {
                    //sql.Append($" AND OwnProductId = N'{queryParam["OwnProductId"]}'");
                    sql.Append($" AND OwnProductId like N'%{queryParam["OwnProductId"]}%'");
                }
                //工单号 是否为空进行查询
                ...
                //流转卡编码 是否为空进行查询
                if (!queryParam["TransferCode"].IsEmpty())
                {
                    //sql.Append($" AND TransferCode = N'{queryParam["TransferCode"]}'");
                    sql.Append($" AND TransferCode like N'%{queryParam["TransferCode"]}%'");
                }
                //流转卡名称 是否为空进行查询
                if (!queryParam["TransferName"].IsEmpty())
                {
                    //sql.Append($" AND TransferName = N'{queryParam["TransferName"]}'");
                    sql.Append($" AND TransferName like N'%{queryParam["TransferName"]}%'");
                }
                //报工工序 是否为空进行查询
                if (!queryParam["BGProcess"].IsEmpty())
                {
                    //sql.Append($" AND BGProcess = N'{queryParam["BGProcess"]}'");
                    sql.Append($" AND BGProcess like N'%{queryParam["BGProcess"]}%'");
                }
                //生产机台 是否为空进行查询
                if (!queryParam["BGMachine"].IsEmpty())
                {
                    //sql.Append($" AND BGMachine = N'{queryParam["BGMachine"]}'");
                    sql.Append($" AND BGMachine like N'%{queryParam["BGMachine"]}%'");
                }
                //报工数量 是否为空进行查询
                if (!queryParam["BGQty"].IsEmpty())
                {
                    //sql.Append($" AND BGQty = N'{queryParam["BGQty"]}'");
                    sql.Append($" AND BGQty like N'%{queryParam["BGQty"]}%'");
                }
                //报工班次 是否为空进行查询
                if (!queryParam["BGShift"].IsEmpty())
                {
                    //sql.Append($" AND BGShift = N'{queryParam["BGShift"]}'");
                    sql.Append($" AND BGShift like N'%{queryParam["BGShift"]}%'");
                }
                //报工小组 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT PB.[Id],
                               ...
                               PO.ProcessRoute,
                               PO.BOMCode,
                               PP.ProcessName ProcessRouteName
                        FROM [dbo].[PM_OwnProductBG] PB
                            INNER JOIN dbo.PM_OwnProductOrder PO
                                ON PB.OwnProductId = PO.Id
                            LEFT JOIN dbo.BS_ModelWithResource M1
                                ON M1.ResourceCode = PB.BGProcess
                            LEFT JOIN dbo.BS_ModelWithResource M2
                                ON M2.ResourceCode = PB.BGMachine
                            LEFT JOIN dbo.BS_ModelWithResource M3
                                ON M3.ResourceCode = PO.ProcessCode
                            LEFT JOIN dbo.V_DataDictionary V
                                ON V.EnCode = 'MaterialSmall'
                                   AND V.ItemValue = PO.SmallClass
                            LEFT JOIN dbo.PL_Process PP
                                ON PP.WorkOrder = PO.WorkOrder
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    sql.Append($" AND PO.OrderType LIKE '%{queryParam["OrderType"]}%' ");
                }
                //报工ID 是否为空进行查询
                if (!queryParam["OwnProductId"].IsEmpty())
                {
                ...
                {
                    //sql.Append($" AND WorkOrder = N'{queryParam["WorkOrder"]}'");
                    sql.Append($" AND PB.WorkOrder like N'%{queryParam["WorkOrder"]}%'");
                }
                //物料类别 是否为空进行查询
                if (!queryParam["MaterialClass"].IsEmpty())
                {
                    //sql.Append($" AND MaterialClass = N'{queryParam["MaterialClass"]}'");
                    sql.Append($" AND PB.MaterialClass like N'%{queryParam["MaterialClass"]}%'");
                }
                //报工数量 是否为空进行查询
                if (!queryParam["MaterialCode"].IsEmpty())
                {
                ...
                {
                    //sql.Append($" AND Spec = N'{queryParam["Spec"]}'");
                    sql.Append($" AND PB.Spec like N'%{queryParam["Spec"]}%'");
                }
                //物料名称 是否为空进行查询
                if (!queryParam["MaterialName"].IsEmpty())
                {
                    //sql.Append($" AND MaterialName = N'{queryParam["MaterialName"]}'");
                    sql.Append($" AND PB.MaterialName like N'%{queryParam["MaterialName"]}%'");
                }
                //流转卡编码 是否为空进行查询
                if (!queryParam["TransferCode"].IsEmpty())
                {
                ...
                {
                    sql.Append($" AND PB.CreateTime >='{queryParam["StartTime"]}' ");
                }
                //下单日期 是否为空进行查询
                if (!queryParam["EndTime"].IsEmpty())
                {
                    sql.Append($" AND PB.CreateTime <='{queryParam["EndTime"]}' ");
                }
            }
            try
            {
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_OwnProductBG] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            PM_OwnProductBGEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_OwnProductBG] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_OwnProductBGEntity</returns>
        ...
        /// <returns>返回PM_OwnProductBGEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_OwnProductBGEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_OwnProductBGEntity> bomService = new RepositoryFactory<PM_OwnProductBGEntity>();
        
        //    PM_OwnProductBGEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_OwnProductBGDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_OwnProductBG_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<PM_OwnProductBGEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_OwnProductBGEntity> PM_OwnProductBGEntity_list =  db2.FindList<PM_OwnProductBGEntity>(sql.ToString());
                return PM_OwnProductBGEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_OwnProductBGEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_OwnProductBGEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [OwnProductId] as '报工ID'
                      ,[WorkOrder] as '工单号'
                      ,[MaterialClass] as '物料类别'
                      ,[MaterialCode] as '报工数量'
                      ,[Spec] as '规格型号'
                      ,[MaterialName] as '物料名称'
                      ,[TransferCode] as '流转卡编码'
                      ,[TransferName] as '流转卡名称'
                      ,[BGProcess] as '报工工序'
                      ,[BGMachine] as '生产机台'
                      ,[BGQty] as '报工数量'
                      ,[BGShift] as '报工班次'
            ...
            try
            {
                ...
                string saveFileName = "自制半成品报工_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("自制半成品报工", dt, true);
                //保存
