...
namespace ALP.Application.Service.MaterialManage
{
    ...
    /// 1.创建日期: 2021-10-14
    /// 2.创建作者: admin
    /// 3.功能描述: MM_SemiProductMoveRecordService 业务服务类
    /// 4.任务编号: 半成品移库记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_SemiProductMoveRecord_Service : RepositoryFactory<MM_SemiProductMoveRecordEntity>, MM_SemiProductMoveRecord_IService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        /// 创建日期: 2021-10-14 14:43:47
        /// 任务编号: 半成品移库记录
        /// </summary>
        ...
        public IEnumerable<MM_SemiProductMoveRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[MM_SemiProductMoveRecord] where 1=1 ");
        ...
        /// <param name="List<MM_SemiProductMoveRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_SemiProductMoveRecord] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            MM_SemiProductMoveRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_SemiProductMoveRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_SemiProductMoveRecordEntity</returns>
        ...
        /// <returns>返回MM_SemiProductMoveRecordEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_SemiProductMoveRecordEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_SemiProductMoveRecordEntity> bomService = new RepositoryFactory<MM_SemiProductMoveRecordEntity>();

        //    MM_SemiProductMoveRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_SemiProductMoveRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_SemiProductMoveRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<MM_SemiProductMoveRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_SemiProductMoveRecordEntity> MM_SemiProductMoveRecordEntity_list = db2.FindList<MM_SemiProductMoveRecordEntity>(sql.ToString());
                return MM_SemiProductMoveRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_SemiProductMoveRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_SemiProductMoveRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "半成品移库记录_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("半成品移库记录", dt, true);
                //保存
