...
{ 
    ...
    /// 1.创建日期: 2021-09-11
    /// 2.创建作者: liyongguo
    /// 3.功能描述: QC_MaterialInventoryCheckItemService 业务服务类
    /// 4.任务编号: 原材料库存检验
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class QC_MaterialInventoryCheckItem_Service : RepositoryFactory<QC_MaterialInventoryCheckItemEntity>, QC_MaterialInventoryCheckItemIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<QC_MaterialInventoryCheckItemEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[QC_MaterialInventoryCheckItem] where 1=1  ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT [Id],
                               ...
                               [ModifyBy],
                               [ModifyTime]
                        FROM [dbo].[QC_MaterialInventoryCheckItem]
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                // 是否为空进行查询
                if (!queryParam["MaterialInventoryId"].IsEmpty())
                {
                    //sql.Append($" AND MaterialInventoryId = N'{queryParam["MaterialInventoryId"]}'");
                    sql.Append($" AND MaterialInventoryId like N'%{queryParam["MaterialInventoryId"]}%'");
                }
                // 是否为空进行查询
                ...
                // 是否为空进行查询
                if (!queryParam["BadNum"].IsEmpty())
                {
                    //sql.Append($" AND BadNum = N'{queryParam["BadNum"]}'");
                    sql.Append($" AND BadNum like N'%{queryParam["BadNum"]}%'");
                }
                //创建人 是否为空进行查询
                if (!queryParam["Creator"].IsEmpty())
                {
        ...
        /// <param name="List<QC_MaterialInventoryCheckItemEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[QC_MaterialInventoryCheckItem] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            QC_MaterialInventoryCheckItemEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[QC_MaterialInventoryCheckItem] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回QC_MaterialInventoryCheckItemEntity</returns>
        ...
        /// <returns>返回QC_MaterialInventoryCheckItemEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回QC_MaterialInventoryCheckItemEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<QC_MaterialInventoryCheckItemEntity> bomService = new RepositoryFactory<QC_MaterialInventoryCheckItemEntity>();
        
        //    QC_MaterialInventoryCheckItemEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    QC_MaterialInventoryCheckItemDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.QC_MaterialInventoryCheckItem_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<QC_MaterialInventoryCheckItemEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<QC_MaterialInventoryCheckItemEntity> QC_MaterialInventoryCheckItemEntity_list =  db2.FindList<QC_MaterialInventoryCheckItemEntity>(sql.ToString());
                return QC_MaterialInventoryCheckItemEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable QC_MaterialInventoryCheckItemEntity_DataTable = db2.FindTable(sql.ToString());
                return QC_MaterialInventoryCheckItemEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
