...
namespace MESClient.BaseData
{
    public partial class Frm_People : Form
    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < dgvm.Rows.Count; i++)
                {
                    ...
                    {
                        lstUserCode.Add(dgvm.Rows[i].Cells["UserCode"].Value.ToString());
                    }
                }
                if (lstUserCode.Count == 0)
                {
                    MessageBox.Show(Lib.Common.Language.GetText("Frm_People.MessageTips_1", "请选择要打印的数据行!"));
                    return;
                }
                #endregion

                //var peopleList = _baseDataBll.GetPeopleEntityList(lstUserCode).OrderBy(t => t.UserCode).ToList();
                //foreach (var item in peopleList)
                //{
                //    Print(item);
                //};

                string codes = string.Join(",", lstUserCode);
                Dictionary<string, object> dic = new Dictionary<string, object>();
                dic.Add("Code", codes);
                string filename = Application.StartupPath + @"/Template/人员.frx";
        ...
        private void Bind()
        {
            ...
            {
                MessageBox.Show(Lib.Common.Language.GetText("Frm_People.MessageTips_2", "请选择工厂！"));
                return;
            }

            #region 参数赋值
            PeopleDto query = new PeopleDto();
            query.FactoryCode = cmbFactory.SelectedValue.ToString();
            query.UserCode = txtUserCode.Text.Trim();
            query.UserName = txtUserName.Text.Trim();
            query.Dept = txtDept.Text.Trim();
            #endregion
            ...
            query.Sidx = "UserCode";
            int record = 0;
            //var data = _baseDataBll.GetPeopleListWithPage(query, out record);
            var data = _baseDataBll.GetPeopleDataTableWithPage(query, out record);
        ...
        /// <returns></returns>
        private void Print(PeopleEntity item)
        {
            ...
            postData = postData.Substring(0, postData.Length - 1) + ",\"Image:Photo1\": {\"Type\": \"QRCode\",\"Value\": \"" + entity.UserCode + "\",\"Width\": \"80\",\"Height\": \"80\"}}";
            PubFunction.Post(postData, Template.People.ToString());
