...
{ 
    ...
    /// 1.创建日期: 2021-08-05
    /// 2.创建作者: admin
    /// 3.功能描述: PM_TeamPersonService 业务服务类
    /// 4.任务编号: 生产小组人员管理
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_TeamPerson_Service : RepositoryFactory<PM_TeamPersonEntity>, PM_TeamPerson_IService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<PM_TeamPersonEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PM_TeamPerson] where 1=1 ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND PTeamName = N'{queryParam["PTeamName"]}'");
                    sql.Append($" AND a.PTeamName like N'%{queryParam["PTeamName"]}%'");
                }
                //创建人 是否为空进行查询
                if (!queryParam["Creator"].IsEmpty())
                {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_TeamPersonEntity> GetList(string ProcessCode, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
            ...
            {
                sql.Append($@" and ProcessCode = N'{ProcessCode}' ");
            }
            msg = "";
            try
            {
        ...
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_TeamPersonEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_TeamPerson] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO dbo.PM_TeamPerson
                                    (
                                        Id,
                                        FactoryCode,
                                        FactoryName,
        ...
        {
            int result = 0;
            PM_TeamPersonEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_TeamPerson] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_TeamPersonEntity> bomService = new RepositoryFactory<PM_TeamPersonEntity>();
        
        //    PM_TeamPersonEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_TeamPersonDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_TeamPerson_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<PM_TeamPersonEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_TeamPersonEntity> PM_TeamPersonEntity_list =  db2.FindList<PM_TeamPersonEntity>(sql.ToString());
                return PM_TeamPersonEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_TeamPersonEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_TeamPersonEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "生产小组人员管理_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("生产小组人员管理", dt, true);
                //保存
