...
namespace ALP.Application.Service.MaterialManage
{
    ...
    /// 1.创建日期: 2021-08-25
    /// 2.创建作者: liyongguo
    /// 3.功能描述: MM_RawMaterialInService 业务服务类
    /// 4.任务编号: 原材料入库单
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_RawMaterialIn_Service : RepositoryFactory<MM_RawMaterialInEntity>, MM_RawMaterialInIService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<MM_RawMaterialInEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[MM_RawMaterialIn] where 1=1  ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND BusinessId = N'{queryParam["BusinessId"]}'");
                    sql.Append($" AND BusinessId like N'%{queryParam["BusinessId"]}%'");
                }
                //计划订单 是否为空进行查询
                ...
                {
                    //sql.Append($" AND InType = N'{queryParam["InType"]}'");
                    sql.Append($" AND InType like N'%{queryParam["InType"]}%'");
                }
                //入库仓库 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT MM.Id,
							   ...
							   MM.IsPosted,
							   CASE MM.IsPosted WHEN '1' THEN '已同步' ELSE '' END SAPSync,
							   MM.PostedMsg,
							   MM.PostDate,
							   MM.PostedTime,
							   MM.PostedUser,
							   MM.SAP_MBLNR,
							   MM.SAP_MJAHR
                        FROM [dbo].[MM_RawMaterialIn] MM
                            LEFT JOIN dbo.BS_ModelWithResource MW
                                ON MW.ResourceCode = MM.WhsCode
                            LEFT JOIN dbo.Base_SupplierManage s
                                ON MM.SupplierCode = s.SupplierCode
                            LEFT JOIN dbo.BS_ModelWithResource c
                                ON c.ModelLeve = 'StorageLocation'
                                   AND MM.LocationCode = c.ResourceCode
                        WHERE ISNULL(MM.IsDeleted,0)=0 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                JObject queryParam = queryJson.ToJObject();
        ...
        /// <param name="List<MM_RawMaterialInEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_RawMaterialIn] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            MM_RawMaterialInEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_RawMaterialIn] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_RawMaterialInEntity</returns>
        ...
        /// <returns>返回MM_RawMaterialInEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_RawMaterialInEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_RawMaterialInEntity> bomService = new RepositoryFactory<MM_RawMaterialInEntity>();

        //    MM_RawMaterialInEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_RawMaterialInDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_RawMaterialIn_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<MM_RawMaterialInEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_RawMaterialInEntity> MM_RawMaterialInEntity_list = db2.FindList<MM_RawMaterialInEntity>(sql.ToString());
                return MM_RawMaterialInEntity_list;
            }
            catch (Exception ex)
            {
                 throw ex;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_RawMaterialInEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_RawMaterialInEntity_DataTable;
            }
            catch (Exception ex)
            {
                 throw ex;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [BusinessId] as '收料通知单Id'
                      ,[ProductOrder] as '计划订单'
                      ,[DocNum] as '原材料入库单号'
            ...
            try
            {
                ...
                string saveFileName = "原材料入库单_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("原材料入库单", dt, true);
                //保存
