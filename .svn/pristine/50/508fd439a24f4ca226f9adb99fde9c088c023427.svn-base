...
namespace ALP.Application.Service.MaterialManage
{
    ...
    /// 1.创建日期: 2021-10-11
    /// 2.创建作者: admin
    /// 3.功能描述: MM_ProductInService 业务服务类
    /// 4.任务编号: 成品入库记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_ProductIn_Service : RepositoryFactory<MM_ProductInEntity>, MM_ProductIn_IService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<MM_ProductInEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[MM_ProductIn] where 1=1 ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND MarkCode = N'{queryParam["MarkCode"]}'");
                    sql.Append($" AND a.MarkCode like N'%{queryParam["MarkCode"]}%'");
                }
                //订单号 是否为空进行查询
                if (!queryParam["ProductOrder"].IsEmpty())
                {
        ...
        public IEnumerable<MM_ProductInEntity> GetList(Expression<Func<MM_ProductInEntity, bool>> condition)
        {
        ...
        /// <param name="List<MM_ProductInEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_ProductIn] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            MM_ProductInEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_ProductIn] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_ProductInEntity</returns>
        ...
        /// <returns>返回MM_ProductInEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_ProductInEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_ProductInEntity> bomService = new RepositoryFactory<MM_ProductInEntity>();

        //    MM_ProductInEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_ProductInDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_ProductIn_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<MM_ProductInEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_ProductInEntity> MM_ProductInEntity_list = db2.FindList<MM_ProductInEntity>(sql.ToString());
                return MM_ProductInEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_ProductInEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_ProductInEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
