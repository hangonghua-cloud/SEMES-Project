(function () {
    'use strict';
    angular.module('Siemens.SimaticIT.FactoryModelApp.ModelLevel')
        .constant('Siemens.SimaticIT.FactoryModelApp.ModelLevel.ModelLevel.constants', ScreenServiceConstants())
        .service('Siemens.SimaticIT.FactoryModelApp.ModelLevel.ModelLevel.service', ScreenService)
        .run(ScreenServiceRun);

    function ScreenServiceConstants() {
        return {
            data: {
                //appName: 'FactoryModelApp',
                appName: 'FactoryModelApp', // The name of App: in case of extended app, it is the name of AppBase
    ...
    ScreenService.$inject = ['$q', '$state', 'common.base', 'Siemens.SimaticIT.FactoryModelApp.ModelLevel.ModelLevel.constants', 'common.services.logger.service', 'commonService'];
    function ScreenService($q, $state, base, context, loggerService, commonService) {
        var self = this;
        var logger, backendService;

        activate();
        function activate() {
            logger = loggerService.getModuleLogger('Siemens.SimaticIT.FactoryModelApp.ModelLevel.ModelLevel.service');
        ...
        self.getListWithPaging = function (rows, page, sidx, sord, queryJson) {
            var url = commonService.getMesApiAddress("factory") + '/LevelManage/BsModelLevel/GetPageListJson';
            var data = {
                pagination: {
                    rows: rows,
                    page: page,
    ...
    ScreenServiceRun.$inject = ['Siemens.SimaticIT.FactoryModelApp.ModelLevel.ModelLevel.constants', 'common.base'];
