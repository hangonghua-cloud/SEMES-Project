		...
		<view class="u-calendar">
			...
			<view class="u-calendar__bottom">
				<view class="u-calendar__bottom__choose">
					...
					<text v-if="endDate">{{$t('common.To')}}{{endDate}}</text>
				</view>
				<view class="u-calendar__bottom__btn">
					<u-button :type="btnType" shape="circle" size="default"
						@click="btnFix(false)">{{$t('dateModal.confirm')}}</u-button>
				</view>
			</view>
		</view>
	</u-popup>
...
<script>
	 ...
	 * @property {String} active-bg-color 起始/结束日期按钮的背景色(默认#138087)
	 * @property {String Number} z-index 弹出时的z-index值(默认10075)
	 * @property {String} active-color 起始/结束日期按钮的字体颜色(默认#ffffff)
	 * @property {String} range-bg-color 起始/结束日期之间的区域的背景颜色(默认rgba(41,121,255,0.13))
	 * @property {String} range-color 选择范围内字体颜色(默认#138087)
	...
	export default {
		...
		props: {
			...
			startText: {
				type: String,
				default: 'dateModal.startText' //$t('dateModal.startText')
			},
			// mode=range时生效，结束日期自定义文案
			endText: {
				type: String,
				default: 'dateModal.endText' //$t('dateModal.endText')
			},
			//按钮样式类型
			btnType: {
				type: String,
		...
		methods: {
			...
			changeData() {
				...
				this.showTitle = `${this.year}${this.$t('dateModal.year')}${this.month}${this.$t('dateModal.month')}`;
			...
			getWeekText(date) {
				...
				return this.$t('dateModal.week') + this.weekDayZh[week];
			},
			btnFix(show) {
				if (!show) {
					this.close();
