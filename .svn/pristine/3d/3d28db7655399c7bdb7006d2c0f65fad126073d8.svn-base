...
{ 
    ...
    /// 1.创建日期: 2021-07-24
    /// 2.创建作者: liyongguo
    /// 3.功能描述: Base_SupplierManageService 业务服务类
    /// 4.任务编号: 供应商管理
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class Base_SupplierManage_Service : RepositoryFactory<Base_SupplierManageEntity>, Base_SupplierManageIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<Base_SupplierManageEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[Base_SupplierManage] where 1=1  ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND SupplierName = N'{queryParam["SupplierName"]}'");
                    sql.Append($" AND SupplierName like N'%{queryParam["SupplierName"]}%'");
                }
                //供应商类别 是否为空进行查询
                if (!queryParam["Category"].IsEmpty())
                {
                    //sql.Append($" AND Category = N'{queryParam["Category"]}'");
                    sql.Append($" AND Category like N'%{queryParam["Category"]}%'");
                }
                //简称 是否为空进行查询
                if (!queryParam["Abbr"].IsEmpty())
                {
                    //sql.Append($" AND Abbr = N'{queryParam["Abbr"]}'");
                    sql.Append($" AND Abbr like N'%{queryParam["Abbr"]}%'");
                }
                //创建人 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT a.Id,
                               ...
                               a.IsEnabled,
                               a.SupplierLevel,
                               v1.ItemName SupplierLevelName,
                               a.ParentSupplierCode,
                               b.SupplierName ParentSupplierName,
							   a.SupplierWarehouse,
							   c.ResourceName SupplierWhsName
                        FROM dbo.Base_SupplierManage a
                            LEFT JOIN dbo.V_DataDictionary v1
                                ON v1.EnCode = 'SupplierLevel'
                                   AND a.SupplierLevel = v1.ItemValue
                            LEFT JOIN dbo.Base_SupplierManage b
                                ON a.ParentSupplierCode = b.SupplierCode
						    LEFT JOIN dbo.BS_ModelWithResource c ON a.SupplierWarehouse=c.ResourceCode
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND SupplierName = N'{queryParam["SupplierName"]}'");
                    sql.Append($" AND a.SupplierName like N'%{queryParam["SupplierName"]}%'");
                }
                //供应商类别 是否为空进行查询
                if (!queryParam["Category"].IsEmpty())
                {
                    //sql.Append($" AND Category = N'{queryParam["Category"]}'");
                    sql.Append($" AND a.Category like N'%{queryParam["Category"]}%'");
                }
                //简称 是否为空进行查询
                if (!queryParam["Abbr"].IsEmpty())
                {
                    //sql.Append($" AND Abbr = N'{queryParam["Abbr"]}'");
                    sql.Append($" AND a.Abbr like N'%{queryParam["Abbr"]}%'");
                }
                //创建人 是否为空进行查询
                if (!queryParam["Creator"].IsEmpty())
                {
                ...
                {
                    sql.Append($" AND a.IsEnabled = N'{queryParam["IsEnabled"]}'");
                    //sql.Append($" AND 关键编码 like N'%{queryParam["queryCode"]}%'");
                }
                ...
                {
                    sql.Append($" AND (a.SupplierCode LIKE '%{queryParam["Name"]}%' OR a.SupplierName LIKE '%{queryParam["Name"]}%') ");
                }
                //供应商等级
                if (!queryParam["SupplierLevel"].IsEmpty())
                {
                    sql.Append($" AND a.SupplierLevel = N'{queryParam["SupplierLevel"]}'");
                }
                //父级供应商
                if (!queryParam["parentSupplierCodes"].IsEmpty())
                {
                    string parentSupplierCodes = queryParam["parentSupplierCodes"].ToString();
                    parentSupplierCodes =parentSupplierCodes.Replace(",", "','");
                    sql.Append($" AND a.ParentSupplierCode IN('{parentSupplierCodes}') ");
                }
                if (!queryParam["ParentSupplier"].IsEmpty())
                {
                    sql.Append($" AND (a.ParentSupplierCode like N'%{queryParam["ParentSupplier"]}%' OR b.SupplierName like N'%{queryParam["ParentSupplier"]}%' ) ");
                }
            }
            try
            {
        ...
        public IEnumerable<Base_SupplierManageEntity> GetList(string checkType, string Name)
        {
            ...
            {
                sql.Append($@" and (SupplierCode like N'%{checkType}%' OR  SupplierName like N'%{checkType}%')");
            }
            if (!string.IsNullOrEmpty(Name))
            {
                sql.Append($@" and (SupplierCode like N'%{Name}%' OR  SupplierName like N'%{Name}%')");
            }
            try
            {
                return this.BaseRepository().FindList(sql.ToString());
        ...
        /// <param name="List<Base_SupplierManageEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<Base_SupplierManageEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[Base_SupplierManage] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[Base_SupplierManage] (
                                            [Id]
                                            ,[SupplierCode]
                                            ,[SupplierName]
                                            ,[Category]
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[Base_SupplierManage] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回Base_SupplierManageEntity</returns>
        ...
        /// <returns>返回Base_SupplierManageEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回Base_SupplierManageEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<Base_SupplierManageEntity> bomService = new RepositoryFactory<Base_SupplierManageEntity>();
        
        //    Base_SupplierManageEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    Base_SupplierManageDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.Base_SupplierManage_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<Base_SupplierManageEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<Base_SupplierManageEntity> Base_SupplierManageEntity_list =  db2.FindList<Base_SupplierManageEntity>(sql.ToString());
                return Base_SupplierManageEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable Base_SupplierManageEntity_DataTable = db2.FindTable(sql.ToString());
                return Base_SupplierManageEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        #region SAP供应商主数据同步
        public void SaveSAPBase_Supplier(List<Base_SupplierManageEntity> entity_list)
        {
            //开启事务进行数据的插入
            ...
            try
            {
                ...
                foreach (var item in entity_list)
                {
                    ...
                    {
                        throw new Exception("供应商名称必须传！");
                    }
                    if (string.IsNullOrEmpty(item.Category))
                    {
                        throw new Exception("供应商类别必须传！");
                    }
                    if (string.IsNullOrEmpty(item.Abbr))
                    {
                        throw new Exception("供应商简称必须传！");
                    }
                    if (string.IsNullOrEmpty(item.SupplierLevel))
                    {
                        throw new Exception("供应商等级必须传！");
                    }
                    //if (string.IsNullOrEmpty(item.SupplierWarehouse))
                    //{
                    //    throw new Exception("供应商仓库必须传！");
                    //}
                    //判断供应商是否存在
                    var supEntity = Get_ExpressionEntity(t=>t.SupplierCode==item.SupplierCode);
                    if (!item.IsDeleted) { 
                    if (supEntity == null)
                    {
                        ...
                        {
                            //修改逻辑
                            supEntity.SupplierName = item.SupplierName;
                            supEntity.Category = item.Category;
                            supEntity.Abbr = item.Abbr;
                            supEntity.SupplierLevel = item.SupplierLevel;
                            supEntity.SupplierWarehouse = item.SupplierWarehouse;
                            supEntity.ModifyTime= DateTime.Now;
                            db.Update(supEntity);
                        }
                    }
                    else
                    {
                        ...
                        if (supEntity == null)
                        {
                            throw new Exception(item.SupplierCode + "物料编码不存在无法进行删除");
