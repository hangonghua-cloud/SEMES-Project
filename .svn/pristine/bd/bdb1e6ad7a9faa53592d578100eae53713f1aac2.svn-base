...
{ 
    ...
    /// 1.创建日期: 2022-11-23
    /// 2.创建作者: jpf
    /// 3.功能描述: BS_TraitDetailsAttrService 业务服务类
    /// 4.任务编号: 跨工厂调拨
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class BS_TraitDetailsAttr_Service : RepositoryFactory<BS_TraitDetailsAttrEntity>
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: jpf
        ...
        public IEnumerable<BS_TraitDetailsAttrEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[BS_TraitDetailsAttr] where 1=1 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                // 是否为空进行查询
                if (!queryParam["TraitDetailId"].IsEmpty())
                {
                    //sql.Append($" AND TraitDetailId = N'{queryParam["TraitDetailId"]}'");
                    sql.Append($" AND TraitDetailId like N'%{queryParam["TraitDetailId"]}%'");
                }
                // 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                     ...
                     , V.ItemName AttrTypeName
                      ...
                      ,bst.[ModifyBy]
                      ,bst.[ModifyTime]
                  FROM [dbo].[BS_TraitDetailsAttr] bst
                 LEFT JOIN dbo.V_DataDictionary V
                                ON V.EnCode = 'AttrType'
                                   AND V.ItemValue = bst.AttrType
where 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND Id = N'{queryParam["Id"]}'");
                    sql.Append($" AND bst.Id like N'%{queryParam["Id"]}%'");
                }
                // 是否为空进行查询
                if (!queryParam["TraitDetailId"].IsEmpty())
                {
                    //sql.Append($" AND TraitDetailId = N'{queryParam["TraitDetailId"]}'");
                    sql.Append($" AND bst.TraitDetailId like N'%{queryParam["TraitDetailId"]}%'");
                }
                // 是否为空进行查询
                if (!queryParam["AttrCode"].IsEmpty())
                {
                    //sql.Append($" AND AttrCode = N'{queryParam["AttrCode"]}'");
                    sql.Append($" AND bst.AttrCode like N'%{queryParam["AttrCode"]}%'");
                }
                // 是否为空进行查询
                if (!queryParam["AttrName"].IsEmpty())
                {
                    //sql.Append($" AND AttrName = N'{queryParam["AttrName"]}'");
                    sql.Append($" AND bst.AttrName like N'%{queryParam["AttrName"]}%'");
                }
                // 是否为空进行查询
                if (!queryParam["AttrType"].IsEmpty())
                {
                    //sql.Append($" AND AttrType = N'{queryParam["AttrType"]}'");
                    sql.Append($" AND bst.AttrType like N'%{queryParam["AttrType"]}%'");
                }
                // 是否为空进行查询
                if (!queryParam["AttrValue"].IsEmpty())
                {
                    //sql.Append($" AND AttrValue = N'{queryParam["AttrValue"]}'");
                    sql.Append($" AND bst.AttrValue like N'%{queryParam["AttrValue"]}%'");
                }
                //创建人 是否为空进行查询
                if (!queryParam["Creator"].IsEmpty())
                {
                    //sql.Append($" AND Creator = N'{queryParam["Creator"]}'");
                    sql.Append($" AND bst.Creator like N'%{queryParam["Creator"]}%'");
                }
                //创建时间 是否为空进行查询
                if (!queryParam["CreateTime"].IsEmpty())
                {
                    //sql.Append($" AND CreateTime = N'{queryParam["CreateTime"]}'");
                    sql.Append($" AND bst.CreateTime like N'%{queryParam["CreateTime"]}%'");
                }
                //最后修改人 是否为空进行查询
                if (!queryParam["ModifyBy"].IsEmpty())
                {
                    //sql.Append($" AND ModifyBy = N'{queryParam["ModifyBy"]}'");
                    sql.Append($" AND bst.ModifyBy like N'%{queryParam["ModifyBy"]}%'");
                }
                //最后修改时间 是否为空进行查询
                if (!queryParam["ModifyTime"].IsEmpty())
                {
                    //sql.Append($" AND ModifyTime = N'{queryParam["ModifyTime"]}'");
                    sql.Append($" AND bst.ModifyTime like N'%{queryParam["ModifyTime"]}%'");
                }
                //queryName(选择弹窗关键名称) 是否为空进行查询
                if (!queryParam["queryName"].IsEmpty())
                {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<BS_TraitDetailsAttrEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[BS_TraitDetailsAttr] where IsDeleted = 0 ");
        ...
        public int SaveEntity(string keyValue, BS_TraitDetailsAttrEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<BS_TraitDetailsAttrEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<BS_TraitDetailsAttrEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[BS_TraitDetailsAttr] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    //StringBuilder sql = new StringBuilder();
                    //sql.Append($@"INSERT INTO [dbo].[BS_TraitDetailsAttr] (
                    //                        [Id]
                    //                        ,[TraitDetailId]
                    //                        ,[AttrCode]
                    //                        ,[AttrName]
                    ...
                    //        sql.Append($@"(
                    //            N'{Save_obj.Id}'
                    //            ,N'{Save_obj.TraitDetailId}'
                    //            ,N'{Save_obj.AttrCode}'
                    //            ,N'{Save_obj.AttrName}'
                    //            ,N'{Save_obj.AttrType}'
                    //            ,N'{Save_obj.AttrValue}'
                    //            ,N'{Save_obj.Creator}'
        ...
        {
            int result = 0;
            BS_TraitDetailsAttrEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[BS_TraitDetailsAttr] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回BS_TraitDetailsAttrEntity</returns>
        public BS_TraitDetailsAttrEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回BS_TraitDetailsAttrEntity</returns>
        public BS_TraitDetailsAttrEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回BS_TraitDetailsAttrEntity 列表</returns>
        public IEnumerable<BS_TraitDetailsAttrEntity> Get_ExpressionList(Expression<Func<BS_TraitDetailsAttrEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<BS_TraitDetailsAttrEntity> bomService = new RepositoryFactory<BS_TraitDetailsAttrEntity>();
        
        //    BS_TraitDetailsAttrEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    BS_TraitDetailsAttrDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.BS_TraitDetailsAttr_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<BS_TraitDetailsAttrEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<BS_TraitDetailsAttrEntity> BS_TraitDetailsAttrEntity_list =  db2.FindList<BS_TraitDetailsAttrEntity>(sql.ToString());
                return BS_TraitDetailsAttrEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable BS_TraitDetailsAttrEntity_DataTable = db2.FindTable(sql.ToString());
                return BS_TraitDetailsAttrEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
