...
/* ShowModalBox */
MeScroll.prototype.ShowModalBox = function(self, boxTitle, boxType, RowSize) {
	if (!boxTitle || boxTitle == "" || boxTitle == null) {
		console.log("弹出消息不能为空");
		return;
	}
	var tit = "";
	if (RowSize > boxTitle.length) {
		tit = boxTitle;
	} else {
		for (var i = 0; i < boxTitle.length / RowSize; i++) {
			tit += boxTitle.substr(i * RowSize, RowSize) + "\r\n";
		}
	}
