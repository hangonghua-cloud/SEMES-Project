        ...
        .controller(ctrl, ["common.services.modelDriven.dataService", '$q', '$state', '$stateParams', '$translate', 'common.base', 'common.services.modelDriven.service',
            ...
            function (multiDataService, $q, $state, $stateParams, $translate, base, md, $scope, $parse, $rootScope, mdContextSrv, $filter, applInjector, $timeout, SwacUiModuleManager, breadcrumbService, warningService) {
                var self = this, sidePanelManager = base.services.sidePanel.service, backendService = base.services.runtime.backendService,
                    ...
                    actionId = stateId + '_' + actionName, outputParameters = {}, emptyKeepFieldValue = "", fillQueryResult = null,
                ...
                md.loadModule(stateId).then(function (manifest) {
                    ...
                    if (self.displayMode === 'inCanvas') {
                        // showing browser warning when user tries to refresh from an In-Canvas action
                        if ($rootScope.isInCanvasView) {
                    ...
                    function executeExtendedCommand(extension, inputParams, baseCmdOutputParams) {
                        ...
                        //Command Call
                        dataService[actionId][extension.id](inputParams, applName).then(function (outcome) {
                            defer.resolve();
                        });
                    ...
                    function save() {
                        ...
                        //Command Call
                        dataService[actionId](outputParameters, applName).then(function (outcome) {
                            if (outcome) {
                                var promises = [];
                        ...
                        if (action.behavior.query || checkForReadingFuncParam()) { // a navigation "id" ... convention-> edit mode
                            ...
                            dataService.getAll(filter, applName).then(function (data) {
                                if (data.value.length > 0) {
                                    ...
                                    //merge with context
                                    contextInput = { action: { item: fillQueryResult[0], items: fillQueryResult, extensions: {} } };
            ...
            controller: function () {
                this.Value = function (arg) {
                    ...
                    if (this.field && this.row) {
                        var evaluatedValue = $parse(this.field)(this.row.entity);
