...
namespace ALP.Data.Dapper
{
    ...
    public class SqlDatabase : IDatabase
    {
        ...
        public SqlDatabase()
        {
            ...
            string connString = DbConstSettings.BaseDbString;
            
            connectionString = connString;
        }
        #endregion

        #region init dbconnection gzq 20200421 添加，主要用于Dapper
        ...
        {
            DbConnection dbConnection = dbTransaction==null? Connection:dbTransaction.Connection;
        ...
        public int  ExecuteBySql<T>(string strSql, T entity)
        {
            if (dbTransaction == null)
            {
                using (var connection = Connection)
                {
                    return connection.Execute(strSql, entity);
                }
            }
            else
            {
        ...
        public void ExecuteProcedure(string procName, SqlParameter[] dbParameter)
        {
            using (var dbConnection = Connection)
            {
                new DbHelper(dbConnection).ExecuteProc(Connection, procName, dbParameter);
            }
        }
        /// <summary>
        /// 执行存储过程 20210220 LYB
        ...
        /// <param name="strSql"></param>
        /// <param name="dbParameter"></param>
        public DataTable  ExecuteProc_Table(string procName, SqlParameter[] dbParameter)
        {
            using (var dbConnection = Connection)
            {
                var dt = new DbHelper(dbConnection).ExecuteProc_Table(Connection, procName, dbParameter);
                return dt;
            }
        }
        ...
        public int Insert<T>(T entity) where T : class
        {
            ...
            return ExecuteBySql(DatabaseCommon.InsertSql<T>(entity).ToString(),entity);
        }
        /// <summary>
        /// 实体插入(dapper)不作实现
        /// </summary>
        ...
        public DataTable FindTableBySP(string strSql, DbParameter[] dbParameter)
        {
            using (var dbConnection = Connection)
            {
                var IDataReader = new DbHelper(dbConnection).ExecuteReader(CommandType.StoredProcedure, strSql, dbParameter);
                return ConvertExtension.IDataReaderToDataTable(IDataReader);
            }
        }
        ...
        public object FindObjectBySP(string strSql, DbParameter[] dbParameter)
        {
            using (var dbConnection = Connection)
            {
                return new DbHelper(dbConnection).ExecuteScalar(CommandType.StoredProcedure, strSql, dbParameter);
            }
        }
        public dynamic QueryFirst(string strSql)
        {
            using (var dbConnection = Connection)
            {
                var resultList = dbConnection.QueryFirst(strSql);
                return resultList;
            }
        }
        public List<dynamic> Query(string strSql)
        {
            using (var dbConnection = Connection)
            {
                var resultList = dbConnection.Query(strSql).ToList();
                return resultList;
            }
        }
        public List<dynamic> Query(string strSql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total)
        {
            using (var dbConnection = Connection)
            {
                StringBuilder sb = new StringBuilder();
                ...
                sb.Append("Select * From (Select ROW_NUMBER() Over (" + OrderBy + ")");
                sb.Append(" As rowNum, * From (" + strSql + ") As T ) As N Where rowNum > " + num + " And rowNum <= " + num1 + "");
                total = Convert.ToInt32(dbConnection.ExecuteScalar("Select Count(1) From (" + strSql + ") As t", dbParameter));
                var resultList = dbConnection.Query(strSql, dbParameter).ToList();
                return resultList;
            }
        }
        #endregion
