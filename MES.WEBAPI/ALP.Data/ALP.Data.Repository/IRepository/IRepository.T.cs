...
namespace ALP.Data.Repository
{
    ...
    public interface IRepository<T> where T : class, new()
    {
        ...
        DataTable FindTableBySP(string strSql, DbParameter[] dbParameter);
        dynamic QueryFirst(string strSql);
        List<dynamic> Query(string strSql);
        List<dynamic> Query(string strSql, DbParameter[] dbParameter, Pagination pagination);
        T GetFirstOrDefaut(Expression<Func<T, bool>> condition);
        int Count(Expression<Func<T, bool>> condition);
        IQueryable<T> GetListByCondition(Expression<Func<T, bool>> condition);
    }
}