    ...
    public class AccountController : ApiBaseController
    {
        ...
        public HttpResponseMessage GetAccountList(GetAccountListParms parms)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    result.returnMsg = ALP.Application.Service.Resources.Language.GetText("BaseManage.AccountController.Tips_1");//操作失败，请传递请求参数
                }
            }
            catch (Exception ex)
            {
                WriteLog(ALP.Application.Service.Resources.Language.GetText("BaseManage.AccountController.Tips_2",ex.ToString()));//Account/List接口异常：{ex.ToString()}
                result.success = false;
        ...
        public HttpResponseMessage SaveAccount(MESBaseAccountEntity parms)
        {
            ...
            try
            {
                if (parms != null)
                {
                    WriteLog(ALP.Application.Service.Resources.Language.GetText("BaseManage.AccountController.Tips_4",JsonConvert.SerializeObject(parms)));//Account/SaveAccount接口请求参数{JsonConvert.SerializeObject(parms)}
                    BaseAccountService service = new BaseAccountService();
            ...
            {
                WriteLog(ALP.Application.Service.Resources.Language.GetText("BaseManage.AccountController.Tips_5",ex.ToString()));//Account/SaveAccount接口异常：{ex.ToString()}
                result.success = false;
                result.returnMsg = ALP.Application.Service.Resources.Language.GetText("Common.Error");//操作失败，服务器异常
            }
            return Request.CreateResponse(System.Net.HttpStatusCode.OK, result);
