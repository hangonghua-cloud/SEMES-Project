...
{ 
    ...
    /// 1.创建日期: 2021-07-27
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PL_ProductRuleService 业务服务类
    /// 4.任务编号: 生产放量规则
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PL_ProductRule_Service : RepositoryFactory<PL_ProductRuleEntity>, PL_ProductRuleIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<PL_ProductRuleEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PL_ProductRule] where 1=1  ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //规则描述 是否为空进行查询
                if (!queryParam["RuleContent"].IsEmpty())
                {
                    //sql.Append($" AND RuleContent = N'{queryParam["RuleContent"]}'");
                    sql.Append($" AND RuleContent like N'%{queryParam["RuleContent"]}%'");
                }
                //是否有效 是否为空进行查询
        ...
        /// <param name="List<PL_ProductRuleEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PL_ProductRuleEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PL_ProductRule] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PL_ProductRule] (
                                            [Id]
                                            ,[RuleCode]
                                            ,[RuleName]
                                            ,[RuleContent]
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PL_ProductRule] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PL_ProductRuleEntity</returns>
        ...
        /// <returns>返回PL_ProductRuleEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PL_ProductRuleEntity> bomService = new RepositoryFactory<PL_ProductRuleEntity>();
        
        //    PL_ProductRuleEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PL_ProductRuleDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PL_ProductRule_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<PL_ProductRuleEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PL_ProductRuleEntity> PL_ProductRuleEntity_list =  db2.FindList<PL_ProductRuleEntity>(sql.ToString());
                return PL_ProductRuleEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PL_ProductRuleEntity_DataTable = db2.FindTable(sql.ToString());
                return PL_ProductRuleEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "生产放量规则_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("生产放量规则", dt, true);
                //保存
