...
namespace ALP.Application.Service.BaseManage
{
    ...
    public class UserService : RepositoryFactory<UserEntity>, IUserService
    {
        ...
        public IEnumerable<UserEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            //expression = expression.And(t=>t.EnabledMark==1);//角色禁用之后需要有启用按钮
        ...
        {
            return BaseRepository().GetFirstOrDefaut(condition);
        }
        ...
        /// <returns></returns>
        public IEnumerable<UserEntity> GetUserListByRoleId(string roleId)
        {
            return this.BaseRepository().IQueryable(s => s.RoleId == roleId);
        }
        /// <summary>
        /// 根据部门id获取同一个部门下的所有用户
        /// </summary>
        ...
        /// <returns></returns>
        public IEnumerable<UserEntity> GetUserListByDepartId(string departmentId)
        {
            return this.BaseRepository().IQueryable(s => s.DepartmentId == departmentId);
        }
        ...
        /// <returns></returns>
        public UserEntity GetByEnCode(string enCode)
        {
            return this.BaseRepository().FindEntity(d => d.EnCode == enCode);
        }
        #endregion
        ...
        {
            this.BaseRepository().ExecuteBySql($"UPDATE Base_User SET EnabledMark=0,DeleteMark=1 WHERE UserId ='{keyValue}'");
        }
        /// <summary>
        /// 保存用户表单（新增、修改）
        /// </summary>
