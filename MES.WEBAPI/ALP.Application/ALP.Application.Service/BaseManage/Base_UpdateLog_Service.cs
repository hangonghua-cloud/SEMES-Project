...
{ 
    ...
    /// 1.创建日期: 2021-11-27
    /// 2.创建作者: admin
    /// 3.功能描述: Base_UpdateLogService 业务服务类
    /// 4.任务编号: 升级日志
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class Base_UpdateLog_Service : RepositoryFactory<Base_UpdateLogEntity>
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<Base_UpdateLogEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[Creator]
                      ,[CreateTime]
                  FROM [dbo].[Base_UpdateLog] where 1=1 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //版本号 是否为空进行查询
                if (!queryParam["Version"].IsEmpty())
                {
                    //sql.Append($" AND Version = N'{queryParam["Version"]}'");
                    sql.Append($" AND Version like N'%{queryParam["Version"]}%'");
                }
                //更新内容 是否为空进行查询
                if (!queryParam["Content"].IsEmpty())
                {
                    //sql.Append($" AND Content = N'{queryParam["Content"]}'");
                    sql.Append($" AND Content like N'%{queryParam["Content"]}%'");
                }
                //创建人 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT Id,
                               ...
                               CASE BusinessType
                                   ...
                                   ELSE
                                       ''
                               END BusinessTypeName,
                               [Version],
                               Content,
                               Creator,
                               CreateTime
                        FROM dbo.Base_UpdateLog
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<Base_UpdateLogEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, Base_UpdateLogEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<Base_UpdateLogEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<Base_UpdateLogEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[Base_UpdateLog] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    //StringBuilder sql = new StringBuilder();
                    //sql.Append($@"INSERT INTO [dbo].[Base_UpdateLog] (
                    //                        [Id]
                    //                        ,[BusinessType]
                    //                        ,[Version]
                    //                        ,[Content]
        ...
        {
            int result = 0;
            Base_UpdateLogEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[Base_UpdateLog] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回Base_UpdateLogEntity</returns>
        public Base_UpdateLogEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回Base_UpdateLogEntity</returns>
        public Base_UpdateLogEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回Base_UpdateLogEntity 对象</returns>
        public Base_UpdateLogEntity Get_ExpressionEntity(Expression<Func<Base_UpdateLogEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回Base_UpdateLogEntity 列表</returns>
        public IEnumerable<Base_UpdateLogEntity> Get_ExpressionList(Expression<Func<Base_UpdateLogEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<Base_UpdateLogEntity> bomService = new RepositoryFactory<Base_UpdateLogEntity>();
        
        //    Base_UpdateLogEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    Base_UpdateLogDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.Base_UpdateLog_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<Base_UpdateLogEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<Base_UpdateLogEntity> Base_UpdateLogEntity_list =  db2.FindList<Base_UpdateLogEntity>(sql.ToString());
                return Base_UpdateLogEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable Base_UpdateLogEntity_DataTable = db2.FindTable(sql.ToString());
                return Base_UpdateLogEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "升级日志_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("升级日志", dt, true);
                //保存
