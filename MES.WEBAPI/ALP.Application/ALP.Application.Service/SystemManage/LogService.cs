    ...
    public class LogService : RepositoryFactory<LogEntity>, ILogService
    {
        ...
        public IEnumerable<LogEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            if (!queryParam["Category"].IsEmpty())
            {
                ...
                if (categoryId == 3)
                    expression = expression.And(t => t.ExecuteResultJson.Contains("访问页面"));
            }
            //操作时间
        ...
        public DataTable GetUserInfo()
        {
            ...
            try
            {
                string sql = "select Account,RealName from Base_User where Account is not null and RealName is not null";
        ...
        public void WriteLog(LogEntity logEntity)
        {
            ...
            logEntity.Host = string.IsNullOrEmpty(logEntity.Host) ? Net.Host : logEntity.Host;            
            logEntity.Browser = string.IsNullOrEmpty(logEntity.Browser) ? Net.Browser : logEntity.Browser;
