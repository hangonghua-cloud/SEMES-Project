...
{ 
    ...
    /// 1.创建日期: 2021-08-05
    /// 2.创建作者: 王坤
    /// 3.功能描述: EP_EquipmentCheckItemDetailService 业务服务类
    /// 4.任务编号: 设备点检详情
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class EP_EquipmentCheckItemDetail_Service : RepositoryFactory<EP_EquipmentCheckItemDetailEntity>
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: 王坤
        ...
        public IEnumerable<EP_EquipmentCheckItemDetailEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[EP_EquipmentCheckItemDetail] where EnabledMark = 1 ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT [Id],
                               ...
                               [ModifyBy],
                               [ModifyTime]
                        FROM [dbo].[EP_EquipmentCheckItemDetail]
                        WHERE EnabledMark = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<EP_EquipmentCheckItemDetailEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[EP_EquipmentCheckItemDetail] where IsDeleted = 0 ");
        ...
        public int SaveEntity(string keyValue, EP_EquipmentCheckItemDetailEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                ...
                {
                    EP_EquipmentCheckItemDetailEntity detail = this.BaseRepository().FindEntity(t => t.EnabledMark == true && t.CheckItemId == entity.CheckItemId && t.CheckTaskId == entity.CheckTaskId);
                    if (detail == null)
                    {
                        entity.Create();
        ...
        /// <param name="List<EP_EquipmentCheckItemDetailEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<EP_EquipmentCheckItemDetailEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[EP_EquipmentCheckItemDetail] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[EP_EquipmentCheckItemDetail] (
                                            [Id]
                                            ,[CheckTaskId]
                                            ,[CheckItemId]
                                            ,[CheckItemStandard]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,N'{Save_obj.CheckItemId}'
                                ,N'{Save_obj.CheckItemStandard}'
                                ,N'{Save_obj.DataType}'
                                ,1
                                ,N'{Save_obj.Creator}'
        ...
        {
            int result = 0;
            EP_EquipmentCheckItemDetailEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[EP_EquipmentCheckItemDetail] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回EP_EquipmentCheckItemDetailEntity</returns>
        public EP_EquipmentCheckItemDetailEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回EP_EquipmentCheckItemDetailEntity</returns>
        public EP_EquipmentCheckItemDetailEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回EP_EquipmentCheckItemDetailEntity 对象</returns>
        public EP_EquipmentCheckItemDetailEntity Get_ExpressionEntity(Expression<Func<EP_EquipmentCheckItemDetailEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回EP_EquipmentCheckItemDetailEntity 列表</returns>
        public IEnumerable<EP_EquipmentCheckItemDetailEntity> Get_ExpressionList(Expression<Func<EP_EquipmentCheckItemDetailEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<EP_EquipmentCheckItemDetailEntity> bomService = new RepositoryFactory<EP_EquipmentCheckItemDetailEntity>();
        
        //    EP_EquipmentCheckItemDetailEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    EP_EquipmentCheckItemDetailDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.EP_EquipmentCheckItemDetail_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<EP_EquipmentCheckItemDetailEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<EP_EquipmentCheckItemDetailEntity> EP_EquipmentCheckItemDetailEntity_list =  db2.FindList<EP_EquipmentCheckItemDetailEntity>(sql.ToString());
                return EP_EquipmentCheckItemDetailEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable EP_EquipmentCheckItemDetailEntity_DataTable = db2.FindTable(sql.ToString());
                return EP_EquipmentCheckItemDetailEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        /*public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "设备点检详情_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("设备点检详情", dt, true);
                //保存
