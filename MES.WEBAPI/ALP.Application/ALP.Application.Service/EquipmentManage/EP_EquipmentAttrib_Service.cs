...
{ 
    ...
    /// 1.创建日期: 2021-08-05
    /// 2.创建作者: 王坤
    /// 3.功能描述: EP_EquipmentAttribService 业务服务类
    /// 4.任务编号: 设备台账属性
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class EP_EquipmentAttrib_Service : RepositoryFactory<EP_EquipmentAttribEntity>
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: 王坤
        ...
        public IEnumerable<EP_EquipmentAttribEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[EP_EquipmentAttrib] where IsDeleted = 0 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //属性编码 是否为空进行查询
                if (!queryParam["AttribCode"].IsEmpty())
                {
                    //sql.Append($" AND AttribCode = N'{queryParam["AttribCode"]}'");
                    sql.Append($" AND AttribCode like N'%{queryParam["AttribCode"]}%'");
                }
                //属性名称 是否为空进行查询
                if (!queryParam["AttribName"].IsEmpty())
                {
                    //sql.Append($" AND AttribName = N'{queryParam["AttribName"]}'");
                    sql.Append($" AND AttribName like N'%{queryParam["AttribName"]}%'");
                }
                //属性值 是否为空进行查询
                if (!queryParam["AttribValue"].IsEmpty())
                {
                    //sql.Append($" AND AttribValue = N'{queryParam["AttribValue"]}'");
                    sql.Append($" AND AttribValue like N'%{queryParam["AttribValue"]}%'");
                }
                //创建人 是否为空进行查询
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<EP_EquipmentAttribEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, EP_EquipmentAttribEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<EP_EquipmentAttribEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<EP_EquipmentAttribEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[EP_EquipmentAttrib] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[EP_EquipmentAttrib] (
                                            [Id]
                                            ,[EquipmentId]
                                            ,[EquipmentType]
                                            ,[AttribCode]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,N'{Save_obj.AttribCode}'
                                ,N'{Save_obj.AttribName}'
                                ,N'{Save_obj.AttribValue}'
                                ,N'{Save_obj.Creator}'
        ...
        {
            int result = 0;
            EP_EquipmentAttribEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[EP_EquipmentAttrib] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回EP_EquipmentAttribEntity</returns>
        public EP_EquipmentAttribEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回EP_EquipmentAttribEntity</returns>
        public EP_EquipmentAttribEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回EP_EquipmentAttribEntity 对象</returns>
        public EP_EquipmentAttribEntity Get_ExpressionEntity(Expression<Func<EP_EquipmentAttribEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回EP_EquipmentAttribEntity 列表</returns>
        public IEnumerable<EP_EquipmentAttribEntity> Get_ExpressionList(Expression<Func<EP_EquipmentAttribEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<EP_EquipmentAttribEntity> bomService = new RepositoryFactory<EP_EquipmentAttribEntity>();
        
        //    EP_EquipmentAttribEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    EP_EquipmentAttribDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.EP_EquipmentAttrib_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<EP_EquipmentAttribEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<EP_EquipmentAttribEntity> EP_EquipmentAttribEntity_list =  db2.FindList<EP_EquipmentAttribEntity>(sql.ToString());
                return EP_EquipmentAttribEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable EP_EquipmentAttribEntity_DataTable = db2.FindTable(sql.ToString());
                return EP_EquipmentAttribEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        //        }
        //        string saveFileName = "设备台账属性_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
        //        ExcelHelper Excel = new ExcelHelper();
        //        MemoryStream ms = Excel.DataTableToExcel("设备台账属性", dt, true);
        //        //保存
