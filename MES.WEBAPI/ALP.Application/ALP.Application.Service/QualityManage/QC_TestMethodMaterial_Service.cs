...
{ 
    ...
    /// 1.创建日期: 2021-09-09
    /// 2.创建作者: liyongguo
    /// 3.功能描述: QC_TestMethodMaterialService 业务服务类
    /// 4.任务编号: 原料/IQC检测物料小类关联表
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class QC_TestMethodMaterial_Service : RepositoryFactory<QC_TestMethodMaterialEntity>, QC_TestMethodMaterialIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<QC_TestMethodMaterialEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[QC_TestMethodMaterial] where 1=1  ");
        ...
        /// <param name="List<QC_TestMethodMaterialEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[QC_TestMethodMaterial] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            QC_TestMethodMaterialEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[QC_TestMethodMaterial] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回QC_TestMethodMaterialEntity</returns>
        ...
        /// <returns>返回QC_TestMethodMaterialEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回QC_TestMethodMaterialEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<QC_TestMethodMaterialEntity> bomService = new RepositoryFactory<QC_TestMethodMaterialEntity>();
        
        //    QC_TestMethodMaterialEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    QC_TestMethodMaterialDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.QC_TestMethodMaterial_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<QC_TestMethodMaterialEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<QC_TestMethodMaterialEntity> QC_TestMethodMaterialEntity_list =  db2.FindList<QC_TestMethodMaterialEntity>(sql.ToString());
                return QC_TestMethodMaterialEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable QC_TestMethodMaterialEntity_DataTable = db2.FindTable(sql.ToString());
                return QC_TestMethodMaterialEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "原料/IQC检测物料小类关联表_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("原料/IQC检测物料小类关联表", dt, true);
                //保存
