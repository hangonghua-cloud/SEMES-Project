...
{ 
    ...
    /// 1.创建日期: 2022-12-03
    /// 2.创建作者: jpf
    /// 3.功能描述: PL_MarkUploadService 业务服务类
    /// 4.任务编号: 唛头配置
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PL_MarkUpload_Service : RepositoryFactory<PL_MarkUploadEntity>
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: jpf
        ...
        public IEnumerable<PL_MarkUploadEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PL_MarkUpload] where 1 = 1 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //唛头名称 是否为空进行查询
                if (!queryParam["MarkName"].IsEmpty())
                {
                    //sql.Append($" AND MarkName = N'{queryParam["MarkName"]}'");
                    sql.Append($" AND MarkName like N'%{queryParam["MarkName"]}%'");
                }
                //是否有效 是否为空进行查询
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PL_MarkUploadEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PL_MarkUpload] where IsDeleted = 0 ");
        ...
        public int SaveEntity(string keyValue, PL_MarkUploadEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                ...
                {
                    var MarkUploadEntit= this.BaseRepository().IQueryable(t => (t.MarkCode == entity.MarkCode) && t.IsEnabled == true).ToList().FirstOrDefault();
                    if (MarkUploadEntit != null)
                    {
                        if (MarkUploadEntit.Id == entity.Id)
                        {
                ...
                {
                    var MarkUploadEntit = this.BaseRepository().IQueryable(t => ((t.MarkCode == entity.MarkCode) || (t.MarkName==entity.MarkName) )&& t.IsEnabled == true).ToList().FirstOrDefault();
                    if (MarkUploadEntit == null)
                    {
                        entity.Create();
        ...
        /// <param name="List<PL_MarkUploadEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PL_MarkUploadEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PL_MarkUpload] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PL_MarkUpload] (
                                            [Id]
                                            ,[MarkCode]
                                            ,[MarkName]
                                            ,[IsEnable]
        ...
        {
            int result = 0;
            PL_MarkUploadEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PL_MarkUpload] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PL_MarkUploadEntity</returns>
        public PL_MarkUploadEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PL_MarkUploadEntity</returns>
        public PL_MarkUploadEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PL_MarkUploadEntity 列表</returns>
        public IEnumerable<PL_MarkUploadEntity> Get_ExpressionList(Expression<Func<PL_MarkUploadEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PL_MarkUploadEntity> bomService = new RepositoryFactory<PL_MarkUploadEntity>();
        
        //    PL_MarkUploadEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PL_MarkUploadDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PL_MarkUpload_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PL_MarkUploadEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PL_MarkUploadEntity> PL_MarkUploadEntity_list =  db2.FindList<PL_MarkUploadEntity>(sql.ToString());
                return PL_MarkUploadEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PL_MarkUploadEntity_DataTable = db2.FindTable(sql.ToString());
                return PL_MarkUploadEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        //        }
        //        string saveFileName = "唛头配置_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
        //        ExcelHelper Excel = new ExcelHelper();
        //        MemoryStream ms = Excel.DataTableToExcel("唛头配置", dt, true);
        //        //保存
