...
{ 
    ...
    /// 1.创建日期: 2021-07-26
    /// 2.创建作者: liyongguo
    /// 3.功能描述: BS_ProcessOfOperationsService 业务服务类
    /// 4.任务编号: 供应商管理
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class BS_ProcessOfOperations_Service : RepositoryFactory<BS_ProcessOfOperationsEntity>, BS_ProcessOfOperationsIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT P.Id,
                               ...
                               P.ModifyBy,
                               P.ModifyTime
                        FROM [dbo].[BS_ProcessOfOperations] P
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
        ...
        public IEnumerable<BS_ProcessOfOperationsEntity> GetList(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[BS_ProcessOfOperations] where 1 = 0 ");
        ...
        /// </summary>
        /// <returns></returns>
        public int CopyEntity_List(List<BS_ProcessOfOperationsEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                ...
                {
                    bulkCopy.DestinationTableName = "BS_ProcessOfOperations";
        ...
        /// <param name="List<BS_ProcessOfOperationsEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<BS_ProcessOfOperationsEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[BS_ProcessOfOperations] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    //StringBuilder sql = new StringBuilder();
                    //sql.Append($@"INSERT INTO [dbo].[BS_ProcessOfOperations] (
                    //                        [Id]
                    //                        ,[ProcessCode]
                    //                        ,[ProcessName]
                    //                        ,[SN]
                    ...
                    //            ,N'{Save_obj.OperationCode}',N'{Save_obj.OperationName}'
                    //            ,{Save_obj.SN}
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[BS_ProcessOfOperations] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回BS_ProcessOfOperationsEntity</returns>
        ...
        /// <returns></returns>
        public List<BS_ProcessOfOperationsEntity> GetProcessOfOperationsEntityList(string query)
        {
         
            return DbFactory.Base().FindList<BS_ProcessOfOperationsEntity>(t=>t.ProcessCode == query).ToList();
        }
        ...
        /// <returns>返回BS_ProcessOfOperationsEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回BS_ProcessOfOperationsEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<BS_ProcessOfOperationsEntity> bomService = new RepositoryFactory<BS_ProcessOfOperationsEntity>();
        
        //    BS_ProcessOfOperationsEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    BS_ProcessOfOperationsDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.BS_ProcessOfOperations_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<BS_ProcessOfOperationsEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<BS_ProcessOfOperationsEntity> BS_ProcessOfOperationsEntity_list =  db2.FindList<BS_ProcessOfOperationsEntity>(sql.ToString());
                return BS_ProcessOfOperationsEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable BS_ProcessOfOperationsEntity_DataTable = db2.FindTable(sql.ToString());
                return BS_ProcessOfOperationsEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                      [ProcessCode] as '工艺编码'
                      ,[OperationCode] as '工序编码',OperationName as '工序名称'
                      ,[SN] as '顺序号'
                      ,[OutWarehouse] as '产出仓库编码'
                      ,[CuringCycle] as '养生周期(天)'
                      ,[Creator] as '创建人'
                      ,[CreateTime] as '创建时间'
                      ,[ModifyBy] as '最后修改人'
                      ,[ModifyTime] as '最后修改时间'
                  FROM [dbo].[BS_ProcessOfOperations] where 1 = 1 ");
            msg = "成功!";
            if (!checkType.IsEmpty())
            {
