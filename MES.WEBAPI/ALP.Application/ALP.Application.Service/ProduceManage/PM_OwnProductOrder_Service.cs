...
namespace ALP.Application.Service.ProduceManage
{
    ...
    /// 1.创建日期: 2021-08-26
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PM_OwnProductOrderService 业务服务类
    /// 4.任务编号: 自制半成品工单
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_OwnProductOrder_Service : RepositoryFactory<PM_OwnProductOrderEntity>, PM_OwnProductOrderIService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<PM_OwnProductOrderEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PM_OwnProductOrder] where 1=1  ");
            ...
            sql.Append(@"SELECT P.[Id],
                        ...
                        FROM [dbo].[PM_OwnProductOrder] P
                            ...
                            LEFT JOIN dbo.PL_Material M
                                ON P.WorkOrder = M.WorkOrder
                            LEFT JOIN dbo.PL_Process PS
                                ON PS.WorkOrder = P.WorkOrder
                            LEFT JOIN dbo.BS_People BP
                                ON BP.Code = P.Creator
							LEFT JOIN dbo.V_DataDictionary v2 ON v2.EnCode='WorkOrderStatus' AND p.OrderStatus=v2.ItemValue
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                if (!queryParam["OrderStatus"].IsEmpty())
                {
                    sql.Append($" AND P.OrderStatus = N'{queryParam["OrderStatus"]}'");
                }
                //工单号 是否为空进行查询
                if (!queryParam["WorkOrder"].IsEmpty())
                {
                    //sql.Append($" AND WorkOrder = N'{queryParam["WorkOrder"]}'");
                    sql.Append($" AND P.WorkOrder like N'%{queryParam["WorkOrder"]}%'");
                }
                //物料小类 是否为空进行查询
                if (!queryParam["SmallClass"].IsEmpty())
                {
                    sql.Append($" AND P.SmallClass = N'{queryParam["SmallClass"]}'");
                }
                //物料编码 是否为空进行查询
                if (!queryParam["MaterialCode"].IsEmpty())
                {
                ...
                {
                    sql.Append($" AND P.CreateTime >= N'{queryParam["StartTime"]}'");
                }
                if (!queryParam["EndTime"].IsEmpty())
                {
                    sql.Append($" AND P.CreateTime <= N'{queryParam["EndTime"]}'");
                }
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_OwnProductOrderEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, PM_OwnProductOrderEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PM_OwnProductOrderEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_OwnProductOrderEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_OwnProductOrder] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            PM_OwnProductOrderEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_OwnProductOrder] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_OwnProductOrderEntity</returns>
        public PM_OwnProductOrderEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_OwnProductOrderEntity</returns>
        public PM_OwnProductOrderEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_OwnProductOrderEntity 对象</returns>
        public PM_OwnProductOrderEntity Get_ExpressionEntity(Expression<Func<PM_OwnProductOrderEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_OwnProductOrderEntity 列表</returns>
        public IEnumerable<PM_OwnProductOrderEntity> Get_ExpressionList(Expression<Func<PM_OwnProductOrderEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_OwnProductOrderEntity> bomService = new RepositoryFactory<PM_OwnProductOrderEntity>();

        //    PM_OwnProductOrderEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_OwnProductOrderDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_OwnProductOrder_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_OwnProductOrderEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_OwnProductOrderEntity> PM_OwnProductOrderEntity_list = db2.FindList<PM_OwnProductOrderEntity>(sql.ToString());
                return PM_OwnProductOrderEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_OwnProductOrderEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_OwnProductOrderEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "自制半成品工单_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("自制半成品工单", dt, true);
                //保存
        ...
        public int RemoveForm(Expression<Func<PM_OwnProductOrderEntity, bool>> condition)
        {
