...
{ 
    ...
    /// 1.创建日期: 2021-08-24
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PM_FormulaRecordService 业务服务类
    /// 4.任务编号: 小料喂料记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_FormulaRecord_Service : RepositoryFactory<PM_FormulaRecordEntity>, PM_FormulaRecordIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<PM_FormulaRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PM_FormulaRecord] where 1=1  ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT	
							F.Id,F.BOMId,F.FactoryCode,F.FormulaType,F.FormulaCode,F.FormulaName,F.FormulaTime,F.FormulaMachine,F.FormulaNum,F.UserGroup,
							F.Creator,F.CreateTime,F.UserNames,
							V.ItemName FormulaTypeName,M.ResourceName MachineName,dbo.Fun_GetTeamGroupUserName(f.UserGroup) UserName
						FROM dbo.PM_FormulaRecord F
						LEFT JOIN dbo.V_DataDictionary V ON V.EnCode='MaterialSmall' AND V.ItemValue=F.FormulaType
						LEFT JOIN dbo.BS_ModelWithResource M ON M.ResourceCode=F.FormulaMachine
						WHERE 1=1  ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                   sql.Append($" AND F.BOMId = N'{queryParam["BOMId"]}'");
                }
                //工厂编码 是否为空进行查询
                if (!queryParam["FactoryCode"].IsEmpty())
                {
                ...
                {
                     sql.Append($" AND F.CreateTime >= N'{queryParam["StartTime"]}'");
                }
                if (!queryParam["EndTime"].IsEmpty())
                {
                    sql.Append($" AND F.CreateTime <= N'{queryParam["EndTime"]}'");
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_FormulaRecordEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, PM_FormulaRecordEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PM_FormulaRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_FormulaRecordEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_FormulaRecord] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PM_FormulaRecord] (
                                            [Id]
                                            ,[FactoryCode]
                                            ,[FormulaType]
                                            ,[FormulaCode]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,N'{Save_obj.FormulaType}'
                                ,N'{Save_obj.FormulaCode}'
                                ,N'{Save_obj.FormulaName}'
                                ,'{(Save_obj.FormulaTime == null? DateTime.Now:Save_obj.FormulaTime)}'
                                ,N'{Save_obj.FormulaMachine}'
                                ,{Save_obj.FormulaNum}
        ...
        {
            int result = 0;
            PM_FormulaRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_FormulaRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_FormulaRecordEntity</returns>
        public PM_FormulaRecordEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_FormulaRecordEntity</returns>
        public PM_FormulaRecordEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_FormulaRecordEntity 对象</returns>
        public PM_FormulaRecordEntity Get_ExpressionEntity(Expression<Func<PM_FormulaRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_FormulaRecordEntity 列表</returns>
        public IEnumerable<PM_FormulaRecordEntity> Get_ExpressionList(Expression<Func<PM_FormulaRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_FormulaRecordEntity> bomService = new RepositoryFactory<PM_FormulaRecordEntity>();
        
        //    PM_FormulaRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_FormulaRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_FormulaRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_FormulaRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_FormulaRecordEntity> PM_FormulaRecordEntity_list =  db2.FindList<PM_FormulaRecordEntity>(sql.ToString());
                return PM_FormulaRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_FormulaRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_FormulaRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[FormulaCode] as '配方编码'
                      ,[FormulaName] as '配方名称'
                      ,[FormulaTime] as '配方生效时间'
                      ,[FormulaMachine] as '配料机台'
                      ,[FormulaNum] as '配料付数'
                      ,[UserGroup] as '报工班组'
                      ,[Creator] as '报工人'
                      ,[CreateTime] as '报工时间'
        ...
        public int RemoveForm(Expression<Func<PM_FormulaRecordEntity, bool>> condition)
        {
