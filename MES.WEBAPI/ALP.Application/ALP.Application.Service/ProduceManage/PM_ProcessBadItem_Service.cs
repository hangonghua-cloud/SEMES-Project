...
namespace ALP.Application.Service.ProduceManage
{
    ...
    /// 1.创建日期: 2021-08-10
    /// 2.创建作者: admin
    /// 3.功能描述: PM_ProcessBadItemService 业务服务类
    /// 4.任务编号: 报工不良项目配置
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_ProcessBadItem_Service : RepositoryFactory<PM_ProcessBadItemEntity>, PM_ProcessBadItem_IService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<PM_ProcessBadItemEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,[IsEnabled]
                  FROM [dbo].[PM_ProcessBadItem] where IsEnabled=1 ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT a.Id,a.FactoryCode,a.FactoryName,
                               a.ProcessCode,
                               a.BadItemCode,
                               v.ItemName BadItemName,
                               a.Creator,
                               a.CreateTime,
                               a.ModifyBy,
                               a.ModifyTime,
                               a.IsEnabled
                        FROM dbo.[PM_ProcessBadItem] a
                            LEFT JOIN dbo.V_DataDictionary v
                                ON v.EnCode = 'PoorWorkReport'
                                   AND a.BadItemCode = v.ItemValue
                        WHERE a.IsEnabled = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
        ...
        /// 任务编号: 报工不良项目配置
        /// </summary>
        /// <param name="processCode">查询条件</param>
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_ProcessBadItemEntity> GetList(string processCode, out string msg)
        {
            ...
            if (string.IsNullOrEmpty(processCode))
            {
                ...
                sql.Append(@"select ItemValue BadItemCode,itemName BadItemName from 
                                  V_DataDictionary where EnCode='PoorWorkReport'  order by SortCode");
            }
            else
            {
                ...
                if (!string.IsNullOrEmpty(processCode))
                {
                    sql.Append($@" and a.ProcessCode = N'{processCode}' ");
                }
                sql.Append(@" Order BY v.SortCode");
            }
            msg = "";
            try
            {
        ...
        /// <param name="processCode">查询条件</param>
        /// <returns>返回分页列表</returns>
        public DataTable GetDataTableList(string processCode, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT a.Id,
                               a.ProcessCode,
        ...
        /// <param name="List<PM_ProcessBadItemEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_ProcessBadItemEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_ProcessBadItem] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    //StringBuilder sql = new StringBuilder();
                    //sql.Append($@"INSERT INTO [dbo].[PM_ProcessBadItem] (
                    //                        [Id]
                    //                        ,[ProcessCode]
                    //                        ,[BadItemCode]
                    //                        ,[Creator]
        ...
        {
            int result = 0;
            PM_ProcessBadItemEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_ProcessBadItem] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_ProcessBadItemEntity</returns>
        ...
        /// <returns>返回PM_ProcessBadItemEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_ProcessBadItemEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_ProcessBadItemEntity> bomService = new RepositoryFactory<PM_ProcessBadItemEntity>();

        //    PM_ProcessBadItemEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_ProcessBadItemDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_ProcessBadItem_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        public IEnumerable<PM_ProcessBadItemEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_ProcessBadItemEntity> PM_ProcessBadItemEntity_list = db2.FindList<PM_ProcessBadItemEntity>(sql.ToString());
                return PM_ProcessBadItemEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_ProcessBadItemEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_ProcessBadItemEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "报工不良项目配置_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("报工不良项目配置", dt, true);
                //保存
