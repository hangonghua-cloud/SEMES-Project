...
{ 
    ...
    /// 1.创建日期: 2021-09-16
    /// 2.创建作者: admin
    /// 3.功能描述: PM_MaterialBatchUpRecordService 业务服务类
    /// 4.任务编号: 原料批次上机记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_MaterialBatchUpRecord_Service : RepositoryFactory<PM_MaterialBatchUpRecordEntity>, PM_MaterialBatchUpRecordIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<PM_MaterialBatchUpRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,Flag
                  FROM [dbo].[PM_MaterialBatchUpRecord] where 1=1 ");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@" SELECT MBR.FactoryCode,MBR.FactoryName,MBR.CodeBar,
                               MBR.MaterialCode,
                               MBR.MaterialName,
                               MBR.BatchNo,
                               MBR.CreateTime,
                               ...
                               P1.Name AS Modify,
                               MBR.ModifyTime,
                               M1.ResourceName ProcessName,
                               M2.ResourceName MachineName,
                               MBR.ProcessCode,
                               MBR.MachineCode,
                               MBR.Flag,
                               CASE
                                   WHEN MBR.Flag = '1' THEN
                                       '有效'
                                   ELSE
                                       '无效'
                               END isFlag
                        FROM dbo.PM_MaterialBatchUpRecord MBR
                            LEFT JOIN dbo.BS_People P
                                ON P.Code = MBR.Creator
                            LEFT JOIN dbo.BS_People P1
                                ON P1.Code = MBR.ModifyBy
                            LEFT JOIN dbo.BS_ModelWithResource M1
                                ON M1.ResourceCode = MBR.ProcessCode
                            LEFT JOIN dbo.BS_ModelWithResource M2
                                ON M2.ResourceCode = MBR.MachineCode
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    sql.Append($" AND MBR.FactoryCode = N'{queryParam["FactoryCode"]}'");
                }
                ...
                if (!queryParam["BatchNO"].IsEmpty())
                {
                    //sql.Append($" AND BatachNo = N'{queryParam["BatachNo"]}'");
                    sql.Append($" AND MBR.BatchNO like N'%{queryParam["BatchNO"]}%'");
                }
                //上机状态 是否为空进行查询
                if (!queryParam["Flag"].IsEmpty())
                {
                    //sql.Append($" AND BatachNo = N'{queryParam["BatachNo"]}'");
                    sql.Append($" AND MBR.Flag = N'{queryParam["Flag"]}'");
                }
                //工序编码 是否为空进行查询
                if (!queryParam["ProcessCode"].IsEmpty())
                {
                    //sql.Append($" AND ProcessCode = N'{queryParam["ProcessCode"]}'");
                    sql.Append($" AND MBR.ProcessCode = N'{queryParam["ProcessCode"]}'");
                }
                //机台编码 是否为空进行查询
                if (!queryParam["MachineCode"].IsEmpty())
                {
                    //sql.Append($" AND MachineCode = N'{queryParam["MachineCode"]}'");
                    sql.Append($" AND MBR.MachineCode = N'{queryParam["MachineCode"]}'");
                }
                ...
                {
                    //sql.Append($" AND Creator = N'{queryParam["Creator"]}'");
                    sql.Append($" AND MBR.Creator like N'%{queryParam["Creator"]}%'");
                }
                //创建时间 是否为空进行查询
                if (!queryParam["StartTime"].IsEmpty())
                {
                    //sql.Append($" AND CreateTime = N'{queryParam["CreateTime"]}'");
                    sql.Append($" AND MBR.CreateTime >= N'{queryParam["StartTime"]}'");
                }
                if (!queryParam["EndTime"].IsEmpty())
                {
                    //sql.Append($" AND CreateTime = N'{queryParam["CreateTime"]}'");
                    sql.Append($" AND MBR.CreateTime <= N'{queryParam["EndTime"]}'");
                }
            }
            try
            {
        ...
        /// <param name="List<PM_MaterialBatchUpRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_MaterialBatchUpRecord] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            PM_MaterialBatchUpRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_MaterialBatchUpRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_MaterialBatchUpRecordEntity</returns>
        ...
        /// <returns>返回PM_MaterialBatchUpRecordEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_MaterialBatchUpRecordEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_MaterialBatchUpRecordEntity> bomService = new RepositoryFactory<PM_MaterialBatchUpRecordEntity>();
        
        //    PM_MaterialBatchUpRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_MaterialBatchUpRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_MaterialBatchUpRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        public IEnumerable<PM_MaterialBatchUpRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_MaterialBatchUpRecordEntity> PM_MaterialBatchUpRecordEntity_list =  db2.FindList<PM_MaterialBatchUpRecordEntity>(sql.ToString());
                return PM_MaterialBatchUpRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_MaterialBatchUpRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_MaterialBatchUpRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "原料批次上机记录_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("原料批次上机记录", dt, true);
                //保存
