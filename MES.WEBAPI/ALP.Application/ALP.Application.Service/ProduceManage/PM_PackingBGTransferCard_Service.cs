...
namespace ALP.Application.Service.ProduceManage
{
    ...
    /// 1.创建日期: 2021-08-21
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PM_PackingBGTransferCardService 业务服务类
    /// 4.任务编号: 包装记录表
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_PackingBGTransferCard_Service : RepositoryFactory<PM_PackingBGTransferCardEntity>, PM_PackingBGTransferCardIService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<PM_PackingBGTransferCardEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT Id,
                               ...
                               CreateTime,
                               ModifyBy,
                               ModifyTime FROM dbo.PM_PackingBGTransferCard
	                           WHERE 1=1 ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                if (!queryParam["CreateTime"].IsEmpty())
                {
                    sql.Append($" AND CONVERT(VARCHAR(10),CreateTime,120)='{queryParam["CreateTime"]}'");
                    //sql.Append($" AND CreateTime like N'%{queryParam["CreateTime"]}%'");
                }
                //最后修改人 是否为空进行查询
                if (!queryParam["ModifyBy"].IsEmpty())
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PackingBGTransferCardEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT Id,
                               ProductOrder,
        ...
        public int SaveEntity(string keyValue, PM_PackingBGTransferCardEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PM_PackingBGTransferCardEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_PackingBGTransferCardEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_PackingBGTransferCard] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PM_PackingBGTransferCard] (
                                            [Id]
                                            ,[ProductOrder]
                                            ,[WorkOrder]
                                            ,[ContainerNO]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,N'{Save_obj.MaterialCode}',N'{Save_obj.PaperBox}'
                                ,N'{Save_obj.CardCode}',N'{Save_obj.CardName}'
                                ,N'{Save_obj.ReWorkOrder}'
                                ,{Save_obj.Qty},{Save_obj.BadQty},{Save_obj.Unit}
        ...
        {
            int result = 0;
            PM_PackingBGTransferCardEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_PackingBGTransferCard] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_PackingBGTransferCardEntity</returns>
        public PM_PackingBGTransferCardEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_PackingBGTransferCardEntity</returns>
        public PM_PackingBGTransferCardEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_PackingBGTransferCardEntity 对象</returns>
        public PM_PackingBGTransferCardEntity Get_ExpressionEntity(Expression<Func<PM_PackingBGTransferCardEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_PackingBGTransferCardEntity 列表</returns>
        public IEnumerable<PM_PackingBGTransferCardEntity> Get_ExpressionList(Expression<Func<PM_PackingBGTransferCardEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_PackingBGTransferCardEntity> bomService = new RepositoryFactory<PM_PackingBGTransferCardEntity>();

        //    PM_PackingBGTransferCardEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_PackingBGTransferCardDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_PackingBGTransferCard_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PackingBGTransferCardEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_PackingBGTransferCardEntity> PM_PackingBGTransferCardEntity_list = db2.FindList<PM_PackingBGTransferCardEntity>(sql.ToString());
                return PM_PackingBGTransferCardEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_PackingBGTransferCardEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_PackingBGTransferCardEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                  ...
                  FROM [dbo].[PM_PackingBGTransferCard] where 1=1 ");
            msg = "成功!";
            if (!checkType.IsEmpty())
            {
        ...
        public int RemoveForm(Expression<Func<PM_PackingBGTransferCardEntity, bool>> condition)
        {
