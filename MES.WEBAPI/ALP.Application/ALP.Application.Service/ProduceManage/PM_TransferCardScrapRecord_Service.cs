...
{ 
    ...
    /// 1.创建日期: 2021-08-19
    /// 2.创建作者: admin
    /// 3.功能描述: PM_TransferCardScrapRecordService 业务服务类
    /// 4.任务编号: PM_流转卡报废记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_TransferCardScrapRecord_Service : RepositoryFactory<PM_TransferCardScrapRecordEntity>, PM_TransferCardScrapRecord_IService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<PM_TransferCardScrapRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,[IsEnabled]
                  FROM [dbo].[PM_TransferCardScrapRecord] where IsEnabled=1 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //报废数量 是否为空进行查询
                if (!queryParam["ScrapQty"].IsEmpty())
                {
                    //sql.Append($" AND ScrapQty = N'{queryParam["ScrapQty"]}'");
                    sql.Append($" AND ScrapQty like N'%{queryParam["ScrapQty"]}%'");
                }
                //报废原因 是否为空进行查询
                if (!queryParam["ScarpReaon"].IsEmpty())
                {
                    //sql.Append($" AND ScarpReaon = N'{queryParam["ScarpReaon"]}'");
                    sql.Append($" AND ScarpReaon like N'%{queryParam["ScarpReaon"]}%'");
                }
                //操作人 是否为空进行查询
                ...
                //备注 是否为空进行查询
                if (!queryParam["Remak"].IsEmpty())
                {
                    //sql.Append($" AND Remak = N'{queryParam["Remak"]}'");
                    sql.Append($" AND Remak like N'%{queryParam["Remak"]}%'");
                }
                //创建人 是否为空进行查询
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_TransferCardScrapRecordEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, PM_TransferCardScrapRecordEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PM_TransferCardScrapRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_TransferCardScrapRecordEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_TransferCardScrapRecord] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PM_TransferCardScrapRecord] (
                                            [Id]
                                            ,[CardCode]
                                            ,[ProcessCode]
                                            ,[ScrapQty]
        ...
        {
            int result = 0;
            PM_TransferCardScrapRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int RemoveForm(Expression<Func<PM_TransferCardScrapRecordEntity, bool>> condition)
        {
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_TransferCardScrapRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_TransferCardScrapRecordEntity</returns>
        public PM_TransferCardScrapRecordEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_TransferCardScrapRecordEntity</returns>
        public PM_TransferCardScrapRecordEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_TransferCardScrapRecordEntity 对象</returns>
        public PM_TransferCardScrapRecordEntity Get_ExpressionEntity(Expression<Func<PM_TransferCardScrapRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_TransferCardScrapRecordEntity 列表</returns>
        public IEnumerable<PM_TransferCardScrapRecordEntity> Get_ExpressionList(Expression<Func<PM_TransferCardScrapRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_TransferCardScrapRecordEntity> bomService = new RepositoryFactory<PM_TransferCardScrapRecordEntity>();
        
        //    PM_TransferCardScrapRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_TransferCardScrapRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_TransferCardScrapRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_TransferCardScrapRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_TransferCardScrapRecordEntity> PM_TransferCardScrapRecordEntity_list =  db2.FindList<PM_TransferCardScrapRecordEntity>(sql.ToString());
                return PM_TransferCardScrapRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_TransferCardScrapRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_TransferCardScrapRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "PM_流转卡报废记录_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("PM_流转卡报废记录", dt, true);
                //保存
