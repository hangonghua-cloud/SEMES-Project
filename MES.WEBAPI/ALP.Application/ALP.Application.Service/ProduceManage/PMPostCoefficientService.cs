...
{
    /// <summary>
    /// [PM_PostCoefficient]表数据访问类
    /// 作者:Dragon
    /// 创建时间:2022-11-07 13:33:43
    /// </summary>
    public class PMPostCoefficientService : RepositoryFactory<PMPostCoefficientEntity>
    {
        #region 查询分页列表
        /// <summary>
        ...
        public DataTable GetPostPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT a.Id,
                               ...
                               Remark2 FactoryName,
                               a.CreateTime
                        FROM dbo.Base_KeyParameterItem a
                        WHERE a.EnCode = 'ProcessPost'
                              AND a.IsEnabled = 1 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    sql.Append($" AND A.Remark1 = N'{queryParam["FactoryCode"]}'");
                }
                if (!queryParam["ProcessCode"].IsEmpty())
                {
                    sql.Append($" AND A.ItemCode = N'{queryParam["ProcessCode"]}'");
                }
                if (!queryParam["PostCode"].IsEmpty())
                {
                    sql.Append($" AND A.Col1 = N'{queryParam["PostCode"]}'");
                }
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT
                         ...
                         FROM [dbo].[PM_PostCoefficient] A where 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                JObject queryParam = queryJson.ToJObject();
                ...
                {
                    sql.Append($" AND A.PostId = N'{queryParam["PostId"]}'");
                }
                if (!queryParam["PostCode"].IsEmpty())
                {
                    sql.Append($" AND A.PostCode like N'%{queryParam["PostCode"]}%'");
                }
        ...
        ///<returns>PMPostCoefficientEntity</returns>
        public PMPostCoefficientEntity GetEntity(Expression<Func<PMPostCoefficientEntity, bool>> condition)
        {
        ...
        ///</summary>
        ///<param name="condition">查询条件</param>
        ///<returns>PMPostCoefficientEntity列表</returns>
        public IEnumerable<PMPostCoefficientEntity> GetList(Expression<Func<PMPostCoefficientEntity, bool>> condition)
        {
        ...
        ///<param name="condition">查询条件</param>
        ///<returns>PMPostCoefficientEntity列表</returns>
        public bool Any(Expression<Func<PMPostCoefficientEntity, bool>> condition)
        {
        ...
        public int SaveEntity(string keyValue, PMPostCoefficientEntity entity)
        {
            try
            {
        ...
        ///<returns>返回插入条数</returns>
        public int SaveEntity_List(bool isUpdate, List<PMPostCoefficientEntity> list)
        {
            try
            {
                if (isUpdate)
                {
                    ...
                    if (list.Count > 0)
                    {
                        foreach (var Save_obj in list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_PostCoefficient] set ");
                            string keyValue = "";
                ...
                else
                {
                    ...
                    //{
                    //   bulkCopy.DestinationTableName = PM_PostCoefficient;
        ...
        ///<returns>PMPostCoefficientEntity</returns>
        public int RemoveForm(Expression<Func<PMPostCoefficientEntity, bool>> condition)
        {
