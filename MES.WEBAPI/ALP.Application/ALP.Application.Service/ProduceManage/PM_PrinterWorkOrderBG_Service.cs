...
{ 
    ...
    /// 1.创建日期: 2021-08-23
    /// 2.创建作者: liyongguo
    /// 3.功能描述: PM_PrinterWorkOrderBGService 业务服务类
    /// 4.任务编号: 印刷计划工单
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_PrinterWorkOrderBG_Service : RepositoryFactory<PM_PrinterWorkOrderBGEntity>, PM_PrinterWorkOrderBGIService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        /// 创建日期: 2021-08-23 16:05:55
        /// 任务编号: 印刷计划工单
        /// </summary>
        /// <param name="pagination">分页</param>
        /// <param name="queryJson">查询参数</param>
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PrinterWorkOrderBGEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[PM_PrinterWorkOrderBG] where 1=1  ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //白料批次 是否为空进行查询
                if (!queryParam["WhiteBatchNo"].IsEmpty())
                {
                    //sql.Append($" AND WhiteBatchNo = N'{queryParam["WhiteBatchNo"]}'");
                    sql.Append($" AND WhiteBatchNo like N'%{queryParam["WhiteBatchNo"]}%'");
                }
                //报工日期 是否为空进行查询
                if (!queryParam["BGTime"].IsEmpty())
                {
                    //sql.Append($" AND BGTime = N'{queryParam["BGTime"]}'");
                    sql.Append($" AND BGTime like N'%{queryParam["BGTime"]}%'");
                }
                //报工班组 是否为空进行查询
                if (!queryParam["Team"].IsEmpty())
                {
                    //sql.Append($" AND Team = N'{queryParam["Team"]}'");
                    sql.Append($" AND Team like N'%{queryParam["Team"]}%'");
                }
                //报工人 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@" SELECT PWB.Id,
                               ...
                               PWB.BatchNo,
                               PWB.WhiteBatchNo,
                               PWB.ProcessCode BGProcessCode,
	                           M1.ResourceName BGProcessName,
                               PW.MaterialCode,
                               PWB.UserGroup,
                               PWB.ReelNum,
                               PWB.MeterNum,
                               ...
                               PWB.Creator,
                               PWB.CreateTime,
                               T.PTeamName UserGroupName,
                               PM.ProcessName ProcessRoutName,
                               PW.ProcessRoute,
                               dbo.Fun_GetTeamGroupUserName(T.PTeamCode) UserProple
                        FROM dbo.PM_PrinterWorkOrder PW
                            INNER JOIN dbo.PM_PrinterWorkOrderBG PWB
                                ON PWB.WorkOrder = PW.WorkOrder
                            LEFT JOIN dbo.BS_ModelWithResource M
                                ON M.ResourceCode = PWB.ProcessCode
                            LEFT JOIN dbo.BS_ModelWithResource M1
                                ON M1.ResourceCode = PWB.ProcessCode
                            LEFT JOIN dbo.PM_TeamPerson T
                                ON PWB.UserGroup = T.PTeamCode
                                   AND PWB.ProcessCode = T.ProcessCode
                            LEFT JOIN dbo.BS_Process PM
                                ON PW.ProcessRoute = PM.ProcessCode
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND WorkOrder = N'{queryParam["WorkOrder"]}'");
                    sql.Append($" AND PW.PrinterOrder like N'%{queryParam["PrinterOrder"]}%'");
                }
                if (!queryParam["PlanOrder"].IsEmpty())
                {
                    //sql.Append($" AND WorkOrder = N'{queryParam["WorkOrder"]}'");
                    sql.Append($" AND PW.PlanOrder like N'%{queryParam["PlanOrder"]}%'");
                }
                //工单号 是否为空进行查询
                ...
                {
                    //sql.Append($" AND WorkOrder = N'{queryParam["WorkOrder"]}'");
                    sql.Append($" AND PW.DesignColour like N'%{queryParam["DesignColour"]}%'");
                }
                //报工工序 是否为空进行查询
                if (!queryParam["ProcessCode"].IsEmpty())
                {
                    sql.Append($" AND PWB.ProcessCode = N'{queryParam["ProcessCode"]}'");
                }
                ...
                {
                    //sql.Append($" AND BatchNo = N'{queryParam["BatchNo"]}'");
                    sql.Append($" AND PW.BatchNo like N'%{queryParam["BatchNo"]}%'");
                }
                //白料批次 是否为空进行查询
                if (!queryParam["WhiteBatchNo"].IsEmpty())
                {
                ...
                if (!queryParam["Team"].IsEmpty())
                {
                    sql.Append($" AND PWB.Team = N'{queryParam["Team"]}'");
                }
               
                //报工时间 是否为空进行查询
                if (!queryParam["StartOrder"].IsEmpty())
                {
                    sql.Append($" AND PWB.CreateTime >= N'{queryParam["StartOrder"]}'");
 
                }
                if (!queryParam["EndOrder"].IsEmpty())
                {
                    sql.Append($" AND PWB.CreateTime <= N'{queryParam["EndOrder"]}'");
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PrinterWorkOrderBGEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, PM_PrinterWorkOrderBGEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PM_PrinterWorkOrderBGEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_PrinterWorkOrderBGEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_PrinterWorkOrderBG] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[PM_PrinterWorkOrderBG] (
                                            [Id]
                                            ,[WorkOrder]
                                            ,[ProcessCode]
                                            ,[UserGroup]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,N'{Save_obj.UserGroup}'
                                ,{Save_obj.ReelNum}
                                ,{Save_obj.MeterNum}
                                ,N'{Save_obj.BatchNo}'
                                ,N'{Save_obj.WhiteBatchNo}'
                                ,'{(Save_obj.BGTime == null? DateTime.Now:Save_obj.BGTime)}'
                                ,N'{Save_obj.Team}'
                                ,N'{Save_obj.Creator}'
        ...
        {
            int result = 0;
            PM_PrinterWorkOrderBGEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_PrinterWorkOrderBG] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_PrinterWorkOrderBGEntity</returns>
        public PM_PrinterWorkOrderBGEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_PrinterWorkOrderBGEntity</returns>
        public PM_PrinterWorkOrderBGEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_PrinterWorkOrderBGEntity 对象</returns>
        public PM_PrinterWorkOrderBGEntity Get_ExpressionEntity(Expression<Func<PM_PrinterWorkOrderBGEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_PrinterWorkOrderBGEntity 列表</returns>
        public IEnumerable<PM_PrinterWorkOrderBGEntity> Get_ExpressionList(Expression<Func<PM_PrinterWorkOrderBGEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_PrinterWorkOrderBGEntity> bomService = new RepositoryFactory<PM_PrinterWorkOrderBGEntity>();
        
        //    PM_PrinterWorkOrderBGEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_PrinterWorkOrderBGDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_PrinterWorkOrderBG_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_PrinterWorkOrderBGEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_PrinterWorkOrderBGEntity> PM_PrinterWorkOrderBGEntity_list =  db2.FindList<PM_PrinterWorkOrderBGEntity>(sql.ToString());
                return PM_PrinterWorkOrderBGEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_PrinterWorkOrderBGEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_PrinterWorkOrderBGEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public int RemoveForm(Expression<Func<PM_PrinterWorkOrderBGEntity, bool>> condition)
        {
