...
{ 
    ...
    /// 1.创建日期: 2021-08-18
    /// 2.创建作者: admin
    /// 3.功能描述: PM_TransferBGPersonRecordService 业务服务类
    /// 4.任务编号: 流转报工人员记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class PM_TransferBGPersonRecord_Service : RepositoryFactory<PM_TransferBGPersonRecordEntity>, PM_TransferBGPersonRecord_IService
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        /// 创建日期: 2021-08-18 09:27:14
        /// 任务编号: 流转报工人员记录
        /// </summary>
        /// <param name="pagination">分页</param>
        /// <param name="queryJson">查询参数</param>
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_TransferBGPersonRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyTime]
                      ,[IsEnabled]
                  FROM [dbo].[PM_TransferBGPersonRecord] where IsDeleted = 0 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND BGID = N'{queryParam["BGID"]}'");
                    sql.Append($" AND BGID like N'%{queryParam["BGID"]}%'");
                }
                //生产小组编码 是否为空进行查询
                ...
                //生产小组名称 是否为空进行查询
                if (!queryParam["PTeamName"].IsEmpty())
                {
                    //sql.Append($" AND PTeamName = N'{queryParam["PTeamName"]}'");
                    sql.Append($" AND PTeamName like N'%{queryParam["PTeamName"]}%'");
                }
                //岗位编码 是否为空进行查询
                if (!queryParam["PostCode"].IsEmpty())
                {
                    //sql.Append($" AND PostCode = N'{queryParam["PostCode"]}'");
                    sql.Append($" AND PostCode like N'%{queryParam["PostCode"]}%'");
                }
                //岗位编码 是否为空进行查询
                if (!queryParam["PostName"].IsEmpty())
                {
                    //sql.Append($" AND PostName = N'{queryParam["PostName"]}'");
                    sql.Append($" AND PostName like N'%{queryParam["PostName"]}%'");
                }
                //人员编码 是否为空进行查询
                if (!queryParam["UserCode"].IsEmpty())
                {
                    //sql.Append($" AND UserCode = N'{queryParam["UserCode"]}'");
                    sql.Append($" AND UserCode like N'%{queryParam["UserCode"]}%'");
                }
                //人员名称 是否为空进行查询
                if (!queryParam["UserName"].IsEmpty())
                {
                    //sql.Append($" AND UserName = N'{queryParam["UserName"]}'");
                    sql.Append($" AND UserName like N'%{queryParam["UserName"]}%'");
                }
                //创建人编码 是否为空进行查询
        ...
        /// <returns></returns>
        public DataTable GetPageDataTableList1(Pagination pagination, string queryJson)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT PB.CardCode,
                               PT.CardName,
                               PB.ProcessCode,
                               PMB.PTeamCode,
                               PMB.PTeamName,
                               LEFT(PMB.UserNames,LEN(pmb.UserNames)-1) UserNames,
                               M.ResourceName ProcessName,
                               PB.Qty,
                               PB.Unit,
	                           PB.CreateTime
                        FROM dbo.PM_TransferCard PT
                            INNER JOIN dbo.PM_TranferCardBGRecord PB
                                ON PB.CardCode = PT.CardCode
                            LEFT JOIN
                            (
                                SELECT BGID,
                                       A.PTeamCode,
                            ...
                            LEFT JOIN dbo.BS_ModelWithResource M
                                ON M.ResourceCode = PB.ProcessCode
                        WHERE 1 = 1  ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    sql.Append($" AND PT.WorkOrder = N'{queryParam["WorkOrder"]}'");
                }
                if (!queryParam["ProcessCode"].IsEmpty())
                {
                    sql.Append($" AND PB.ProcessCode = N'{queryParam["ProcessCode"]}'");
                }
            }
            try
            {
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT A.Id,
                               ...
                               B.Unit
                        FROM [dbo].[PM_TransferBGPersonRecord] A
                            LEFT JOIN dbo.[PM_TranferCardBGRecord] B
                                ON A.BGID = B.Id
                        WHERE A.IsEnabled = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                if (!queryParam["BGID"].IsEmpty())
                {
                    sql.Append($" AND A.BGID = N'{queryParam["BGID"]}'");
                }
                //生产小组编码 是否为空进行查询
                if (!queryParam["PTeamCode"].IsEmpty())
                {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_TransferBGPersonRecordEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public IEnumerable<PM_TransferBGPersonRecordEntity> GetList(Expression<Func<PM_TransferBGPersonRecordEntity, bool>> condition)
        {
        ...
        public int SaveEntity(string keyValue, PM_TransferBGPersonRecordEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<PM_TransferBGPersonRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<PM_TransferBGPersonRecordEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[PM_TransferBGPersonRecord] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            PM_TransferBGPersonRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int RemoveForm(Expression<Func<PM_TransferBGPersonRecordEntity, bool>> condition)
        {
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[PM_TransferBGPersonRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回PM_TransferBGPersonRecordEntity</returns>
        public PM_TransferBGPersonRecordEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回PM_TransferBGPersonRecordEntity</returns>
        public PM_TransferBGPersonRecordEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回PM_TransferBGPersonRecordEntity 对象</returns>
        public PM_TransferBGPersonRecordEntity Get_ExpressionEntity(Expression<Func<PM_TransferBGPersonRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回PM_TransferBGPersonRecordEntity 列表</returns>
        public IEnumerable<PM_TransferBGPersonRecordEntity> Get_ExpressionList(Expression<Func<PM_TransferBGPersonRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<PM_TransferBGPersonRecordEntity> bomService = new RepositoryFactory<PM_TransferBGPersonRecordEntity>();
        
        //    PM_TransferBGPersonRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    PM_TransferBGPersonRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.PM_TransferBGPersonRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        //        entity.IsEnabled = false;
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<PM_TransferBGPersonRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<PM_TransferBGPersonRecordEntity> PM_TransferBGPersonRecordEntity_list =  db2.FindList<PM_TransferBGPersonRecordEntity>(sql.ToString());
                return PM_TransferBGPersonRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable PM_TransferBGPersonRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return PM_TransferBGPersonRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "流转报工人员记录_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("流转报工人员记录", dt, true);
                //保存
