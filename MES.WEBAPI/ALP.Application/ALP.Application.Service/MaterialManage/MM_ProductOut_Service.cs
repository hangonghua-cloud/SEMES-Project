...
namespace ALP.Application.Service.MaterialManage
{
    ...
    /// 1.创建日期: 2021-10-11
    /// 2.创建作者: admin
    /// 3.功能描述: MM_ProductOut_Service 业务服务类
    /// 4.任务编号: 成品入库记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_ProductOut_Service : RepositoryFactory<MM_ProductOutEntity>
    {
        /// <summary>
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT a.[Id],
                               ...
                               a.[Remark],
                               a.[Creator],      --入库人编码
                               b.Name UserName,  --入库名称
                               a.[CreateTime],   --入库时间
                               a.[ModifyBy],
                               a.[ModifyTime]
                        FROM [dbo].[MM_ProductOut] a
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                if (!queryParam["Creator"].IsEmpty())
                {
                    ...
                    sql.Append($" AND (a.Creator like N'%{queryParam["Creator"]}%' OR b.Name LIKE N'%{queryParam["Creator"]}%')");
                }
                //创建时间 是否为空进行查询
                ...
                {
                    sql.Append($" AND a.CreateTime>='{queryParam["StartTime"]}'");
                }
                //结束时间 是否为空进行查询
                if (!queryParam["EndTime"].IsEmpty())
                {
                    sql.Append($" AND a.CreateTime<='{queryParam["EndTime"]}'");
                }
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_ProductOutEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[MM_ProductOut] where 1=1 ");
        ...
        public IEnumerable<MM_ProductOutEntity> GetList(Expression<Func<MM_ProductOutEntity, bool>> condition)
        {
        ...
        public int SaveEntity(string keyValue, MM_ProductOutEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<MM_ProductOutEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<MM_ProductOutEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_ProductOut] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            MM_ProductOutEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_ProductOut] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_ProductOutEntity</returns>
        public MM_ProductOutEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回MM_ProductOutEntity</returns>
        public MM_ProductOutEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回MM_ProductOutEntity 对象</returns>
        public MM_ProductOutEntity Get_ExpressionEntity(Expression<Func<MM_ProductOutEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_ProductOutEntity 列表</returns>
        public IEnumerable<MM_ProductOutEntity> Get_ExpressionList(Expression<Func<MM_ProductOutEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_ProductOutEntity> bomService = new RepositoryFactory<MM_ProductOutEntity>();

        //    MM_ProductOutEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_ProductOutDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_ProductOut_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_ProductOutEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_ProductOutEntity> MM_ProductOutEntity_list = db2.FindList<MM_ProductOutEntity>(sql.ToString());
                return MM_ProductOutEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_ProductOutEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_ProductOutEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "成品入库记录_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("成品入库记录", dt, true);
                //保存
