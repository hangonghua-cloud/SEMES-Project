...
namespace ALP.Application.Service.MaterialManage
{
    ...
    /// 1.创建日期: 2021-10-13
    /// 2.创建作者: admin
    /// 3.功能描述: MM_ProductDispatchDetailService 业务服务类
    /// 4.任务编号: 成品发货详情
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_ProductDispatchDetail_Service : RepositoryFactory<MM_ProductDispatchDetailEntity>, MM_ProductDispatchDetail_IService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<MM_ProductDispatchDetailEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[MM_ProductDispatchDetail] where 1=1 ");
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT a.Id,
							   ...
							   a.DetailGrossWeight,
							   a.DetailVolume
                        FROM dbo.MM_ProductDispatchDetail a
                        WHERE 1 = 1 ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //查询条件 
                //Id 是否为空进行查询
                if (!queryParam["DispatchItemId"].IsEmpty())
                {
                    //sql.Append($" AND Id = N'{queryParam["Id"]}'");
                    sql.Append($" AND DispatchItemId = N'{queryParam["DispatchItemId"]}'");
                }
                //发货单号 是否为空进行查询
                if (!queryParam["DocNum"].IsEmpty())
                {
                    //sql.Append($" AND DocNum = N'{queryParam["DocNum"]}'");
                    sql.Append($" AND a.DocNum like N'%{queryParam["DocNum"]}%'");
                }
                //工单号 是否为空进行查询
        ...
        public IEnumerable<MM_ProductDispatchDetailEntity> GetList(Expression<Func<MM_ProductDispatchDetailEntity, bool>> condition)
        {
        ...
        /// <param name="List<MM_ProductDispatchDetailEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_ProductDispatchDetail] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            MM_ProductDispatchDetailEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int RemoveForm(Expression<Func<MM_ProductDispatchDetailEntity, bool>> condition)
        {
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_ProductDispatchDetail] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_ProductDispatchDetailEntity</returns>
        ...
        /// <returns>返回MM_ProductDispatchDetailEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_ProductDispatchDetailEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_ProductDispatchDetailEntity> bomService = new RepositoryFactory<MM_ProductDispatchDetailEntity>();

        //    MM_ProductDispatchDetailEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_ProductDispatchDetailDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_ProductDispatchDetail_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<MM_ProductDispatchDetailEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_ProductDispatchDetailEntity> MM_ProductDispatchDetailEntity_list = db2.FindList<MM_ProductDispatchDetailEntity>(sql.ToString());
                return MM_ProductDispatchDetailEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_ProductDispatchDetailEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_ProductDispatchDetailEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "成品发货详情_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("成品发货详情", dt, true);
                //保存
