...
{ 
    ...
    /// 1.创建日期: 2021-10-29
    /// 2.创建作者: liyongguo
    /// 3.功能描述: MM_SaleDomesticService 业务服务类
    /// 4.任务编号: 内销成品库存
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_SaleDomestic_Service : RepositoryFactory<MM_SaleDomesticEntity>
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        /// 创建日期: 2021-10-29 16:15:07
        /// 任务编号: 内销成品库存
        /// </summary>
        ...
        public DataTable GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT M.[Id],
                               ...
                               M.[MaterialCode],
                               M.[WhsCode],
                               W1.ResourceName WhsName,
                               M.[LocationCode],
                               W2.ResourceName LocationName,
                               M.[Attr],
                               M.[IsEnabled],
                               M.Remark,
                               M.CreateTime,
                               M.BoxQty - ISNULL(MD.BoxQty, 0) BoxQty,
                               M.[PieceQty] - ISNULL(MD.PieceQty, 0) PieceQty
                        FROM [dbo].[MM_SaleDomestic] M
                            LEFT JOIN dbo.BS_ModelWithResource W1
                                ON W1.ResourceCode = M.WhsCode
                            LEFT JOIN dbo.BS_ModelWithResource W2
                                ON W2.ResourceCode = M.LocationCode
                            LEFT JOIN
                            (
                                SELECT MD1.ProductOrder,
                                       MD1.MaterialCode,
                                       MD1.WhsCode,
                                       MD1.LocationCode,
                                       SUM(MD1.BoxQty) BoxQty,
                                       SUM(MD1.PieceQty) PieceQty
                                FROM dbo.MM_SaleDomesticOut MO
                                    INNER JOIN dbo.MM_SaleDomesticOutDetail MD1
                                        ON MD1.DocNum = MO.DocNum
                                WHERE MO.Status = '1' --创建发货单
                                GROUP BY MD1.ProductOrder,
                                         MD1.MaterialCode,
                                         MD1.WhsCode,
                                         MD1.LocationCode
                            ) MD
                                ON MD.LocationCode = M.LocationCode
                                   AND MD.MaterialCode = M.MaterialCode
                                   AND MD.WhsCode = M.WhsCode
                                   AND M.ProductOrder = MD.ProductOrder
                        WHERE M.[PieceQty] - ISNULL(MD.PieceQty, 0) > 0 ");
            //AND M.IsEnabled = '1' AND M.Attr = '1' 成品
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    sql.Append($" AND M.FactoryCode = N'{queryParam["FactoryCode"]}'");
                }
                //订单号 是否为空进行查询
                ...
                //库位属性 是否为空进行查询
                if (!queryParam["Attr"].IsEmpty())
                {
                    sql.Append($" AND Attr = N'{queryParam["Attr"]}'");
                }
                if (!queryParam["IsEnabled"].IsEmpty())
                {
                    sql.Append($" AND M.IsEnabled = N'{queryParam["IsEnabled"]}'");
                }
            ...
            sql.Append(@"SELECT M.[Id],
                        ...
                        FROM [dbo].[MM_SaleDomestic] M
                            ...
                            LEFT JOIN dbo.BS_People P
                                ON M.Creator = P.Code
	                        LEFT JOIN dbo.Base_Material a ON m.MaterialCode=a.MaterialCode
                        WHERE M.[IsEnabled] = '1' ");
            var parameter = new List<DbParameter>();
            if (!string.IsNullOrEmpty(queryJson))
            {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_SaleDomesticEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[MM_SaleDomestic] where 1=1 ");
        ...
        public int SaveEntity(string keyValue, MM_SaleDomesticEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<MM_SaleDomesticEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<MM_SaleDomesticEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_SaleDomestic] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            MM_SaleDomesticEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            { 
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_SaleDomestic] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_SaleDomesticEntity</returns>
        public MM_SaleDomesticEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回MM_SaleDomesticEntity</returns>
        public MM_SaleDomesticEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回MM_SaleDomesticEntity 对象</returns>
        public MM_SaleDomesticEntity Get_ExpressionEntity(Expression<Func<MM_SaleDomesticEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_SaleDomesticEntity 列表</returns>
        public IEnumerable<MM_SaleDomesticEntity> Get_ExpressionList(Expression<Func<MM_SaleDomesticEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_SaleDomesticEntity> bomService = new RepositoryFactory<MM_SaleDomesticEntity>();
        
        //    MM_SaleDomesticEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_SaleDomesticDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_SaleDomestic_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_SaleDomesticEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_SaleDomesticEntity> MM_SaleDomesticEntity_list =  db2.FindList<MM_SaleDomesticEntity>(sql.ToString());
                return MM_SaleDomesticEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_SaleDomesticEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_SaleDomesticEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "内销成品库存_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";
                
                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("内销成品库存", dt, true);
                //保存
