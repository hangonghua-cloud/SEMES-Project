...
namespace ALP.Application.Service.MaterialManage
{
    ...
    /// 1.创建日期: 2021-10-13
    /// 2.创建作者: admin
    /// 3.功能描述: MM_ProductDispatchBillService 业务服务类
    /// 4.任务编号: 成品发货单
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_ProductDispatchBill_Service : RepositoryFactory<MM_ProductDispatchBillEntity>, MM_ProductDispatchBill_IService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<MM_ProductDispatchBillEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[CreateTime]
                      ,[ModifyBy]
                      ,[ModifyTime],DeliveryType,Remark,DeliveryDate,Operator
                  FROM [dbo].[MM_ProductDispatchBill] where 1=1 ");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT a.Id,a.DocNum,a.Customer,
                               a.DeliveryType,
                               ...
                               a.Operator,
                               a.CreateTime
                        FROM dbo.MM_ProductDispatchBill a
                            LEFT JOIN dbo.V_DataDictionary v1
                                ON v1.EnCode = 'DeliveryType'
                                   AND a.DeliveryType = v1.ItemValue
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND DocNum = N'{queryParam["DocNum"]}'");
                    sql.Append($" AND a.DeliveryType like N'%{queryParam["DeliveryType"]}%'");
                }
                //客户 是否为空进行查询
                if (!queryParam["Customer"].IsEmpty())
                {
                ...
                {
                    //sql.Append($" AND Status = N'{queryParam["Status"]}'");
                    sql.Append($" AND a.Status like N'%{queryParam["Status"]}%'");
                }
                //创建人 是否为空进行查询
                if (!queryParam["Creator"].IsEmpty())
                {
        ...
        public IEnumerable<MM_ProductDispatchBillEntity> GetList(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[CreateTime]
                      ,[ModifyBy]
                      ,[ModifyTime],DeliveryType,Remark
                  FROM [dbo].[MM_ProductDispatchBill] where 1=1 ");
        ...
        public IEnumerable<MM_ProductDispatchBillEntity> GetList(Expression<Func<MM_ProductDispatchBillEntity, bool>> condition)
        {
        ...
        /// <param name="List<MM_ProductDispatchBillEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<MM_ProductDispatchBillEntity> entity_list, out string msg)
        {
            ...
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_ProductDispatchBill] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[MM_ProductDispatchBill] (
                                            [Id]
                                            ,[DocNum]
                                            ,[Customer]
                                            ,[Status]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,N'{Save_obj.DeliveryType}',N'{Save_obj.Remark}'
        ...
        {
            int result = 0;
            MM_ProductDispatchBillEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_ProductDispatchBill] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_ProductDispatchBillEntity</returns>
        ...
        /// <returns>返回MM_ProductDispatchBillEntity 对象</returns>
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_ProductDispatchBillEntity 列表</returns>
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_ProductDispatchBillEntity> bomService = new RepositoryFactory<MM_ProductDispatchBillEntity>();

        //    MM_ProductDispatchBillEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_ProductDispatchBillDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_ProductDispatchBill_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        public IEnumerable<MM_ProductDispatchBillEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_ProductDispatchBillEntity> MM_ProductDispatchBillEntity_list = db2.FindList<MM_ProductDispatchBillEntity>(sql.ToString());
                return MM_ProductDispatchBillEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_ProductDispatchBillEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_ProductDispatchBillEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                string saveFileName = "成品发货单_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("成品发货单", dt, true);
                //保存
