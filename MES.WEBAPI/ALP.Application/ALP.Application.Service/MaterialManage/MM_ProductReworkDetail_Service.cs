...
{ 
    ...
    /// 1.创建日期: 2021-10-19
    /// 2.创建作者: admin
    /// 3.功能描述: MM_ProductReworkDetailService 业务服务类
    /// 4.任务编号: 成品返工单明细
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_ProductReworkDetail_Service : RepositoryFactory<MM_ProductReworkDetailEntity>
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<MM_ProductReworkDetailEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[MM_ProductReworkDetail] where IsDeleted = 0 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //客户PO号 是否为空进行查询
                if (!queryParam["PO号"].IsEmpty())
                {
                    //sql.Append($" AND PO号 = N'{queryParam["PO号"]}'");
                    sql.Append($" AND PO号 like N'%{queryParam["PO号"]}%'");
                }
                //客户型号 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //成品返工单Id 是否为空进行查询
                if (!queryParam["ReWorkOrderId"].IsEmpty())
                {
                    //sql.Append($" AND ReWorkOrderId = N'{queryParam["ReWorkOrderId"]}'");
                    sql.Append($" AND ReWorkOrderId like N'%{queryParam["ReWorkOrderId"]}%'");
                }
                //唛头号 是否为空进行查询
                if (!queryParam["MarkCode"].IsEmpty())
                {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_ProductReworkDetailEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, MM_ProductReworkDetailEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<MM_ProductReworkDetailEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<MM_ProductReworkDetailEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_ProductReworkDetail] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            MM_ProductReworkDetailEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_ProductReworkDetail] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_ProductReworkDetailEntity</returns>
        public MM_ProductReworkDetailEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回MM_ProductReworkDetailEntity</returns>
        public MM_ProductReworkDetailEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回MM_ProductReworkDetailEntity 对象</returns>
        public MM_ProductReworkDetailEntity Get_ExpressionEntity(Expression<Func<MM_ProductReworkDetailEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_ProductReworkDetailEntity 列表</returns>
        public IEnumerable<MM_ProductReworkDetailEntity> Get_ExpressionList(Expression<Func<MM_ProductReworkDetailEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_ProductReworkDetailEntity> bomService = new RepositoryFactory<MM_ProductReworkDetailEntity>();
        
        //    MM_ProductReworkDetailEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_ProductReworkDetailDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_ProductReworkDetail_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_ProductReworkDetailEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_ProductReworkDetailEntity> MM_ProductReworkDetailEntity_list =  db2.FindList<MM_ProductReworkDetailEntity>(sql.ToString());
                return MM_ProductReworkDetailEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_ProductReworkDetailEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_ProductReworkDetailEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
