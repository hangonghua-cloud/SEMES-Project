...
namespace ALP.Application.Service.MaterialManage
{
    ...
    /// 1.创建日期: 2021-10-14
    /// 2.创建作者: admin
    /// 3.功能描述: MM_ProductMoveRecordService 业务服务类
    /// 4.任务编号: 成品移库记录
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_ProductMoveRecord_Service : RepositoryFactory<MM_ProductMoveRecordEntity>, MM_ProductMoveRecord_IService
    {
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: admin
        ...
        public IEnumerable<MM_ProductMoveRecordEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[MM_ProductMoveRecord] where 1=1 ");
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                //源库位编码 是否为空进行查询
                if (!queryParam["OutLocationCode"].IsEmpty())
                {
                    //sql.Append($" AND OutLocationCode = N'{queryParam["OutLocationCode"]}'");
                    sql.Append($" AND OutLocationCode like N'%{queryParam["OutLocationCode"]}%'");
                }
                //目标库位编码 是否为空进行查询
                if (!queryParam["InLocationCode"].IsEmpty())
                {
                    //sql.Append($" AND InLocationCode = N'{queryParam["InLocationCode"]}'");
                    sql.Append($" AND InLocationCode like N'%{queryParam["InLocationCode"]}%'");
                }
                //移库人 是否为空进行查询
        ...
        public DataTable GetPageDataTableList(Pagination pagination, string queryJson)
        {
            ...
            if (!string.IsNullOrEmpty(queryJson))
            {
                ...
                {
                    //sql.Append($" AND CreateTime = N'{queryParam["CreateTime"]}'");
                    sql.Append($" CONVERT(VARCHAR(10),CreateTime,120)='{queryParam["CreateTime"]}'");
                }
                //最后修改人 是否为空进行查询
                if (!queryParam["ModifyBy"].IsEmpty())
                {
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_ProductMoveRecordEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, MM_ProductMoveRecordEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<MM_ProductMoveRecordEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<MM_ProductMoveRecordEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
                ...
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_ProductMoveRecord] set ");
                            string keyValue = "";
                            //循环实体
        ...
        {
            int result = 0;
            MM_ProductMoveRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
                //删除禁用标记
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_ProductMoveRecord] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_ProductMoveRecordEntity</returns>
        public MM_ProductMoveRecordEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回MM_ProductMoveRecordEntity</returns>
        public MM_ProductMoveRecordEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回MM_ProductMoveRecordEntity 对象</returns>
        public MM_ProductMoveRecordEntity Get_ExpressionEntity(Expression<Func<MM_ProductMoveRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_ProductMoveRecordEntity 列表</returns>
        public IEnumerable<MM_ProductMoveRecordEntity> Get_ExpressionList(Expression<Func<MM_ProductMoveRecordEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_ProductMoveRecordEntity> bomService = new RepositoryFactory<MM_ProductMoveRecordEntity>();

        //    MM_ProductMoveRecordEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_ProductMoveRecordDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_ProductMoveRecord_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_ProductMoveRecordEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_ProductMoveRecordEntity> MM_ProductMoveRecordEntity_list = db2.FindList<MM_ProductMoveRecordEntity>(sql.ToString());
                return MM_ProductMoveRecordEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_ProductMoveRecordEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_ProductMoveRecordEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[WhsCode] as '仓库编码'
                      ,[OutLocationCode] as '源库位编码'
                      ,[InLocationCode] as '目标库位编码'
                      ,[Operator] as '移库人'
                      ,[Remark] as '备注'
                      ,[Creator] as '创建人'
                      ,[CreateTime] as '创建时间'
            ...
            try
            {
                ...
                string saveFileName = "成品移库记录_" + DateTime.Now.ToString("yyyy-MM-dd-HHmm") + ".xls";

                ExcelHelper Excel = new ExcelHelper();
                MemoryStream ms = Excel.DataTableToExcel("成品移库记录", dt, true);
                //保存
