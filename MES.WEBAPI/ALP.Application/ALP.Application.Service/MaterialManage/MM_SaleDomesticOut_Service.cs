...
{ 
    ...
    /// 1.创建日期: 2021-10-29
    /// 2.创建作者: liyongguo
    /// 3.功能描述: MM_SaleDomesticOutService 业务服务类
    /// 4.任务编号: 内销发货单
    /// 5.最后修改日期: 
    /// 6.最后修改作者: 
    /// </summary>
    public class MM_SaleDomesticOut_Service : RepositoryFactory<MM_SaleDomesticOutEntity>
    { 
        /// <summary>
        /// 功能描述: 查询分页列表
        /// 创　　建: liyongguo
        ...
        public IEnumerable<MM_SaleDomesticOutEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[ModifyBy]
                      ,[ModifyTime]
                  FROM [dbo].[MM_SaleDomesticOut] where 1=1  ");
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_SaleDomesticOutEntity> GetList(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT 
                      [Id]
        ...
        public int SaveEntity(string keyValue, MM_SaleDomesticOutEntity entity, out string msg)
        {
            int n = 0;
            msg = "";
            try
        ...
        /// <param name="List<MM_SaleDomesticOutEntity>">实体对象数组</param>
        /// <param name="msg">输出错误内容</param>
        /// <returns>返回int 成功1, 失败0 </returns>
        public int SaveEntity_List(bool IsUpdate, string CreatedByName, List<MM_SaleDomesticOutEntity> entity_list, out string msg)
        {
            int n = 0;
            msg = "";
            try
            {
                if (IsUpdate)
                {
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            ...
                            sql_temp.Append("UPDATE [dbo].[MM_SaleDomesticOut] set ");
                            string keyValue = "";
                            //循环实体
                ...
                else
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append($@"INSERT INTO [dbo].[MM_SaleDomesticOut] (
                                            [Id]
                                            ,[DocNum]
                                            ,[Customer]
                                            ,[Status]
                    ...
                    if (entity_list.Count > 0)
                    {
                        foreach (var Save_obj in entity_list)
                        {
                            sql.Append($@"(
                                ...
                                ,N'{Save_obj.Status}'
                                ,N'{Save_obj.Remark}'
                                ,N'{Save_obj.DeliveryDate}'
                                ,N'{Save_obj.Operator}'
                                ,N'{Save_obj.OperatorName}'
                                ,N'{Save_obj.Creator}'
        ...
        {
            int result = 0;
            MM_SaleDomesticOutEntity entity = this.BaseRepository().FindEntity(keyValue);
            if (entity != null)
            {
        ...
        public int Delete_SQL(string keyValue, out string msg)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append($@"DELETE FROM [dbo].[MM_SaleDomesticOut] WHERE Id=N'{keyValue}'");
                n = this.BaseRepository().ExecuteBySql(sql.ToString());
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="keyValue">主键值</param>
        /// <returns>返回MM_SaleDomesticOutEntity</returns>
        public MM_SaleDomesticOutEntity GetEntity(string keyValue)
        {
        ...
        /// <returns>返回MM_SaleDomesticOutEntity</returns>
        public MM_SaleDomesticOutEntity GetEntityByQuery(string QueryField)
        {
        ...
        /// <returns>返回MM_SaleDomesticOutEntity 对象</returns>
        public MM_SaleDomesticOutEntity Get_ExpressionEntity(Expression<Func<MM_SaleDomesticOutEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        /// </summary>
        /// /// <param name="condition">Expression 条件</param>
        /// <returns>返回MM_SaleDomesticOutEntity 列表</returns>
        public IEnumerable<MM_SaleDomesticOutEntity> Get_ExpressionList(Expression<Func<MM_SaleDomesticOutEntity, bool>> condition)
        {
            // 根据实际情况更换某字段, 这个字段内容在列表是唯一值
        ...
        //    StringBuilder sql = new StringBuilder();
        //    //子表服务类
        //    RepositoryFactory<MM_SaleDomesticOutEntity> bomService = new RepositoryFactory<MM_SaleDomesticOutEntity>();
        
        //    MM_SaleDomesticOutEntity entity = this.BaseRepository().FindEntity(keyValue);
        //    //根据主表在子表的ID与主表主键查找实体, 如果是多条,使用循环遍历删除
        //    MM_SaleDomesticOutDetailEntity bomEntity = bomService.BaseRepository().IQueryable(t => t.MM_SaleDomesticOut_Id == entity.Id).FirstOrDefault();
        //    if (entity != null)
        //    {
        //        //主表删除标记
        ...
        /// <returns>返回分页列表</returns>
        public IEnumerable<MM_SaleDomesticOutEntity> GetList_TestOtherEntity(string checkType, out string msg)
        {
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                ...
                //实体映射查询
                IEnumerable<MM_SaleDomesticOutEntity> MM_SaleDomesticOutEntity_list =  db2.FindList<MM_SaleDomesticOutEntity>(sql.ToString());
                return MM_SaleDomesticOutEntity_list;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public DataTable GetDataTable_TestOtherEntity(string checkType, out string msg)
        {
            ...
            try
            {
                ...
                Data.Dapper.SqlDatabase db2 = new Data.Dapper.SqlDatabase();
                //实体映射查询
                DataTable MM_SaleDomesticOutEntity_DataTable = db2.FindTable(sql.ToString());
                return MM_SaleDomesticOutEntity_DataTable;
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public string GetList_export(string checkType, out string msg)
        {
            ...
            sql.Append(@"SELECT 
                      ...
                      ,[Status] as '发货单状态'
                      ,[Remark] as '备注'
                      ,[DeliveryDate] as '发货日期'
                      ,[Operator] as '发货人'
                      ,[OperatorName] as '发货人名称'
                      ,[Creator] as '创建人'
                      ,[CreatorName] as '创建人名称'
                      ,[CreateTime] as '创建时间'
                      ,[ModifyBy] as '最后修改人'
