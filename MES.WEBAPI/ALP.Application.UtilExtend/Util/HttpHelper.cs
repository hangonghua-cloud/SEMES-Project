...
namespace ALP.Util.Commons
{
    public class HttpHelper
    {
        private HttpClient _httpClient;
        private string _baseIPAddress;//请求的基础IP，例如：http://192.168.0.33:8080/
        ...
        public HttpHelper(string userName, string pwd = "", string uriString = "") : this(uriString)
        {
        ...
        /// <para>黄勇于2016-07-20 19:01:59</para>
        /// </summary>
        ...
        /// Post Dic数据
        /// <para>最终以formurlencode的方式放置在http体中</para>
        /// <para>黄勇于2016-07-15 15:28:41</para>
        /// </summary>
        /// <returns>System.String.</returns>
        ...
        {
            var url = ConcatURL(requestUrl);

            var result = _httpClient.PostAsync(url, content);

            var resp = result.Result.Content.ReadAsStringAsync().Result;
            return resp;
        }

        public string PostNoBaseUrl(object entity, string requestUri)
        {
            string request = string.Empty;
            if (entity != null)
                request = JsonHelper.Instance.Serialize(entity);
            HttpContent httpContent = new StringContent(request);
            httpContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");
            return PostNoBaseUrl(requestUri, httpContent);
        }

        private string PostNoBaseUrl(string requestUrl, HttpContent content)
        {
            var result = _httpClient.PostAsync(requestUrl, content);
            result.Wait();
            var resp = result.Result.Content.ReadAsStringAsync().Result;
            return resp;
        }
        ...
        /// <summary>
        /// 把请求的URL相对路径组合成绝对路径
        /// <para>黄勇于2016-07-21 9:54:07</para>
        /// </summary>
